...
//
// Produces:
//   apps/web/public/lol/manifest.json         — script-internal: paths + hash + bytes (used for diffing)
//   apps/web/src/lol/_shared/manifest.gen.ts  — slim runtime mirror imported by URL helpers
...
const RUNTIME_MANIFEST_PATH = path.join(SRC_LOL_SHARED, "manifest.gen.ts");
...
// Emits a slim TS module mirroring presence in the JSON manifest. The web
// bundle imports this instead of manifest.json so the ~250 KB JSON (paths,
// hashes, byte counts) doesn't get inlined into the JS chunk. Paths are
// derived at the call site from bucket conventions; hashes/bytes are
// script-internal and stay in the JSON. See lol-image-pipeline.md Decision 3.
async function writeRuntimeManifest(manifest: Manifest): Promise<void> {
  const championLines = Object.keys(manifest.champions)
    .sort()
    .map((alias) => {
      const entry = manifest.champions[alias];
      let bits = 0;
      if (entry.square?.path) bits |= 1;
      if (entry.card?.path) bits |= 2;
      if (entry.backdrop?.path) bits |= 4;
      return `  ${JSON.stringify(alias)}: ${bits},`;
    })
    .join("\n");

  const setEntries = (keys: string[]): string =>
    keys
      .sort()
      .map((k) => `  ${JSON.stringify(k)},`)
      .join("\n");

  const out = `// AUTO-GENERATED by scripts/refresh-lol-assets.mts. Do not edit.
...
export const manifestPatch = ${JSON.stringify(manifest.patch)};
...
${setEntries(Object.keys(manifest.items))}
]);

export const runeIds: ReadonlySet<string> = new Set([
${setEntries(Object.keys(manifest.runes))}
]);

export const summonerSpellIds: ReadonlySet<string> = new Set([
${setEntries(Object.keys(manifest.summonerSpells))}
]);

export const roleIconSlugs: ReadonlySet<string> = new Set([
${setEntries(Object.keys(manifest.roleIcons))}
]);
`;
