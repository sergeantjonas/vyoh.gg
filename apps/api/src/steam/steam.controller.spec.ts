...
describe("SteamController", () => {
  ...
  it("delegates to SteamSessionsService.getSessions with the viewer's curation", async () => {
    const sessions: SteamSessions = {
      window: {
    ...
    const stub = vi.fn().mockResolvedValue(sessions);
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: SteamUpcomingService, useValue: {} },
        { provide: SteamPortraitService, useValue: {} },
        { provide: SteamSessionsService, useValue: { getSessions: stub } },
        { provide: SteamGameCurationService, useValue: CURATION_STUB },
        { provide: AuthService, useValue: {} },
      ],
    }).compile();
    ...
    const controller = moduleRef.get(SteamController);
    await expect(controller.getSessions(12, false)).resolves.toBe(sessions);
    expect(stub).toHaveBeenCalledWith(12, NO_CURATION);
  });
});
