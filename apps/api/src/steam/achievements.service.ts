...
export class SteamAchievementsService {
  ...
  // lastUnlockedAt desc).
  // Returns the raw per-achievement unlock timestamps (ascending). The chip
  // detects sessions client-side via inter-unlock gaps — server-side monthly
  // bucketing hid within-month session structure on completed-in-one-day
  // playthroughs. `achievementCount` is the schema size; chip compares it
  // against `unlocks.length` to flag 100% completion.
  async getUnlockTimeline(appid: number): Promise<GameUnlockTimeline> {
    const [rows, meta] = await Promise.all([
      ...
      this.prisma.steamGameAchievementMeta.findUnique({
        where: { appid },
        select: { achievementCount: true },
      }),
    ]);
    return {
      unlocks: rows.map((r) => r.unlockedAt.toISOString()),
      total: rows.length,
