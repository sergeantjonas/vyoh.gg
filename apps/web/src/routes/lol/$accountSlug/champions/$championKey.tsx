...
function ChampionDetailPage() {
  ...
  // Cold arrival: write activeChampion from the URL so the list (mounted by
  // the parent layout) can decorate the matching row as the morph target.
  // Idempotent — re-renders bail when activeChampion already matches.
  useEffect(() => {
    if (activeChampion?.toLowerCase() !== championKey.toLowerCase()) {
      setActiveChampion(championKey);
    }
  }, [activeChampion, championKey, setActiveChampion]);
  ...
  const handleClose = () => {
    navigate({ to: "/lol/$accountSlug/champions", params: { accountSlug } });
  };
