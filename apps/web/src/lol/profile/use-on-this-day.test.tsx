...
describe("useOnThisDay", () => {
  it("does not fetch when no account resolves from the slug", () => {
    ...
    const { result } = renderHook(() => useOnThisDay("ahri"), { wrapper: makeWrapper() });
    expect(result.current.fetchStatus).toBe("idle");
    expect(result.current.isError).toBe(false);
    expect(vi.mocked(fetch).mock.calls.length).toBe(0);
  });

  it("requests the account's on-this-day URL without a session cookie", async () => {
    vi.mocked(useAccountFromSlug).mockReturnValue(account);
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify({ years: [] }), { status: 200 })
    );
    const { result } = renderHook(() => useOnThisDay("ahri"), { wrapper: makeWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    const [url, init] = vi.mocked(fetch).mock.calls[0] ?? [];
    expect(String(url)).toBe(
      "http://localhost:2010/lol/summoners/euw1/Vyoh/Ahri/on-this-day"
    );
