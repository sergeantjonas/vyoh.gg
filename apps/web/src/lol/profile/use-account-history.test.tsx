...
describe("useAccountHistory", () => {
  it("does not fetch when no account resolves from the slug", () => {
    ...
    const { result } = renderHook(() => useAccountHistory("ahri"), {
      wrapper: makeWrapper(),
    });
  ...
  it("requests the account's history URL without a session cookie", async () => {
    vi.mocked(useAccountFromSlug).mockReturnValue(account);
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify({ totalGames: 0, firstGame: null }), { status: 200 })
    );
    const { result } = renderHook(() => useAccountHistory("ahri"), {
      wrapper: makeWrapper(),
    });
    ...
    expect(String(url)).toBe(
      "http://localhost:2010/lol/summoners/euw1/Vyoh/Ahri/history"
    );
