...
describe("WishlistChip", () => {
  ...
  // The forward-looking fact moved to the `On the radar` chip with the calendar's
  // route. This card must not grow one back: it would be reading a list that, by
  // construction, cannot contain the release most likely to be nearest.
  it("keeps the backlog framing even when something dated is on the list", () => {
    mockHook({
      data: {
        steamId: "x",
        items: [
          makeItem({ appid: 3, name: "Oldest", dateAdded: 1_500_000_000 }),
          makeItem({
            appid: 4,
            name: "Imminent",
            comingSoon: true,
            releaseDate: Math.floor(Date.now() / 1_000) + 60 * 60 * 24 * 5,
  ...
  // The row deep-link is this card's own affordance: `?appid` scrolls the list to
  // that row and highlights it, which only works for a title the list holds.
  it("deep-links each preview row to its row on the list route", () => {
    mockHook({
      data: {
        steamId: "x",
        items: [makeItem({ appid: 42, name: "Deep", dateAdded: 1_500_000_000 })],
        fetchedAt: 0,
      },
      isPending: false,
      isError: false,
    ...
    expect(links.map((a) => a.getAttribute("to"))).toContain("/steam/wishlist");
    expect(links.map((a) => a.getAttribute("data-search"))).toContain('{"appid":42}');
  });
