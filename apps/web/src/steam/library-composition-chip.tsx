...
export function LibraryCompositionChip() {
  const query = useSteamLibrarySummary();

  return (
    <FactCardData
      query={query}
      title="Library"
      pendingLabel="Loading library composition…"
      errorLabel="Library composition is unavailable right now."
      emptyLabel="Library hasn't synced yet — first poll lands at 04:00 Brussels time."
      isEmpty={(data) => data.ownedCount === 0}
    >
      {(data) => {
