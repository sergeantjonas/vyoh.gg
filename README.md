...
**Bounded waits + queue observability.** A correctly-tuned rate limiter has a failure mode that masquerades as a network hang: when the slow reservoir snaps to zero (after `syncFromHeaders` reads a saturated header), a queued job sits silently for up to 120 s waiting for the next refresh tick. To prevent this, every `schedule()` call has a 30 s deadline (`Promise.race` against the limiter promise); breaching it throws `RateLimiterTimeoutError`, which the filter maps to HTTP 503 with `"Upstream rate limit saturated — please retry in a moment"`. Two diagnostic hooks make the wait visible while it's happening: a 5 s `still pending` warning logs `N queued, M in flight` so the operator can tell queue-starvation from network-stall at a glance, and a post-execution log fires whenever a job spent >2 s in the queue before running.

**Fetch-level timeout.** Node's `fetch()` has no default timeout — a stalled connection hangs until *something* aborts it. Without intervention, every Riot fetch that doesn't return is a slot wedged in Bottleneck's `executing` count, slowly draining concurrency until the process is restarted. The schedule deadline above frees the *caller's* promise, but the underlying HTTP call keeps holding the slot. Fix: `AbortSignal.timeout(10_000)` on every Riot fetch; on abort, the catch block throws `RiotError(504)` which the filter maps to HTTP 504 `"Riot API timed out — please retry"`. 10 s is generous slack on Riot's typical 100–500 ms response time, and it caps how long a zombie fetch can hold a limiter slot. The 30 s schedule deadline stays as the outer backstop for genuine queue-starvation; the 10 s fetch timeout handles the more common in-flight-stall case.
...
**Background sync — taking Riot off the user's critical path.** Even with all of the above, fetching ~30 cold matches on a personal-tier key (100 req / 2 min) is genuinely tight, and the architectural mismatch is treating Riot as the request-time data source. The fix: a `MatchSyncService` runs `@Cron('*/5 * * * *')` (every 5 min) and walks the whitelisted accounts, fetching match-IDs and backfilling missing details for each one — through the same rate-limited `RiotService`, so it cooperates with on-demand traffic. A per-tick re-entrancy lock prevents overlap. Trends and Champions then read from a separate `GET /matches/cached` endpoint that's pure DB — no Riot, no backfill, no possibility of a 30 s hang on overview screens. The match list keeps its `useInfiniteQuery` infinite-scroll behavior because that *is* the "show me new games" surface; everything else now sees Riot only as a name in the worker's logs. Steady-state Riot traffic for ~5 tracked accounts: roughly **1–3 calls/min**, comfortably under any rate ceiling. See [apps/api/src/lol/match-sync.service.ts](apps/api/src/lol/match-sync.service.ts).
