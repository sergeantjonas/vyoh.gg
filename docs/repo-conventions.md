...
| steam-portrait | ≤ 30 | ≤ 100 ms | ≤ 2 | 24–25 layers / 51–55 ms median ~53 / 0 long tasks / dropped=0 across a 3-run bracket (2026-08-02, first baseline). Cheaper than the profile page it moved off despite carrying more content: two editorial hero bands with no `backdrop-filter` at all, nine frosted chips, and no art. Both its queries are primed in the route loader, so the numbers are a warm render rather than a fetch waterfall. |
