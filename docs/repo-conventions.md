...
The compositor + paint baseline measured by [`tools/perf-probe`](../tools/perf-probe/) (chromium, 1440×900, dev server) defines a budget that any new surface added to the app must respect. The six currently baselined scenarios are `lol-overview`, `lol-champion-panel`, `steam-library`, `recap`, `lol-patches` and `wishlist-upcoming`. Full numbers and the diagnostic trail live in [progressive-paint-audit.md](working-notes/cross-cutting/progressive-paint-audit.md).
...
| steam-library | ≤ 35 | ≤ 150 ms | ≤ 2 | 26 layers / 59–74 ms median ~67 / 0–1 long tasks across a 3-run bracket (2026-08-02, after the portrait layout pass), against 30 / 100 ms / 1 before it — eleven frosted fact cards became nine chips plus two hero bands carrying no `backdrop-filter`, which lowered both the layer count and the raster floor. **The handle predates `/steam/library` existing and measures `/steam`, the profile page**; the virtualised library route has no scenario of its own. |
