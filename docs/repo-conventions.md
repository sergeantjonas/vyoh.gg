...
The Steam session floor is the third instance: `SESSION_MIN_MINUTES`, `isBlipSession()` and `excludeBlipSessions()` in `packages/shared/src/steam/sessions/beats.ts` drop a closed `SteamPlaySession` row the poller saw on a single tick (it closes at its own `startedAt` and reads as zero minutes). Every reader of that table iterates the helper — the sessions page, the `home-*` services, `steam-moments` and the portrait. It takes a row with a null `endedAt` and keeps it, since a live sitting is not a flicker, so a reader that includes in-progress rows needs no second filter. Skipping it was visible in two places: the portrait counted a flicker as a second launch day, enough to rescue a short game into the meaningful cohort, and the recap's first-time receipt could name a flicker as the first sit-down. A new aggregation over that table iterates the helper. Unlinted, like the api response types; it holds by review.
