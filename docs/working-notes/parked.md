...
- **LoL service trio god-class watch (D1)** — [`lol.service.ts`](../../apps/api/src/lol/lol.service.ts) (1113L), [`lol-static-sync.service.ts`](../../apps/api/src/lol/lol-static-sync.service.ts) (1079L), [`lol-analytics.service.ts`](../../apps/api/src/lol/lol-analytics.service.ts) (1024L, counts re-measured 2026-09-06). Trigger: any new arc that would extend one of these past ~1250L (new analytics dimension, new static sync source, new core-service orchestration responsibility). Likely-splits sketched in the note. Standing watch — do NOT pre-emptively split. **The trigger fired once and was actioned:** `lol-analytics.service.ts` reached 1443L and was split on 2026-07-26, moving the five `championKey`-scoped methods to [`lol-champion-analytics.service.ts`](../../apps/api/src/lol/lol-champion-analytics.service.ts) (423L) and leaving 1059L behind. Re-measured 2026-09-06 with none past the trigger; the first cuts are now named in the note (`backfillMissingMatches` beside `match-sync.service.ts`, the wiki client out of static sync). The watch stands for all three files. → [project-hygiene-2026-05-31.md § D1](cross-cutting/project-hygiene-2026-05-31.md)
...
- **API audit A3 — structured logging + request correlation** — Pino + request ids only pay off once logs are aggregated. Trigger: the app is hosted. **The trigger fires on deploy day**, and the landing slot is now named: chunk 3 of [observability-floor.md](ops/observability-floor.md), week one post-deploy rather than pre-launch, because chunk 2 (error tracking) buys most of the same visibility for far less work. Still parked until the box exists. → [audit-api-structure.md](cross-cutting/audit-api-structure.md)
...
- **OG 404s are cached for thirty days** — found 2026-09-25. `OG_CACHE_HEADER` (`s-maxage=2592000`) outranks nginx's `proxy_cache_valid 404 1h`, so an untracked slug or alias, a missing primary owner or an unknown match leaves that card broken in nginx for a month; `og.service.ts` also turns any failure from `getGameRecap` / `getCuration`, a database outage included, into one of those 404s. Parked because it rarely fires (a crawler only fetches cards for links someone shares, and the site links only to pages that exist), it is cosmetic, browsers keep it a day, and the post-deploy cache grep clears it. Fix shape: a one-hour cache on OG 404s, and only a real miss becomes one. Trigger: before actively sending vyoh.gg links to prospects, the first broken card seen in the wild, or the next change to `apps/api/src/og/`. → [post-launch-ops.md](ops/post-launch-ops.md)
- **SOPS + age for `/srv/vyoh/.env`** — encrypted secrets committed to the repo, decrypted on the box with a key held only there; buys versioning, a which-secret-rotated diff, and recovery that is `git clone` plus one key. Parked 2026-09-17 in favour of a hand-written file with the password manager as the system of record, which closes the only real gap (recovery) at no cost. Revisit if a second box or a second person appears, if secrets start rotating often enough that "which one changed" becomes a real question, or if the portfolio value of a documented secret-management story is worth the setup. → [hosting.md § 3](ops/hosting.md)
