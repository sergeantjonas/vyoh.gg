...
**Chunk B shipped 2026-05-15** (commit TBD). Profile tab (`routes/steam/index.tsx`) now has a heading + description. `LibraryCompositionChip` gains a "See the full library →" link matching the WishlistChip and ForeverGamesChip pattern.
...
**Chunk C-4 (deferred from C-3) — game.$appid hero redesign.** Replace the inline capsule card on `/steam/game/$appid` with a `library_hero.jpg` banner + `logo.png` overlay treatment matching Steam's library-page aesthetic. Split out of C-3 at scoping time so the section-layout visual baseline ships independently from the detail-page redesign. File: `apps/web/src/routes/steam/game.$appid.tsx`. Backend already has everything needed — `library_hero.jpg` and `logo.png` are conventional Steam CDN URLs derivable from the appid, no new endpoint required.
