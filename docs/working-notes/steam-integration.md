...
- **Logo-asset gap on recent titles.** The Steam client clearly renders a logo for newer titles like RE Requiem, but `steamLibraryLogoUrl` (unhashed `…/apps/{appid}/logo.png`) returns nothing for them — our text fallback kicks in. `IStoreBrowseService/GetItems` was ruled out 2026-05-15 in S4.6 C-2 (no `logo` key in the payload). **Constraint on diagnosis:** the Steam desktop client doesn't expose DevTools, and the store page layout differs enough from the client that store-page network traffic isn't representative of how the client resolves the logo. Investigation paths that don't require the client: (a) inspect the `appdetails` JSON beyond `filters=screenshots` for any unmapped logo-shaped fields, (b) blind-probe plausible CDN paths (a hashed `library_logo.png` similar to `library_hero`/`library_capsule`, or an alt host), (c) cross-reference community-documented Steam Web API endpoints for asset queries we haven't tried. Current fallback works fine — no urgency, but worth picking up alongside any S4.6 / S7 work that touches game-detail or library art.
