...
- **C-3.1 shipped 2026-05-16** — hovercard shell. `@radix-ui/react-hover-card@1.1.15` added as direct dep on `apps/web` (umbrella `radix-ui` package re-exports it, but the project lists each primitive directly to match the `react-tooltip`/`react-popover`/`react-dialog` pattern). New `apps/web/src/steam/library/library-tile-hovercard.tsx` renders `library_hero.jpg` (with the same `wsrv` silent-404 `naturalWidth === 0` trick the outer `HeroFallback` uses) → blurred `header.jpg` fallback when hero is absent, title, and Steam-faithful "TIME PLAYED" block with stacked rows ("Last two weeks" + "Total"). Local `formatPlaytime` helper differs from the tile's `Xm`/`Yh` — uses Steam's "0 min" / "N min" / "N.N hrs" (sub-10h tenths) / "N hrs" (rounded ≥10h) bands; the never-played zero case renders "0 min" rather than hiding. `library-tile.tsx` wraps the existing `<Link>` in `HoverCardPrimitive.Root` (200ms open / 100ms close) + `Trigger asChild`; content portaled with `side="right"`, `align="start"`, `sideOffset={16}`, `collisionPadding={16}` so the rightmost grid column flips to `side="left"` without the popover crossing the viewport edge. ~256px wide (`w-64`). Adjusted the tooltip class for HoverCard's `data-state` (no `delayed-open` state — that's Tooltip-only). Click still navigates via the tile's `Link` — hover-only popover, no `pointer-events-none` so users can mouse into screenshots once Chunk 2 lands.
- **C-3.2 — Rotating screenshots.** Adds the screenshot feed. Schema: `screenshots Json? @default("[]")` + `screenshotsFetchedAt DateTime?` on `SteamGame` (or a sibling table if normalization wins out at scope time). Shared DTO extension in `packages/shared/src/steam/owned-games.ts` (or a dedicated `SteamGameMedia` type returned by the new endpoint) — `screenshots: { thumbUrl: string; fullUrl: string }[]`. New `screenshot.service.ts` calls `store.steampowered.com/api/appdetails?appids=<id>&filters=screenshots&l=english`, normalizes the path pair, persists with a 30-day TTL, routes through the existing `rate-limiter.service.ts`. New `GET /steam/games/:appid/media` lazy endpoint. Frontend: `useQuery(['steam','game-media',appid], …, { enabled: open })` triggered by hovercard open state; cross-fade screenshots every ~2.5s; pause when `document.visibilityState === 'hidden'`; fall back to hero image when zero/loading. **No prefetch across the full library** — appdetails caps at ~200 req / 5 min per IP, and only a handful of the 167+ owned games will ever be hovered. Persist `screenshots: []` on empty responses to short-circuit re-poll loops.
