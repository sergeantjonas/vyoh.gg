...
- `ops/image-pipeline.md` — images built by GitHub Actions and pushed to GHCR, the box only pulls. Chunk 1 shipped 2026-09-16 (the `images` job in `ci.yml`), chunk 3 the same day (the docs it invalidated); chunk 2 turns `deploy.sh` into pull-not-build and is a prerequisite of runbook step 5. **Read before touching `ci.yml`, `compose.prod.yaml`, `scripts/deploy.sh`, either Dockerfile, or anything that assumes the box builds its own images** — and before arguing the 8 GB box is undersized, since that sizing rests entirely on builds staying off it.
