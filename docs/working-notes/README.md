...
- `lol/match-depth-roadmap.md` — match-detail expansion arc. Phases A/B/C fully shipped (2026-05-16); **Phase D complete 2026-09-01** (LP-overlay per duo last, owner-only); **Phase E complete 2026-09-08** (owner-only rune page in the Your Game tab, Score-of-game grade beside the recap badges); parked sub-items in [parked.md](parked.md). Read this for context before scoping any remaining D/E item.
...
- `steam/play-sittings.md` — shipped 2026-09-14/15, all chunks plus the live-state emphasis pass (live hero, brushed strip, heatmap, ledger, landing chip; route server-primed at 45 kB, refreshed off the root presence poll when a game opens or closes; single-tick rows floored out by `excludeBlipSessions()`, which any surface reading `SteamPlaySession` should iterate; the poll-gap bound widened to an hour after a Steam maintenance window split a sitting on 2026-09-22, and the live hero refetching when the presence poll resumes; the landing chip filled out 2026-09-18 with a per-day strip and session rows, and `CardShell` gained `evidenceFills` for the stretch-grid slack `mt-auto` opens); also records why neither section strip has a Profile tab any more (the identity is the link): the `/steam/sessions` page over observed play sessions and their unlocks, with the beat model that gives a zero-unlock sitting a headline. **Read before adding any surface that reads `SteamPlaySession` or joins unlocks to time windows** — the join slack, the coverage caveat and the off-camera ledger are decided there.
- `steam/hidden-games.md` — shipped 2026-08-21: the per-game privacy overlay (`SteamGameCuration`, two axes `hiddenAt` / `unfeaturedAt`, quarantine-on-purchase, viewer-scoped query keys). **Read before adding any api read path or web query that names a Steam game** — the four-piece viewer-aware contract is codified in [repo-conventions.md](../repo-conventions.md), this note holds the reasoning and the leak inventory.
...
- `ops/post-launch-ops.md` — the day-after companion to the launch runbook, opened 2026-09-17. Routine operations (deploy, roll back, notice something is wrong) plus three chunks the launch itself exposed. **Read before touching `scripts/deploy.sh`** — the highest-value chunk is that its smoke tests loopback only and reported success while the site was unreachable over IPv4.
- `ops/image-pipeline.md` — images built by GitHub Actions and pushed to GHCR, the box only pulls. Chunks 1, 2 and 3 all shipped 2026-09-16 — the `images` job in `ci.yml`, `deploy.sh` turned into manifest-check → pull → `up --no-build`, and the docs they invalidated. Proven on the box only at runbook step 5. **Read before touching `ci.yml`, `compose.prod.yaml`, `scripts/deploy.sh`, either Dockerfile, or anything that assumes the box builds its own images** — and before arguing the 8 GB box is undersized, since that sizing rests entirely on builds staying off it.
