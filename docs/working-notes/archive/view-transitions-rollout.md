...
**Status:** Shipped 2026-05-24 to 2026-05-25 — archived 2026-06-06. Section-shell VT migration, champion + match + Steam library/row morphs, screenshot strip → lightbox, patches cross-version reflow, Steam library + LoL champion-table reorder, and the Safari snapshot-cost bypass all landed (`778a805`, `5193301`, `fbb4703`, `646f7da`, `0c4d134`, `0db5547`, `54dce85`, `816b9c1`, plus chunk-5 + screenshot-lightbox + champion-table-reorder follow-ups). LoL multi-element refinement and LoL match-list queue-filter reorder both closed as abandoned (didn't justify the restructure cost / collided with Motion mount animations). The rect-morph fallback stays in place — it covers reduced-motion + non-VT browsers and removing it is gated on telemetry that isn't worth instrumenting. Catalogued candidates left open (wishlist↔detail, achievements sort/grouping, trophy-case expand) are gated on product/UX decisions that don't exist yet; pick them up via their owning surfaces if the UX ever lands. New per-element morphs reuse the patterns documented below without re-opening this arc.
...
- ✅ **Steam library tile/row reorder** under [`library-controls.tsx`](../../../apps/web/src/steam/library/library-controls.tsx) — shipped 2026-05-24. `withReorderViewTransition` ([`view-transition-nav.ts`](../../../apps/web/src/lib/view-transition-nav.ts)) wraps the discrete pref handlers (`sort`, `playedFilter`, `appTypeFilter`) in `library.tsx`, `flushSync`s the state update so the NEW snapshot captures the post-sort DOM, and tags the transition `intra-section` so the 320 ms iOS-spring curve and `mix-blend-mode: normal` from [`view-transitions.css`](../../../apps/web/src/styles/view-transitions.css) apply. Each li carries an always-on `library-row-${appid}` / `library-tile-${appid}` name; the click-time `steam-game-${appid}-hero/-logo` names on inner imgs carve out as their own VT groups at OLD-snapshot capture. **Engine-gated on WebKit** per the same Steam-shaped snapshot-cost reasoning that drove the router-VT bypass ([`safari-vt-snapshot-cost.md`](safari-vt-snapshot-cost.md)) — Safari snaps as before. Deliberately NOT wrapped: `onQueryChange` (per-keystroke noise) and `onLayoutChange` (tile↔row swap is intentionally instant). `onSelectedTagIdsChange` IS wrapped — the popover doesn't close on each toggle, so the list reorders live underneath; the user can see the morph in any portion of the list not covered by the popover, and on popover close the list is already in its final state.
  - **Microtask deferral inside the helper.** `withReorderViewTransition` runs the `startViewTransition`/`flushSync` pair inside `queueMicrotask` rather than synchronously. The Radix Select / Popover `onValueChange` callback fires from the still-dispatching click event on a portaled item — committing parent state via `flushSync` *during* that event chain leaves Radix's dismissable-layer half-finalised and the next open of any dropdown stops accepting item clicks. One microtask of slack lets Radix complete its close before the snapshot captures.
  - **Virtualization caveat (known limitation).** With the virtualizer rendering only ~15 lis at fixed absolute `translateY` positions and overscan: 4, items that don't appear in both OLD and NEW snapshots cross-fade *in place* at the same screen y, which reads as near-zero motion. Filter changes that swap most of the visible set (e.g. "All" → "Never launched") therefore look almost static even though the morph is firing. Items that *do* appear in both snapshots (popular games near the top across most sorts/filters) morph their y-position visibly. This is inherent to virtualization + per-item VT names and would require a fundamentally different visual treatment (container morph, stagger) to fix — not pursued because the morph still delivers on the "popular games" overlap path and the filter-swap path is a low-information moment to begin with.
- ✅ **LoL champion table reorder** under [`champion-sort-selector.tsx`](../../../apps/web/src/lol/champions/champion-sort-selector.tsx) + role chip strip — shipped 2026-05-25. `withReorderViewTransition` wraps both `setSort` (local state) and `setRole` (router navigate, since role is a search param). Each `<m.li>` in [`champion-table.tsx`](../../../apps/web/src/lol/champions/champion-table.tsx) carries an always-on `champion-${alias}` name; the click-time `champion-${alias}-${position}` applied to the inner cardRef during forward-nav lives in a disjoint namespace so the navigation morph still pairs with the destination hero. The "skip if Motion `layout` is fine" gate didn't apply — Motion's `layout` prop was never actually wired on the m.li (current behavior was snap-on-change), so this is net new motion rather than a replacement. Bounded ~150 champions with no virtualization means dense OLD↔NEW overlap → morph reads richly, unlike the virtualized Steam library.
- ❌ **LoL match list queue filter reorder** — attempted 2026-05-25, **abandoned**. Tried `withReorderViewTransition` wrapping the `navigate()` in [`queue-filter.tsx`](../../../apps/web/src/lol/_shared/queue/queue-filter.tsx) with always-on `match-row-${matchId}` on each virtualized row, then a container-level `view-transition-name: match-list-container` for a coherent height-morph signal. Both reads as no motion. **Root cause:** [`match-list.tsx`](../../../apps/web/src/lol/matches/match-list.tsx) runs Motion entrance animations on every row (`initial={{ opacity: 0 }}` / `animate={{ opacity: 1 }}` / heldDuringSettle / isNew + isFlashNew stagger). At NEW-snapshot capture time the freshly-mounted rows are still at opacity 0 from Motion's mount initial, so the container snapshot is "list with invisible contents" → cross-fades to that → Motion's per-row fade-in then runs in the live DOM *after* the VT ends. The competing animation systems can't be reconciled without either disabling Motion's mount animations conditionally (fragile — they exist for the paged-fetch isNew/isFlashNew flash) or fully migrating the match-list out of Motion onto VT, which is far more scope than the queue-filter morph is worth. Reverted to plain snap-on-change. Lesson generalised below.
