...
**Status:** SHIPPED 2026-05-31. End-state: chrome went from three layers (primary nav + identity header + secondary tabs) to two; section context is restored inside detail pages via Model 3 master→detail (single always-on strip carries detail sub-tabs + a `‹ Matches` breadcrumb); the avatar elevated into a cinematic identity hero on Profile that morphs into the section strip on scroll AND on Profile↔tab navigation (M2 + M2b), on both LoL and Steam. Side-quests landed alongside: hero-avatar activity ring (1.2), topbar `AccountRow` showcase (1.5 LoL portion), narrow-viewport polish pass, display-headline + animated rank crest (1.3b moves 2 + 3). Two micro-items have homes outside this arc — see "Trailing items" at the end. Full shipped-chunk-by-chunk record below. Removed from [open-work.md](../open-work.md).

**Original (active-arc) Status line, preserved as historical record:** **Chunk 1.1 SHIPPED 2026-05-30** (merged tiered strip, Model 3 master→detail nav, breadcrumb section-switcher, strip-action-icon parity). **Chunk 1.5 LoL portion SHIPPED 2026-05-30** (topbar `AccountRow` showcase: 44px avatar, last-played splash wash, open-stagger; Steam-card variant deferred — blocked on Steam growing a nav dropdown. **Live "in-game now" pill DROPPED 2026-05-30** — see the 1.5 entry). **Chunk 1.3a LoL portion SHIPPED 2026-05-30** — landed as a **cinematic champion-splash hero** (`LolIdentityHero`), not the bare block: signature-champion splash (sharp 1280px `splash` variant) brought into focus over the ambient backdrop, avatar + `Vyoh#Ahri` + rank crest + last-played, champion-accent glow, Ken-Burns + pointer parallax. Avatar/square icon proxies bumped to native 128px. **M2 (scroll-collapse morph into the strip) SHIPPED 2026-05-30**, plus a scope extension **M2b (the same identity morphs on Profile↔tab navigation) SHIPPED 2026-05-30** — see the 1.3a entry. (Perf-validated on host Chrome 2026-05-30: nav-morph INP 8ms, no regression.) **Chunk 1.3a Steam portion SHIPPED 2026-05-30** — landed as a cinematic Steam identity hero (`SteamIdentityHero`) with live-game-or-top-played backdrop, avatar + persona name + identity headline (`Member since · Level · top N%`) + presence line, plus a 4-cell `SteamStatBand` (recently played + total playtime + most played + library played with bar). **Steam M2 + M2b morph parity SHIPPED 2026-05-30** — Steam now mirrors LoL's hero↔strip scroll-collapse morph and the Profile↔tab nav-morph driver (WebKit-bail intact for the Safari snapshot-cost gate). See the 1.3a entry for Steam-specific decisions. **Chunk 1.2 SHIPPED 2026-05-30** — hero-avatar activity ring on both Steam (7-state presence palette, brand-aligned: `online` flipped emerald→sky to reserve emerald for in-game) and LoL (binary in-live-game emerald). Pulse vocabulary differentiated by scale: avatar-scale halo gets calm `animate-pulse` + blur ("live, but settled"), the inline presence dot keeps `animate-ping`. **Narrow-viewport polish pass SHIPPED 2026-05-31** — primary nav: orb-only logo + icon-only nav items below `sm`, dropdown positioning override for the shadcn-default `md:absolute` defect; secondary nav: `heroOwnsIdentity` dropdown row-promote at scroll-top, controlled-open dropdown to fix morph-`preventDefault` lingering, pathname-driven `compact` reset to fix the stale-flag bug across all viewport widths. See the dedicated entry below. **Chunk 1.3b SHIPPED (partial) 2026-05-31** — display-type headline (Geist `wght` axis + tighter tracking + bigger scale + heavier drop-shadow on the LoL hero name) and animated rank crest (spring reveal layered with subliminal ambient drift, replays on every `compact` flip). Move 4 (splash crossfade into separate rank cards) DROPPED as architecturally moot since 1.3a shipped with the rank strip fused into the hero card; Move 1 (avatar hover parallax) stays parked pending the [pointer-parallax-splash](pointer-parallax-splash.md) hook arc. Promoted to [open-work.md](../open-work.md).
...
- ~~**Primary-nav (`nav.tsx`) responsiveness at low widths.**~~ **SHIPPED 2026-05-31** — see "Narrow-viewport polish pass" below.
- ~~**Topbar LoL account picker behaves oddly at small viewports.**~~ **SHIPPED 2026-05-31** — root cause turned out to be a shadcn-stock defect in [`navigation-menu.tsx`](../../apps/web/src/components/ui/navigation-menu.tsx), not anything specific to the picker: `NavigationMenuContent` has `md:absolute md:w-auto` in its base className, so below 768px the dropdown was `position: static` with `width: 100%` and inlined itself into the nav's flex layout, shoving the bar around. Fix is a per-instance className override in `nav.tsx` forcing `!absolute` at all widths plus `max-w-[calc(100vw-1.5rem)]` so the 288px panel can't overflow on the narrowest phones. See "Narrow-viewport polish pass" below.
...
> **LoL portion SHIPPED (2026-05-30).** The topbar LoL `AccountRow` ([`nav.tsx`](../../apps/web/src/components/nav.tsx)) now ships the three agreed visual moves: avatar 28→44px (`size-11`, rank emblem kept), a faint last-played-champion splash wash from `championBackdropSplashUrl(summary.lastPlayedChampionAlias, ddVersion)` (right-anchored mask, `opacity-0.12` + `blur-2px`, overscanned + clipped, `aria-hidden`), and a per-row open-stagger via the `.nav-account-row` keyframe (`--row-index` × 40ms, reduced-motion → `animation: none` in `index.css`). Tests added to `nav.test.tsx` (splash present/absent, per-row stagger var). **Deferred (Steam single-card variant):** blocked — the Steam nav is a plain `SimpleNavItem` with no dropdown yet; trigger-flagged at the Steam nav item in `nav.tsx` and in [steam-lol-parity.md](steam-lol-parity.md) § "Deferred — trigger-gated", pick up when Steam grows a nav menu. **DROPPED — live "in-game now" / last-active pill (2026-05-30).** Originally deferred pending live-presence plumbing; on review it's redundant with **Chunk 1.2's hero presence ring** which now covers the "am I in a live game" read on the active account's Profile hero. The pill's only unique value left was "see which *non-active* alt is live without clicking in," which is a niche multi-account-alt case that doesn't justify per-row live-game polling. If "find the live alt at a glance" becomes a real need later, it returns as a tiny scope (one live-game query reused across dropdown rows). **Perf gate — CLEARED 2026-05-30.** Host-Chrome re-baseline on the real ~7-account list (worst case): Performance record over the open showed no long task >50 ms / no dropped frames, second open is all cache hits, Web Vitals all green. Full numbers in [perf-baseline.md](perf-baseline.md) § "dropdown-open re-baseline". No fix sub-chunk needed. **The LoL portion of 1.5 is fully done**; only the deferred Steam single-card variant remains.
...
### Narrow-viewport polish pass (2026-05-31)

> **SHIPPED 2026-05-31.** A focused pass on both bars at narrow viewports, fixing one shadcn-default defect, two scaling problems, and two state-management bugs that only surfaced once the morph wiring went in. Discovered iteratively from owner screenshots; consolidated here so the rationales survive when the diffs don't.

**Primary nav ([`nav.tsx`](../../apps/web/src/components/nav.tsx)):**

- **LoL dropdown was breaking the layout** at <768px. Root cause: [`navigation-menu.tsx`](../../apps/web/src/components/ui/navigation-menu.tsx) (the shadcn-stock primitive) carries `md:absolute md:w-auto` in `NavigationMenuContent`'s base className. Below 768px the dropdown was `position: static` with `width: 100%` — it took inline space in the nav's flex layout when opened and reshuffled every sibling. Fix is a per-instance override in `nav.tsx` (`!absolute !w-72 max-w-[calc(100vw-1.5rem)]`); not mutated in the shared primitive because other consumers may rely on its static-at-narrow behaviour.
- **Bar overflow at narrow widths**. Three-step collapse mirroring the existing `⌘K → 🔍` pattern: orb-only logo (`vyoh.gg` text becomes `hidden sm:flex`, `aria-label` carries the affordance), inter-item gap tightens (`gap-3 sm:gap-6`), and nav-item labels collapse to icons (`hidden sm:inline` on each label span + `aria-label` on the link/trigger). Future-proof for "more nav items coming" per owner — bar fits ~10 items on a 360px viewport now instead of overflowing at 5.

**Secondary nav ([`section-shell.tsx`](../../apps/web/src/_shared/section-layout/section-shell.tsx) + [`section-nav.tsx`](../../apps/web/src/_shared/section-layout/section-nav.tsx)):**

- **Empty-identity row whitespace at scroll-top.** With M2 wired, the hero owns identity at scroll-top → the strip's identity slot resolves to `null` → at <640px (own-row dropdown tier), row 1 read as dead space with the dropdown sitting alone on row 2. New `heroOwnsIdentity?: boolean` prop on `SectionShell`: when true and not compact, the dropdown promotes from `order-last basis-full` (row 2) to `order-3 basis-0 grow` (row 1). Match-detail and other non-hero routes are unaffected because they leave the prop at its default `false`.
- **Dropdown lingering open after morph navs.** Radix's default close-on-click checks `event.defaultPrevented`; the identity morph driver calls `preventDefault()` to hand-roll its own VT, so Radix didn't close. Visible bug: profile→tab and tab→profile navs at narrow widths (the only paths where the morph + dropdown overlap). Fix: `SectionTabsDropdown` is now controlled-open via `useState`, and the item's onClick wrapper calls `setOpen(false)` explicitly after the consumer's `onSelect`. Sidesteps the `defaultPrevented` check entirely.
- **`compact` state stuck across navs (all viewport widths).** `SectionShell` is the section layout — it doesn't remount on tab nav, so `compact` persisted across pathname changes. The morph driver does `mainScrollRef.scrollTop = 0`, which *should* fire a scroll event and reset compact — but the listener has a 400ms cooldown (defending against the scroll-anchoring flap loop) that can block the exit when the user scrolled into compact and clicked the dropdown quickly. Symptom: returning to Profile from a scrolled-down other tab left the hero identity faded (`compact && opacity-0` still applied to the morphing avatar/name) and the strip identity rendered, until the user manually scrolled. Fix: new `pathname?: string` prop drives a `useEffect` that resets `compact` + `bandOpaque` + clears the cooldown timestamp on every route change. Cooldown-independent and matches the scroll-reset that `useScrollResetOnNav` already does in the section root.

**Carry-forwards:**

- The shadcn `NavigationMenuContent` default could itself be patched to drop the `md:` prefix (would benefit every future consumer), but doing so changes shared-primitive behaviour; deferred unless a second consumer hits the same defect.
- Bar still uses unbranded icons for "Status" (Activity icon) and others — fine today, may need tooltips if more items land below `sm` and the icon vocabulary gets harder to distinguish.

### Chunk 1.2 — Hero avatar activity ring

> **SHIPPED 2026-05-30.** Steam: 7-state presence ring via the existing [`PRESENCE`](../../../apps/web/src/steam/profile/steam-identity-hero.tsx) map (the `dot`/`ring` pair reads from one source of truth so they can't drift); `online` flipped emerald→sky so emerald stays reserved as the cross-stream **live-activity** vocabulary. LoL: binary in-live-game emerald ring on the hero avatar via `useLiveGame(account)` already wired in [routes/lol/$accountSlug/index.tsx](../../../apps/web/src/routes/lol/$accountSlug/index.tsx) (the same query `<LiveGameChip>` reads; TanStack Query dedupes the in-flight fetch). **Live treatment** is solid emerald `ring-2` plus a sibling **breathing halo** (`animate-pulse` + `blur-md` at `-inset-1`) — softened from the originally-specced `animate-ping` after one round of owner review: at avatar scale the scale-out ping felt aggressive, while the inline presence-dot keeps `animate-ping` because the small footprint reads as urgent in a good way. Reduced motion drops the halo; the solid emerald ring stays so the signal still reads. **No shared `AvatarRing` primitive** — the cardinality gap (7 vs 1) and divergent surrounding chrome would have made the abstraction lossy; promote later if a third surface needs it. Tests in same commit: per-state ring color on Steam (parametrised), in-game emerald-override + halo present on both heroes, axe stays clean.
...
**Steam M2 + M2b morph parity SHIPPED 2026-05-30.** The Steam strip now mirrors the LoL pattern end-to-end. **M2 (scroll-collapse):** shared Motion `layoutId` literals in [`identity-layout.ts`](../../../apps/web/src/steam/profile/identity-layout.ts) (`steam-identity-*` namespaced so they can't collide with LoL's `lol-identity-*`); `SteamIdentityHero` ([steam-identity-hero.tsx](../../../apps/web/src/steam/profile/steam-identity-hero.tsx)) reads `compact` from `useSectionShellState`, drops the `layoutId` + `data-identity-*` markers when compact, and fades its avatar wrapper + name to `opacity-0` so Motion still measures the hidden box as the morph source. The strip identity slot (`SteamIdentity` in [routes/steam.tsx](../../../apps/web/src/routes/steam.tsx)) returns `null` on `/steam + !compact` (hero owns identity) and renders unconditionally otherwise, carrying the same conditional `layoutId` + unconditional identity markers as LoL. **Surrounding chrome reveal**: identity headline + presence line use `detailReveal(delay 0.22, duration 0.26)`; `SteamStatBand` accepts `compact?: boolean` and fades its grid via the same delayed pattern `HeroRankStrip` uses (delay 0.3, duration 0.3) — so the surrounding text and glass cells arrive *after* the morph lands rather than sitting there waiting. **M2b (nav-morph):** [`identity-morph-nav.ts`](../../../apps/web/src/steam/profile/identity-morph-nav.ts) mirrors the LoL driver shape — same imperative VT pattern (synchronous source-name → start VT → clear-navigate-name destination → cleanup on `finished`). One Steam-specific gate: **bail on `isWebKit()`** so Safari/iOS falls through to the existing `safari-slide-in-from-*` CSS substitute, since running a second hand-rolled VT on Steam-shaped DOM would re-introduce the exact snapshot cost the intra-section bypass was paid to avoid. Wired via `SectionTab.onSelect` on each Steam tab in [routes/steam.tsx](../../../apps/web/src/routes/steam.tsx). Tests: 9 driver cases in [identity-morph-nav.test.tsx](../../../apps/web/src/steam/profile/identity-morph-nav.test.tsx) (including the WebKit-bail and the `library↔game-detail returns false` short-circuit) + compact-gated marker/opacity cases in [steam-identity-hero.test.tsx](../../../apps/web/src/steam/profile/steam-identity-hero.test.tsx).
...
### Chunk 1.3b — Profile-tab visual flair

> **SHIPPED 2026-05-31 (partial — moves 2 + 3).** Two of the four originally-scoped moves landed; one is dropped as architecturally moot; one stays waiting on its dependency arc. The original "structure first, polish second" framing was partially moot once 1.3a shipped as cinematic rather than bare, so this chunk's real job was to push the headline display further and add the rank-crest motion moment.
>
> **Move 2 — display-type headline (SHIPPED on both LoL + Steam).** The LoL hero's `<m.h2>` ([identity-hero.tsx](../../../apps/web/src/lol/profile/identity-hero.tsx)) bumps from `font-semibold text-3xl/sm:text-4xl tracking-tight` to `font-bold text-4xl/sm:text-5xl -tracking-[0.02em] drop-shadow-md`, with the `#tag` segment dropped to `font-medium` so it reads as caption against the display name. The Steam hero's persona-name `<m.h2>` ([steam-identity-hero.tsx](../../../apps/web/src/steam/profile/steam-identity-hero.tsx)) gets the same class bump for cross-app parity (no `#tag` segment to differentiate). Geist ships only the `wght` axis (no `opsz`) so the "display look" comes from weight + tracking + size + drop-shadow rather than a true optical-size cut — the [editorial-typography](editorial-typography.md) arc verified Geist's axes in its Chunk 1.
>
> **Move 3 — animated rank crest (SHIPPED, LoL-only).** Each [`RankCell`](../../../apps/web/src/lol/profile/hero-rank-strip.tsx) wraps its emblem in a two-layer motion stack: outer `m.div` carries the **reveal** (initial opacity 0 + scale 0.6 + blur(4px) → spring into opacity 1 + scale 1 + blur(0), delay 0.42 so each crest "lands" *into* the strip after `HeroRankStrip`'s own 0.3-delay fade-in), inner `m.div` carries the **ambient drift** (rotate keyframes `[0, 1.2, 0, -1.2, 0]` over 12s on infinite repeat, subliminal but contributes to "the surface is alive"). The tier-glow halo rides inside the drift wrapper so the emblem-and-halo unit breathes as one piece. Both layers replay on every `compact` flip false (not mount-only) so the crest lands again on scroll-up from the collapsed strip. Reduced motion: reveal and drift both suspended; emblem renders static. `RankCell` gained `compact` + `reduced` props threaded down from `HeroRankStrip`. **Not applied to Steam:** the Steam parallel of the rank strip is `SteamStatBand` whose prominent visuals are publisher wordmarks (ELDEN RING, Silksong, etc.) — wordmarks are designed to be read statically, and a continuous ±1.2deg drift would look off on publisher art the way it doesn't on a tier sigil. The design intent diverges enough that extending move 3 to Steam would be a different design call rather than parity; left for a future pass if/when Steam's stat-band entries get their own cinematic treatment.
>
> **Move 4 — splash transition into rank cards (DROPPED).** The 1.3b spec was written when the rank tiles were going to be **separate elements below** the hero (the original bare-1.3a). 1.3a actually shipped with `HeroRankStrip` fused into the hero card as the glass strip on its bottom edge, sharing the splash backdrop and the card frame already. There is no "rank cards below" to crossfade *into* — the visual gap move 4 was designed to fill no longer exists. Dropped as moot.
>
> **Move 1 — avatar hover parallax (STILL DEFERRED).** Pulls from [pointer-parallax-splash](pointer-parallax-splash.md) which is still Planned. Not strictly blocked (the hero already implements its own inline pointer parallax on the splash — line 121-137 of identity-hero.tsx — so a second instance for the avatar could just inline another), but framed as "would benefit from the shared `usePointerParallax` hook extraction" the parallax arc will produce. Owner call: not load-bearing on top of the existing splash parallax, leave parked until the dependency arc lands and the hook can be consumed cleanly.

**Original goal:** layer the cinematic moves onto 1.3a's identity block.

- ~~**Parallax on the avatar frame on hover**~~ — DEFERRED (see above).
- **Display-type headline with variable-font weight + optical-size axis** — SHIPPED 2026-05-31 (weight + tracking only; Geist has no `opsz`).
- **Animated rank crest** — SHIPPED 2026-05-31.
- ~~**Splash transition into the rank cards**~~ — DROPPED (architecturally moot since 1.3a; see above).

**Gating decision (now resolved):** 1.3b shipped when [editorial-typography](editorial-typography.md) landed (2026-05-27). The original "at least one of pointer-parallax-splash or editorial-typography" gate held — typography was the gate, parallax wasn't load-bearing on top of the existing splash parallax.
...
1. **Seam-straddle behaviour with the ⌘K palette open — moot.** The seam-straddle was dropped during 1.1 implementation (`LolIdentity` is plain inline). No avatar poking through anything.
2. **Steam Profile data dependencies — resolved 2026-05-30.** `memberSinceUnix` (`timecreated`) and live `currentGame` (`gameid`) are both ingested and consumed by the shipped `SteamIdentityHero` (identity headline + in-game backdrop). No ingest expansion was needed.
3. **Coordination with [live-presence-chip](live-presence-chip.md) — resolved 2026-05-30.** 1.2 shipped without a separate live overlay (the breathing halo IS the live signal, driven by poll-based `useLiveGame` / `playerState`); 1.5's "in-game now" pill was dropped (covered by the hero ring). If `live-presence-chip` ships an SSE-pushed signal later, the hero ring inherits it for free via the same hook.
4. **Avatar size in the seam — moot.** Seam dropped (see #1). Strip avatar tuned during 1.1 implementation; hero avatar is `size-20` / `size-24` (sm:).
...
## Trailing items (homed elsewhere)

Two micro-items remain from this arc, both trigger-gated on external arcs rather than on anything inside this one. Their proper homes are:

- **Steam single-card variant of the 1.5 picker showcase.** Blocked on Steam growing a nav dropdown (currently a plain `SimpleNavItem`); a one-item dropdown is poor UX, so this only earns its place when Steam nav has more to host. Trigger-flagged at the Steam nav item in [`nav.tsx`](../../apps/web/src/components/nav.tsx) and recorded in [steam-lol-parity.md § "Deferred — trigger-gated"](steam-lol-parity.md). Pick up there when Steam grows a menu.
- **1.3b Move 1 — avatar hover parallax.** Not strictly blocked (the hero already implements its own inline pointer parallax on the splash), but framed as "would benefit from the shared `usePointerParallax` hook extraction" that [pointer-parallax-splash](pointer-parallax-splash.md) will produce. Lives more naturally as a small sub-step of that arc when it starts — the avatar consumes the same hook the splash arc proposes. Not load-bearing on top of the existing splash parallax.
