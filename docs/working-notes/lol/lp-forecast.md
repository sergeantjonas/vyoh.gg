...
**Status:** Active — Phase LP1 (directional-only verdict) shipped 2026-05-14. Phase LP2 shipped 2026-05-20 (retroactive signal-replay + directional-accuracy calibration, "How is this computed?" disclosure). Phase LP2.6 shipped 2026-05-20 (signal-builder thresholds tuned to fix stuck-positive tilt/champion and noisy time-slot reads on high-WR accounts; per-signal sample-size weighting deferred until variance returns). Phase LP3 (personal linear fit) remains the long-tail target. See [open-work.md](../open-work.md).
