...
- **2026-06-18** — D.3 (damage-profile radar) shipped — the first remaining D-item that is **reusable on both Profile and Champion detail** per the deliverable. **Normalization decision:** "normalized by role" interpreted as **share-of-team per metric** (owner / team-total, averaged over games), *not* a ratio-vs-owner-role-median. Rationale: share-of-team is role-fair without any external baseline (a support reads low-damage / high-vision naturally), and — load-bearing — it stays meaningful at *both* scopes, whereas a self-vs-role-median ratio collapses to ~1.0 at profile scope (the subject set = the baseline set). A cross-player role baseline isn't available (we only store the owner's games). `getDamageProfile(region, name, tag, championKey?, count)` is one method serving both routes: Match query filtered to `teamPosition: { not: "" }` (Rift only — vision share is degenerate in ARAM/Arena) + optional champion, remake-excluded, then `MatchDetailCache` for per-teammate totals (damage to champions, damage taken, vision score, minions+neutral CS); full-roster teams only (`team.length >= 4`, skips Arena subteams). Each metric's mean is over the games that had a non-zero team total for *that* metric (independent per-metric counts). New `DamageProfile` shared type. **Web:** `useDamageProfile` (championKey-optional, hits the champion-scoped or profile route) + reusable `DamageProfileRadar` — a Recharts `RadarChart` (already in-stack from the live-page composition radar; no new dep) with 4 spokes scaled `share × 5` so the dashed reference polygon at radius 1 = an even five-way split, palette via `chart-palette` (`CHART_SERIES`/`CHART_GRID`/`CHART_AXIS`), wrapped in `ConclusionCard` (works on both surfaces), verdict headlines the top-share axis, + a 4-up absolute-% legend below the radar. Mounted in a `CvSection(340)` on Champion detail (below `ChampionLanePhase`) and Profile (below `ProfileCarryProfile`); `MIN_GAMES=5`. Tests: api 4 (share averaging w/ per-metric counts, champion+teamPosition where-clause scope, full-roster skip → empty, whitelist guard) + web 4 (pending null, low-sample empty state, top-axis verdict + per-axis %, axe scan). Remaining unblocked D-items after this: D.6 (objective participation — timeline alive-during-objective) and D.7 (ARAM dashboard — large, needs its own chunk plan).
