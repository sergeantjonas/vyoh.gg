...
**Status:** Shipped — Phases A + B + C fully shipped (2026-05-16). **Phase D complete 2026-09-01** (squad detection + per-duo champion pairs 2026-06-16, D.9/D.2/D.4/D.5/D.3 2026-06-17→18, D.6 2026-06-21, D.7 2026-06-22, LP-overlay per duo 2026-09-01 — owner-only `GET …/duos/lp`, the LP split on the Profile duo rows, and a duo picker on the LP-history chart ringing the snapshots after that duo's games). **Phase E complete 2026-09-08** (full rune page owner-only, composite Score-of-game grade beside the badges). Both tracked in [open-work.md](../open-work.md); parked sub-items (build-order component-collapse, hover-highlight, boot collapse, soul drake type) in [parked.md](../parked.md). Full Status section with commit refs is [further down](#status).
...
- ~~**Full rune page panel**~~ **Shipped 2026-09-08**, owner-only: all 6 runes + 3 stat shards for the owner in the Your Game tab. The other nine participants are stored keystone-only and cannot be refetched, so a per-participant version is out. See [Status](#status).
- ~~**Composite "Score-of-game" grade**~~ **Shipped 2026-09-08** as an addition beside the Phase A badges, not the replacement first pitched: the owner judged the badges not cluttered and wanted the grade anyway. `scoreOfGame()` in `@vyoh/shared` averages each participant's percentile over the six badge metrics; see [Status](#status).
...
- **Phase D** — partially shipped. **D.10 (duo / squad detection)** v1 shipped 2026-05-10 (`42549b4`): Profile section showing top recurring teammates with W-L + most-played champion. **Champion synergy chord** shipped 2026-05-11 (`dafd316`) as a follow-on to D.10 — new `GET /lol/.../champion-pairs` endpoint aggregates per-match `(yourChamp, teammateChamp)` pairs; `ProfileSynergy` renders a `@visx/chord` bipartite layout (your champ pool on one side, teammates' picks on the other) with ribbon weight = games played together, ribbon color = win rate. **D.1 (death heatmap on Champion detail)** shipped 2026-05-11 (`0fb4720`) as a **minute × matchup** grid rather than position-on-Rift — reads `deathTimings` + `laneOpponent.championName` from `MatchSummary`, no backend changes. Position-on-Rift heatmap is now paired with Phase E (op.gg overlay) since both need event x/y projected onto MatchSummary — better to build together. **D.8 (build-order delta)** shipped 2026-05-11 (`d16b171`) as a `d3-sankey` diagram on Champion detail — new `GET /lol/.../champions/:championKey/build-flow` endpoint intersects timeline `ITEM_PURCHASED` events with `Match.items` final inventory to get completion-order item sequences; Sankey shows item-1 → item-2 → item-3 paths with lift-vs-baseline color encoding. **Match-list duo highlight** shipped 2026-06-15: `getDuos` now returns `matchIds[]` per duo (the shared matches within the detection window); `MatchList` builds a `matchId → Duo[]` reverse index from the same cached `useDuos` query and threads matched duos into each `MatchRow`, which renders a "with {duo}" badge (lucide `Users`, sky-toned, mirrors the `vs` lane-opponent label). Windowed to the duo-detection scope (~last 100 matches) — older paged-in rows don't badge. **Duo-detection precision pass (same day):** pure same-team co-occurrence over-counts random matchmaking repeats (worst in high elo / low-pop regions / small queues), and **Match-V5 exposes no premade/party id for SR** (confirmed via live probe — 133 participant fields; the only party-shaped fields, `playerSubteamId`/`subteamPlacement`, are Arena's 2v2v2v2 subteams, zeroed on CLASSIC). So inference is the only route. `getDuos` now gates the ≥3-game floor on a **temporal-clustering** signal: a teammate qualifies only with a same-session shared-game pair (≤3h apart) OR high volume (≥6 games) — random repeats are scattered in time, premades play back-to-back. The match-list badge adds a stricter volume cut (`DUO_BADGE_MIN_GAMES = 5`) on top, since a per-row badge makes a more confident claim than the soft-ranked profile list. (Arena could later use `playerSubteamId` for exact duo partners, but it's niche and not cached.) **Squad detection (3+ groupings) — API shipped 2026-06-16** (`GET /lol/.../squads`): `getSquads` buckets recurring ≥2-teammate *sets* (every size-≥2 subset per match), reuses the duo temporal-clustering gate, and drops nested subgroups fully explained by a larger stack — see decision log. The duo gate + cache prelude were extracted into shared helpers (`qualifiesAsRecurring`/`hasSameSessionPair`/`ownerTeammates`/`loadOwnerMatchCache`) consumed by both `getDuos` and `getSquads`. The Profile `ProfileSquads` web surface shipped 2026-06-16 too: a separate "Squads" section beside Duos (`useSquads` hook mirroring `useDuos`), rendering overlapped member avatars + names + stack label (Trio / 4-stack / 5-stack) + W-L/WR, and rendering **nothing** (no empty state) in the common no-3-stack case so the Profile carries no permanent empty zone — it's mounted un-CV-gated for that reason. **Per-duo champion pairs shipped 2026-06-16** (see decision log): `getDuos` now accumulates a per-duo `(yourChamp|teammateChamp)` pair map (owner's own champ captured via the extended `ownerTeammates`), ranked by games + capped at top 6, surfaced on `Duo.championPairs` (reuses the existing `ChampionPair` shape). `ProfileDuos` became an expandable Accordion (mirroring `ProfileSynergy`): the collapsed row keeps the duo summary, the expansion lists each champion combo with a win-rate bar — distinct from the champion-keyed global Synergy chord because it's scoped to one recurring person. **D.9 (rune-page diversity) shipped 2026-06-17** — `GET …/champions/:championKey/rune-diversity` reads the owner's keystone per match from `MatchDetailCache` (`perks.styles[0].selections[0].perk`; no schema change, no timeline dep), tallies keystone→{games,wins}; `ChampionRuneDiversity` renders a row list of keystone WR bars (absolute-WR-tier colored + lift-vs-baseline ▲/▼/· marker) below `ChampionBuildPath`. **D.2 (lane-phase differential) shipped 2026-06-18** — `GET …/champions/:championKey/lane-phase` resolves the owner + `laneOpponent.puuid` participant ids in the stored timeline and averages the owner-minus-opponent CS@10/CS@15/gold@10 differentials (+ per-metric ahead-rate) read live from `MatchTimelineCache` (no migration — opponent per-frame stats aren't projected onto `Match`); `ChampionLanePhase` renders 3 sign-colored stat tiles below `ChampionRuneDiversity`. **D.4 (first-blood/first-tower) shipped 2026-06-18** — `GET …/objective-firsts` reads the owner's recent non-remake `MatchDetailCache` rows for personal `firstBloodKill` + team `objectives.tower.first`, each tallied with the win count; `ProfileObjectiveFirsts` renders a 2-stat frosted strip (rate % + "win when you do") near the other Profile counter strips. **D.5 (carry vs carried) shipped 2026-06-18** — `GET …/carry-profile` ranks the owner within their team by `totalDamageDealtToChampions` per `MatchDetailCache` row (full-roster teams only) and splits wins/losses into top-3 vs bottom-2 damage buckets; `ProfileCarryProfile` renders a plain-language verdict + 2-tier WR comparison strip with neutral framing. **D.3 (damage-profile radar) shipped 2026-06-18** — `GET …/damage-profile` (+ champion-scoped `GET …/champions/:championKey/damage-profile`) computes the owner's mean share of their team's totals for damage to champions / vision / CS over recent non-remake positional games (full-roster teams only) from `MatchDetailCache`; the reusable `DamageProfileRadar` plots them as a 3-spoke Recharts radar scaled so a dashed reference polygon = an even five-way split, mounted on **both** Champion detail (below `ChampionLanePhase`) and Profile (below `ProfileCarryProfile`). **Damage-taken axis dropped 2026-06-19** (originally specced as a 4th axis): the lean `MatchDetailCache` projection (`match-projection.ts`) strips `totalDamageTaken` from non-owner participants, so its team-share always read 100% (the owner is the only contributor) and is unrecoverable for historical matches without a Riot re-fetch. Gold share was considered as a 4th axis but rejected — it tracks damage share too closely to add signal; Damage/Vision/CS are orthogonal (support reads low-damage/low-CS/high-vision). **D.6 (objective participation) shipped 2026-06-21** (`GET …/objective-participation` + `ProfileObjectiveParticipation` — owner takedowns ÷ team objective kills for dragon/baron/herald from `MatchDetailCache`, takedown framing chosen over the specced alive-during-objective; see decision log). **D.7 (ARAM dashboard) shipped 2026-06-22** (`GET …/aram-profile` + `ProfileAramDashboard` — queue-isolated ARAM Profile section: WR/KDA + per-game heal+shield / damage-taken / self-mitigated + most-played ARAM champs, from `MatchDetailCache`; additive, existing surfaces unchanged; see decision log). **LP-overlay per duo — shipped 2026-09-01, closing Phase D:** owner-only `GET …/duos/lp` (`@UseGuards(OwnerGuard)`, listed in the conventions lint's guarded-route table) returns each duo's `DuoLpOverlay` — the owner's ranked LP delta in the games with that duo vs the rest of the same window, plus the per-match points — computed by `computeDuoLpOverlays()` in `@vyoh/shared`; `ProfileDuos` reads it through the owner-gated `useDuoLp` and adds a "+142 LP together over 30 ranked · −18 LP in the 40 without" line to each collapsed row, and `ProfileLpHistory` gains an owner-only duo picker (same tab chrome as the queue/range tabs, a toggle) that rings the snapshot following each of that duo's games on the active ladder.
- **Phase E** — shipped. **Timeline + map integration** shipped 2026-05-11 (`db80d95`): full-screen modal triggered from the event timelines section, static SVG Rift minimap with kill/objective dots, scrollable event feed two-way-synced to the map, gold-lead chart as scrubber (`GoldLeadBrush` + `BrushTraveller`), filter chips per event type (kills/towers/dragons/heralds/barons/inhibitors/voidgrubs + your-kills-only / your-deaths-only), spring-staggered dot reveals gated on `useReducedMotion`. The paired Phase-D.1 follow-on — **Rift-position death heatmap** on Champion detail — shipped 2026-05-12 (`a1fb10c`) using a new `timeline-summary-mapper` projection that adds death x/y to `MatchSummary` (Prisma migration + `backfill-position-metrics.ts`). **Full rune page panel** shipped 2026-09-08, owner-only: the stored owner participant keeps the full Riot perk tree but the other nine keep only the keystone and history cannot be refetched, so the panel is a Your Game tab section rather than a scoreboard column. Chunk 1 shipped 2026-09-07: `ParticipantOwnerExtras.runes` (primary + secondary `RuneTree`, three stat shards) projected by `match-mapper.ts`, verified against 40 live cache rows. Chunk 2 shipped 2026-09-07: `syncPerks` also reads CommunityDragon `perks.json` (DDragon `runesReforged.json` omits the shards), so the ten stat shards are `LolPerk` rows and every perk carries a sanitized `descriptionHtml`; live run produced 72 rows, all described, and the image proxy already served shard icons. Chunk 3 shipped 2026-09-08: `match-rune-page.tsx` renders both trees (named through their perks' `path`, keystone enlarged) and the three shard slots as a Your Game tab section between Build and Casts, reusing `KeystoneIcon` for icon + tooltip; `PerkInfo` gained `path` for the tree label. **Composite "Score-of-game" grade** shipped 2026-09-08 beside the badges: `scoreOfGame()` (`packages/shared/src/lol/score-of-game.ts`) gives every participant the mean of six in-game percentiles (damage, KDA, vision, KP, CS, fewest deaths, ties at half credit) mapped to S+/S/A/B/C/D with a 1-based standing; `GradeChip` in `participant-row.tsx` renders it before the badge with a tooltip naming the standing and the formula, muted below A. Remake-length games (duration under `REMAKE_DURATION_S`) get no grade. **Phase E is complete.**
...
- **2026-06-16** — Squad detection API shipped (chunk 1 of 2 for the 3+-stack arc). `getSquads` extends `getDuos` from single-teammate buckets to recurring teammate *sets*: per match it enumerates every size-≥2 subset of the owner's teammates (≤11/match — a team has ≤4 teammates), buckets by sorted-puuid key, applies the same temporal-clustering gate as duos, then **drops a subgroup whose game count is within 1 of a larger qualifying superset** (the subgroup almost never played without the bigger stack → redundant; a subgroup that recurs notably more often is a distinct stack and survives). Output capped at top 5 by games. Members carry latest gameName/tagLine + most-played champion; `size` = members + 1 (the owner). Refactor in the same change: the duo temporal gate (`hasSameSessionPair`, `qualifiesAsRecurring`), participant extraction (`ownerTeammates`), and the cache-loading prelude (`loadOwnerMatchCache`) are now shared by both `getDuos` and `getSquads` (the existing 5 duo tests pin the refactor). Chunk 2 = Profile `ProfileSquads` web surface (separate section beside Duos, renders only when a 3+ stack exists). Per-duo champion pairs and LP-overlay remain blocked (the latter on owner-auth).
- **2026-06-16** — Squad detection chunk 2 (web) shipped. `ProfileSquads` + `useSquads` mirror the duo pair; mounted between `ProfileDuos` and `ProfileSynergy` in [$accountSlug/index.tsx](../../../apps/web/src/routes/lol/$accountSlug/index.tsx). Deliberately **not** wrapped in `CvSection`: squads are absent for most windows, so a reserved `minHeight` would leave a permanent empty gap — the component returns null (no empty state) when no 3+ stack exists and is light enough (≤3 rows) to mount eagerly. Member avatars use overlapping `-space-x-2` champion squares; stack label is Trio / 4-stack / 5-stack off `Squad.size`. The 3+-stack arc is now end-to-end; remaining duo-arc items (per-duo champion pairs, LP-overlay) stay deferred/blocked.
- **2026-06-16** — Per-duo champion pairs shipped (2 chunks). **Chunk 1 (api+shared):** `getDuos` accumulates a per-duo `(yourChamp|teammateChamp)` pair map alongside the existing aggregate — the owner's own champ per match is now captured by extending `ownerTeammates` to return `ownerChampion` (previously only `win` + teammates). Ranked by games, capped at `DUO_PAIR_TOP_N = 6`, exposed on `Duo.championPairs` reusing the shipped `ChampionPair` type (the global `getChampionPairs`/Synergy endpoint can't be sliced per-duo — it drops the teammate puuid). **Chunk 2 (web):** `ProfileDuos` converted from a flat list to an expandable Accordion mirroring `ProfileSynergy` — collapsed row keeps the duo summary (now with a combo-count hint + the `useChampionName`-corrected most-played label, fixing a latent raw-alias render), expansion lists each combo as overlapped owner+partner champion squares + a win-rate bar. The `wrAccent` helper was extracted from `profile-synergy.tsx` to `lol/_shared/wr-accent.ts` and is now shared by both. Distinct from Synergy by design: Synergy is champion-keyed across *all* teammates (random matchmaking included); this is person-keyed and scoped to one *recurring* duo — "what do we two actually run together." Tests: api pairing assertions (split-by-owner-champ + ranking) + web keyboard-expansion + axe scan. LP-overlay per duo remains blocked on owner-auth.
- **2026-06-17** — D.9 (rune-page diversity) shipped (2 chunks). **Chunk 1 (api+shared):** `getChampionRuneDiversity` mirrors `getChampionBuildFlow`'s champion-scoped, remake-excluded `Match` query, then reads the matched `MatchDetailCache` rows and extracts the owner's keystone from the raw Riot shape (`perks.styles[0].selections[0].perk`) — same source as the match-detail participant rows, so no schema change and no timeline dependency (covers every match with a detail cache, unlike build-flow which needs a timeline). Aggregates keystone→{games,wins}, sorts by games desc; new `ChampionRuneDiversityEntry` shared type. **Chunk 2 (web):** `useChampionRuneDiversity` hook + `ChampionRuneDiversity` `ConclusionCard` tile mounted below `ChampionBuildPath` in a `CvSection`. Gated at `MIN_ENTRIES=5` total games; per-keystone WR only colors/marks at `MIN_WR_GAMES=3` (1–2 game keystones stay neutral, no arrow — small-sample noise suppression mirroring build-path's n=1 WR suppression). Bar color = absolute WR tier via the shared `wrAccent` (the ProfileDuos-pairs visual); ▲/▼/· marker = lift vs the champion's overall WR. Keystone icon/name via `KeystoneIcon`/`usePerks` (retired keystones already filtered there). Tests: api 3 (aggregation/sort, remake-exclusion + missing-keystone skip, whitelist guard) + web 6 (pending/undefined null, empty state, dominant + spread verdicts, low-sample marker suppression, axe scan).
- **2026-06-18** — D.2 (lane-phase differential) shipped (2 chunks). Scope decision: **live opponent diff, no migration** (chosen over an owner-only-vs-your-average reframe and over a precomputed-column migration) — the meaningful stat is "+CS vs your lane opponent," and the opponent's per-frame CS/gold isn't projected onto `Match`, but full timelines are already stored in `MatchTimelineCache` (which build-flow already reads live), so the diff is recoverable per-request without a schema change. Baseline framing = even lane (0) + ahead-rate, **not** a cross-champ average (that would need reading every champ's timelines per request). **Chunk 1 (api+shared):** exported `resolveParticipantId` + `frameAtMinute` from `timeline-summary-mapper.ts` (avoid duplicating the frame-at-minute + puuid→participantId logic); `getChampionLanePhase` filters `Match` by champion (remake-excluded) to rows with a `laneOpponent.puuid`, reads their timelines, and averages owner-minus-opponent CS@10/CS@15/gold@10 with independent per-metric sample counts (a game can reach 10 min without 15). New `ChampionLanePhase`/`LanePhaseMetric` shared types. **Chunk 2 (web):** `useChampionLanePhase` hook + `ChampionLanePhase` `ConclusionCard` tile (3 transparent-recipe stat tiles, signed diff colored by sign — emerald/rose/neutral — + ahead-rate), gated at `MIN_ENTRIES=5` on `sampleSize`, mounted in a `CvSection` below `ChampionRuneDiversity`. Tests: api 3 (diff/ahead-rate averaging w/ per-metric sample, remake+no-opponent drop before timeline query, whitelist guard) + web 5 (pending/undefined null, empty state, out-farm + out-farmed verdicts w/ sign-colored tiles, axe scan).
- **2026-06-18** — D.4 (first-blood/first-tower) shipped. Profile-side (not champion-detail): `getObjectiveFirsts` reads the owner's recent non-remake matches' `MatchDetailCache` (remake-excluded via the same `Match` prelude as rune-diversity), counting personal first blood (`participant.firstBloodKill`) and team first tower (`info.teams[myTeam].objectives.tower.first`), each with the win count in those games. New `ObjectiveFirsts`/`ObjectiveFirstTally` shared types. `ProfileObjectiveFirsts` is a frosted 2-stat strip (mirrors `ProfileStatsBar`'s layout — `HeroLabel`/`HeroNumber`/`CountUp`, `divide-x` bar) showing each rate % + a "{winWhen}% win when you do" sub-line; gated at `MIN_GAMES=10`, CV-gated at `minHeight=100` beside the other counter strips (present for most windows on the owner's accounts, unlike `ProfileSquads`). Tests: api 3 (FB+FT tally w/ win counts, remake exclusion before cache read, whitelist guard) + web 5 (pending null, low-sample null, populated rates + win-when, em-dash when objective never happened, axe scan).
- **2026-06-18** — D.5 (carry vs carried) shipped. Profile-side; surface wasn't named in the deliverable, chosen as Profile (cross-champion playstyle aggregate, consistent with D.4). `getCarryProfile` reads the owner's recent non-remake `MatchDetailCache` rows, ranks the owner within their team by `totalDamageDealtToChampions` (rank = 1 + teammates with strictly more), and buckets wins/losses into top-3 vs bottom-2 (full-roster teams only — `team.length >= 4` skips Arena 2-player subteams). New `CarryProfile`/`CarryProfileSplit` shared types. Neutral framing per the roadmap (no "carried" pejorative): tiers labelled "Top-3 Damage" / "Bottom-2 Damage". `ProfileCarryProfile` renders a plain-language verdict ("you win X% when top-3 in team damage, Y% when not") + a 2-tier WR comparison; gated at `MIN_GAMES=10`, CV-gated at `minHeight=140`, mounted below `ProfileObjectiveFirsts`. Empty tier shows "—" not a fake 0%. Tests: api 3 (rank bucketing, full-roster skip, whitelist guard) + web 5 (pending null, low-sample null, comparison verdict + tier WRs, always-top-3 reframe + em-dash, axe scan).
- **2026-06-18** — D.3 (damage-profile radar) shipped — the first remaining D-item that is **reusable on both Profile and Champion detail** per the deliverable. **Normalization decision:** "normalized by role" interpreted as **share-of-team per metric** (owner / team-total, averaged over games), *not* a ratio-vs-owner-role-median. Rationale: share-of-team is role-fair without any external baseline (a support reads low-damage / high-vision naturally), and — load-bearing — it stays meaningful at *both* scopes, whereas a self-vs-role-median ratio collapses to ~1.0 at profile scope (the subject set = the baseline set). A cross-player role baseline isn't available (we only store the owner's games). `getDamageProfile(region, name, tag, championKey?, count)` is one method serving both routes: Match query filtered to `teamPosition: { not: "" }` (Rift only — vision share is degenerate in ARAM/Arena) + optional champion, remake-excluded, then `MatchDetailCache` for per-teammate totals (damage to champions, vision score, minions+neutral CS); full-roster teams only (`team.length >= 4`, skips Arena subteams). Each metric's mean is over the games that had a non-zero team total for *that* metric (independent per-metric counts). New `DamageProfile` shared type. **Web:** `useDamageProfile` (championKey-optional, hits the champion-scoped or profile route) + reusable `DamageProfileRadar` — a Recharts `RadarChart` (already in-stack from the live-page composition radar; no new dep) scaled `share × 5` so the dashed reference polygon at radius 1 = an even five-way split, palette via `chart-palette` (`CHART_SERIES`/`CHART_GRID`/`CHART_AXIS`), wrapped in `ConclusionCard` (works on both surfaces), verdict headlines the top-share axis, + an absolute-% legend below the radar. Mounted in a `CvSection(340)` on Champion detail (below `ChampionLanePhase`) and Profile (below `ProfileCarryProfile`); `MIN_GAMES=5`. Tests: api 4 + web 4. Remaining unblocked D-items after this: D.7 (ARAM dashboard — large, needs its own chunk plan).
- **2026-06-21** — D.6 (objective participation) shipped. **Metric decision:** the roadmap specced "how often you're alive for drake/baron contests" (timeline alive-during-objective), but the **takedown framing was chosen instead** — owner's `challenges.{dragon,baron,riftHerald}Takedowns` ÷ team's `objectives.{...}.kills`, both from `MatchDetailCache`. Feasible because the lean projection (`match-projection.ts`) only leans *non-owner* participants — the owner participant is stored full, so its `challenges` block survives; team objectives survive via `...infoRest`. Rationale: this is the op.gg "objective participation" metric, mirrors the D.4/D.5 cache-read pattern exactly, needs no timeline walk, and avoids hardcoding the BRW/TIF respawn table (no repo prior art existed for aliveness/respawn, and pinning it would soft-violate [[feedback_no_hardcoded_riot_data]]). No grubs/horde axis — `RiotChallenges` has no horde takedown field and team objectives carry no `horde`. `getObjectiveParticipation` is SR-only (in-loop `teamPosition` gate — ARAM/Arena have no neutral objectives), accrues a type only in games where the team killed it ≥1× (honest denominator), and clamps takedowns to team kills. New `ObjectiveParticipation`/`ObjectiveParticipationTally` shared types + `GET …/objective-participation` route. `ProfileObjectiveParticipation` renders a 3-stat frosted strip (Dragons/Barons/Heralds: participation % + "{takedowns} of {teamKills}" sub-line, em-dash when never contested), `MIN_GAMES=10`, CV-gated at `minHeight=100` beside `ProfileObjectiveFirsts`. Tests: api 4 (per-type tally, clamp + non-SR skip, remake exclusion, whitelist guard) + web 5 (pending null, low-sample null, rates + context, em-dash, axe).
- **2026-06-22** — D.7 (ARAM dashboard) shipped — the last unblocked Phase D deliverable. **Scope decision (owner):** *additive only* — a new ARAM Profile section; the all-queue headline `ProfileStatsBar` keeps blending ARAM (left unchanged), and the serious-queues surfaces (`DEFAULT_SERIOUS_QUEUE_IDS = [420, 440]`) already exclude ARAM by definition, so "pull ARAM out of habits" was largely pre-solved. **Surface decision (owner):** Profile section, not a `/aram` sub-route (promote later if it grows). `getAramProfile` filters Match rows to `queueType === queueTypeName(450)` (ARAM, single source of truth via @vyoh/shared queue-types), remake-excluded, then reads `MatchDetailCache` for the owner-full sustain/tank fields (`challenges.effectiveHealAndShielding`, `totalDamageTaken`, `damageSelfMitigated`, `totalDamageDealtToChampions`) and ranks most-played champions. Returns integer **sums**; the web derives means/rates at the display site. **Data gap:** the spec's "healing taken" (healing *received* from allies) isn't a Match-V5 field — sustain is expressed via heal+shield-delivered + damage-taken/self-mitigated instead. New `AramProfile`/`AramChampionTally` shared types + `GET …/aram-profile` route. `ProfileAramDashboard`: `SectionTitle "ARAM"` + a games/WR/KDA strip + a per-game heal+shield/damage-taken/self-mitigated strip + a top-5 ARAM-champ list (square icon + `useChampionName` + games·WR). `MIN_GAMES=10`, CV-gated `minHeight=280`, mounted below `ProfileQueueDistribution`. Tests: api 4 (sum aggregation + champ ranking, ARAM queue filter, remake exclusion, whitelist guard) + web 5 (pending null, low-sample null, WR/KDA/sustain, champ list, axe). **Phase D is now complete except LP-overlay-per-duo (owner-auth-blocked).** Perf: one more below-fold CV-gated frosted section like its siblings; a host re-probe of `lol-overview` is optional (devcontainer can't measure host Chrome).
- **2026-06-19** — D.3 follow-up: **damage-taken axis dropped, radar now 3 spokes** (Damage / Vision / CS). Owner-reported bug: the Tanked axis read exactly 100% on every account. Root cause — `MatchDetailCache` is a **lean** projection (`match-projection.ts`, "strips heavy non-owner participant fields", landed 2026-05-17): non-owner participants keep `totalDamageDealtToChampions`, `visionScore`, `totalMinionsKilled`/`neutralMinionsKilled` (so Damage/Vision/CS shares were correct) but **not** `totalDamageTaken` (nor any defensive stat — `damageSelfMitigated`/`totalHeal` are owner-only too). So the team damage-taken total = the owner's value alone → share ≡ 1.0. Unrecoverable for historical matches without re-fetching raw Riot JSON (the reprojection already overwrote it). Fix: removed `damageTakenShare` from the `DamageProfile` DTO + `getDamageProfile` + the radar + tests. Gold share rejected as a replacement 4th axis (correlates with damage share — redundant). Lesson captured: [[project_lean_match_detail_cache]] — any future aggregation reading non-owner participant fields from the cache must check `match-projection.ts` first.
- **2026-09-01** — LP-overlay per duo, half 1 (api + shared + duo rows). `GET …/duos/lp` is owner-only (`@UseGuards(OwnerGuard)`, added to the guarded-route table in `conventions.spec.ts`): LP is the owner's ladder, and a with-vs-without split says how the owner's ranked games went, which the public duo list deliberately does not. `getDuoLp` reuses the duo pass (`getDuos` split into `loadOwnerMatchCache` + `buildDuos`) and adds one read of the owner's ranked `Match` rows in the same window for the LP snapshots; the split itself is `computeDuoLpOverlays()` in `@vyoh/shared` (ranked queues only, remakes out via `excludeRemakes`, only games carrying both snapshots count on either side, solo + flex summed since each per-match delta is self-contained). The per-match delta moved out of the web's `use-lp-delta.ts` into shared `matchLpDelta()`/`computeLpDeltaMap()` so api and web read the same formula. `ProfileDuos` adds the split line under each collapsed row through `useDuoLp` — owner-gated by `enabled`, not viewer-scoped: a visitor gets 401, so there is no public projection to key apart. Half 2 marks the selected duo's games on `ProfileLpHistory` and flips Phase D to complete.
- **2026-09-01** — LP-overlay per duo, half 2 (chart marks) — **Phase D complete.** `ProfileLpHistory` reads the same `useDuoLp` overlay (owner-gated) plus the public duo list for names, and offers a `DuoTabs` picker — the queue/range tab chrome, toggle semantics, capped at the three duos `ProfileDuos` shows — listing only duos with a measurable game on the *active* ladder (a flex-only duo drops out of the solo chart rather than ringing nothing; `DuoLpMatchPoint` gained `queueId` for this). Each game maps to the chart point that captured its post-match snapshot via `findSnapshotIndices` (first point whose realT ≥ game start; a day bucket closes on its last snapshot so the mapping holds under aggregation; games from >24h before the first snapshot or with no snapshot after them are dropped rather than pinned to an endpoint), rendered as a hollow `CHART_TREND` ring `ReferenceDot` around the line's own dot, brush-clipped like the tier markers. Rejected: interpolating x from `playedAt` — the ring would float off the line between snapshots, and the snapshot *is* the game's LP outcome.
- **2026-09-07 → 08** — Phase E closed. Full rune page shipped **owner-only** in three chunks (owner `runes` on `ParticipantOwnerExtras`; stat shards + perk descriptions from CommunityDragon `perks.json` since DDragon has neither; `match-rune-page.tsx` in the Your Game tab). Per-participant was ruled out because the lean projection keeps the other nine keystone-only and history cannot be refetched. Score-of-game then shipped as an **addition** rather than the alternative the 2026-05-09 entry framed: the owner looked at the badges, judged them not cluttered, and asked for the grade anyway. Formula is position-neutral percentile averaging over the six badge metrics — deliberately no per-role weighting table to maintain, at the cost of slightly favouring rounded stat lines over specialist ones.
