...
**Status:** Active — Phases A + B + C fully shipped (2026-05-16). **Phase D complete 2026-09-01** (squad detection + per-duo champion pairs 2026-06-16, D.9/D.2/D.4/D.5/D.3 2026-06-17→18, D.6 2026-06-21, D.7 2026-06-22, LP-overlay per duo 2026-09-01 — owner-only `GET …/duos/lp`, the LP split on the Profile duo rows, and a duo picker on the LP-history chart ringing the snapshots after that duo's games). Phase E partial: full rune page (owner-only) in progress, chunks 1 + 2 shipped 2026-09-07; composite S grade gated. Both tracked in [open-work.md](../open-work.md); parked sub-items (build-order component-collapse, hover-highlight, boot collapse, soul drake type) in [parked.md](../parked.md). Full Status section with commit refs is [further down](#status).
...
- **Full rune page panel** on match detail, **owner-only**: all 6 runes + 3 stat shards for the owner in the Your Game tab, not just the keystone. The other nine participants are stored keystone-only and cannot be refetched, so a per-participant version is out. Visual reference: op.gg's runes tab on a match. In progress — see [Status](#status).
