...
| Ranked emblems | ✓ | `Season_2023_-_{Tier}.png` | Confirmed working for all 10 tiers. 2023 was the last redesign — Riot has not shipped new crest art for 2024/2025, so the 2023 set *is* the current canonical emblem. The helper takes an optional `year` argument; the API serves the resolved year via `GET /lol/patches/ranked-emblem-year` (HEAD-probes the wiki backwards from the current year, caches 24h, falls back to 2023). Clients call `useRankedEmblemYear()` and thread the value into `wikiRankedEmblemUrl(tier, year)`. A future redesign is picked up without a code change. |
