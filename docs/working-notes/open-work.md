...
- **Editorial typography (Tier 2)** — Chunks 1–5+7 shipped 2026-05-27. `HeroNumber`/`HeroLabel`/`HeroPair` primitives + tests in `apps/web/src/components/ui/hero-number.tsx`; adopted in profile KPI strip + multikill strip + rank tile, champion-detail per-game K/D/A + DeltaTile. `formatKda`/`formatLpDelta`/`formatPercent` exported from `@vyoh/shared`. MatchHero, bento tiles, and trend ConclusionCard surfaces intentionally not migrated (verdict-sentence + card-chrome identity surfaces). Chunk 6 (section-title sweep) deferred — converging LoL sentence-case `<h3>` to Steam's uppercase-tracked rhythm needs visual sign-off; call-site sweep of new format helpers tracked in [quick-wins.md](cross-cutting/quick-wins.md). → [editorial-typography.md](cross-cutting/editorial-typography.md)
