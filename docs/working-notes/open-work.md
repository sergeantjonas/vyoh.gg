...
- **LP forecast Phase LP3 (personal linear fit)** — LP2 (calibration) + LP2.6 (signal tuning) shipped 2026-05-20. The first per-account calibration on Agurin's 174 Solo games showed two signals stuck-positive and one anti-predictive on warnings; LP2.6 re-threshold-ed tilt/champ against the player's overall WR and widened the slot deadband. LP2.5 (per-signal weighting) parked until a re-run shows variance per signal. LP3 (personal linear fit) still long-tail. → [lp-forecast.md](lol/lp-forecast.md)
