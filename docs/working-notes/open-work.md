...
- **Mount + overlay motion (Tier 1)** — Chunks 1 (bento `.stagger-children` cascade), 4 (Steam library `data-mount-stagger` opt-in, virtualizer-safe), 5 (Radix popper `@starting-style` entry transition for bare Tooltip/HoverCard) all shipped 2026-05-27. Chunks 2 + 3 descoped same day — match-list and champion-table already had Motion-driven entry stagger; CSS-layer migration would have removed flash-spring + settle-hold + sort-reflow behavior. Remaining: 6) sweep Motion wrappers superseded by CSS layer (existing shadcn primitives use tw-animate-css and stay; Motion wrappers around Tooltip/HoverCard would be the candidates if any exist). → [mount-and-overlay-motion.md](cross-cutting/mount-and-overlay-motion.md)
