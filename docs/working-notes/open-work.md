...
- **Steam Phase S4.6 — fully shipped 2026-05-15** — Chunk 1 (rows↔tiles toggle, search, sort, played/never filter) + Chunk 2 sub-series: `SteamGameEnrichment` table + monthly enrichment poller (boot backfill + on-add hook via syncOwnedGames diff), hashed asset URL pass across every Steam image helper, app-type filter (defaults to Games), tag filter popover backed by `SteamTag` global catalog + monthly `GetTagList` poller, plus tile hover polish (anchored `@property --sheen-extent` sheen + 3D tilt). Logo-asset gap resolved by outcome (c) from the risk note — `GetItems` doesn't surface a logo key, so `steamLibraryLogoUrl` stays on unhashed `logo.png`. Next Steam surface is S5 (Achievement surfaces MVP). S4.5 + S4.6 numbered as half-steps to keep the `lol-image-pipeline.md` "after Steam S5" reference stable. → [steam-integration.md](steam-integration.md)
