...
- **Steam Phase S8 closing arc** — S8.1–S8.7 shipped 2026-05-17 (S8.7 added the LoL-vs-Steam evening-split tile; stacked-bar 24h x-axis matching chronotype). Remaining: S8.8 session-length distribution (exit criterion). Substrate decision: `SteamPlaySession` (forward-only) is canonical; achievement-anchor reconstruction parked. → [steam-integration.md § S8](steam-integration.md#phase-s8--temporal--cross-stream)
