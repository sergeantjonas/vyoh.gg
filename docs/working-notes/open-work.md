...
- **Page composition (active)** — Chunk 1 (audit) shipped 2026-05-27, table appended to the note. Top 3 priority sweep targets confirmed: Steam game-detail (asymmetric band chrome), LoL profile (partially-grouped IA), LoL match-detail "Your Game" tab (bare chart sections). Pending Chunk 2 (IA decisions per surface) → Chunk 3 (container-chrome rule) → Chunk 4 (per-surface sweep, one commit each) → Chunk 5 (convention update in repo-conventions.md). → [page-composition.md](cross-cutting/page-composition.md)
