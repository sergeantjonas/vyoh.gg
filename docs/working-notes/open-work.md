...
- **Match review surface — MR1 (next-up)** — game arc viz + phase verdict strip + route plumbing under `/lol/$accountSlug/matches/$matchId/review`. Headline next move after Tier 1 owner-data shipped 2026-05-21. MR2 (baseline deviation), MR3 (moment highlights), MR4 (decision narrative) follow. → [match-review.md](lol/match-review.md)
...
- **Match detail panel — damage stacked bar (MD1)** — all-10 physical/magic/true stacked bar; data already in `MatchDetail.participants`, pure frontend. Promoted 2026-05-21. → [lol-owner-data-features.md § Arc 3](lol/lol-owner-data-features.md#arc-3-match-detail-panel-additions)
