...
- **Steam Phase S4.5 — Chunk C-4 (next)** — game.$appid hero redesign (`library_hero.jpg` banner + `logo.png` overlay), split from C-3 at scoping time. Chunks A/B (IA + Profile heading) shipped 2026-05-15. Chunks C-1/C-2/C-3 shipped 2026-05-15 (identity row + animated tabs + sticky compact header + profile-background backdrop + animated avatar + portaled header band aligned with LoL). S4.5 numbered as half-step to keep `lol-image-pipeline.md` "after Steam S5" reference stable. → [steam-integration.md](steam-integration.md)
...
- Section layout extraction — pull the sticky compact header + animated tabs + page-slide system + portaled header band shared between `routes/lol/$accountSlug.tsx` and `routes/steam.tsx` into an `_shared/section-layout/` primitive. Surfaced during S4.5 C-1; identity-slot shape now informed by C-3's profile-background + animated-avatar work (deferment lifted). Sequenced after S4.5 C-4 so all S4.5 sub-chunks ship under the current two-caller shape before extraction. → [section-layout-extraction.md](section-layout-extraction.md)
