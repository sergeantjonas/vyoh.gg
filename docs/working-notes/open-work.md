...
- **Steam Phase S4 + S5 — in progress (started 2026-05-15)** — S4 (achievement data layer) is the prerequisite for S5 (user-facing achievement surfaces). Chunk plan: S4 chunks 1-5 (schema + 3 pollers + read endpoints), then S5 chunks 6-9 (per-game panel, completion verdict card, recent-unlocks Profile chip, global `/steam/achievements` page filling the IA slot opened in S4.5 C-1). **S4 fully shipped 2026-05-15** across chunks 1-5: schema, schema poller, unlocks poller, rarity poller (weekly Sunday `30 5 * * 0` Brussels), and read endpoints — `GET /steam/game/:appid/achievements` + `GET /steam/achievements/recent?limit=N` returning the new `SteamGameAchievements` + `SteamRecentUnlocks` DTOs. Spoiler masking deferred to the frontend (server returns truth). **S5 chunk 6 shipped 2026-05-15** — per-game `AchievementPanel` wired into `routes/steam/game.$appid.tsx` below the playtime block; `useGameAchievements` hook mirrors the owned-games TanStack pattern; spoiler mask (`hidden && !unlockedAt` → `???`) lives on the frontend per the S4 design; click-to-reveal-name on masked rows (`displayName` is real even for hidden); stronger unlocked/locked styling with an emerald left-accent bar on unlocked rows. **S4 hidden-description fix 2026-05-15** — `GetSchemaForGame` blanks hidden-row descriptions anti-spoiler; switched the schema poller to `IPlayerService/GetGameAchievements` (`language=english`, not `l=`) which exposes `localized_desc` for hidden rows. Companion `refresh-steam-achievement-schemas.ts` script ran live: filled 2268/2268 blanks across 142 games with achievements. **S4 cadence bump 2026-05-15** — owned-games daily→every 15min, unlocks daily→every 15min (offset +5), schema monthly→weekly Sunday; ~14% of Steam's 100k/day budget at the new rates. Next: S5 chunk 7 (completion verdict card). → [steam-integration.md](steam-integration.md)
