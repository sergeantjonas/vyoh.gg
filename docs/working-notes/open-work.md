...
- **Mount + overlay motion (Tier 1)** — Chunk 1 (bento `.stagger-children` cascade) shipped 2026-05-27. Chunks 2 + 3 descoped same day — match-list and champion-table already had Motion-driven entry stagger; CSS-layer migration would have removed flash-spring + settle-hold + sort-reflow behavior. Remaining: 4) Steam library tiles via per-item `data-mount-stagger` opt-in (virtualizer-safe), 5) `@starting-style` + `allow-discrete` overlay CSS for Radix popper, 6) sweep Motion wrappers superseded by CSS layer. → [mount-and-overlay-motion.md](cross-cutting/mount-and-overlay-motion.md)
