...
- **Mount + overlay motion (Tier 1)** — picked up 2026-05-27. Tile/list mount stagger via CSS `animation-delay` cascade + `@starting-style` + `transition-behavior: allow-discrete` for Radix overlays. Chunk plan: 1) `.stagger-children` utility + bento, 2) match-list above-fold, 3) champion-table (verify Motion `layout` interplay), 4) Steam library, 5) overlay entry CSS for Radix popper, 6) sweep Motion wrappers superseded by CSS layer. → [mount-and-overlay-motion.md](cross-cutting/mount-and-overlay-motion.md)
