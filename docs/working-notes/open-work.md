...
- **Page composition (next arc)** — opened 2026-05-27 as the natural follow-on to the editorial-typography arc. Decides per-surface IA (when to group cards under section dividers vs flat list) and container convention (chrome around section bodies vs bare), then sweeps ~10 product surfaces with per-surface decisions. Owner-flagged candidates: Steam game-detail (currently flat-cards) could use grouping dividers; match-detail chart sections (Gold lead, Kill & objective timeline, Lane phase, Build order, etc.) sit bare on page bg while equivalent Steam surfaces wrap in chrome. Five-chunk plan (audit → IA decisions → container rule → per-surface sweep → conventions update). Don't touch chart-section chrome or Steam game-detail IA mid-stream — go through the arc's planning chunks first. → [page-composition.md](cross-cutting/page-composition.md)
