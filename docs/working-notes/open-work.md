...
- **Steam Phase S4 + S5 — in progress (started 2026-05-15)** — S4 (achievement data layer) is the prerequisite for S5 (user-facing achievement surfaces). Chunk plan: S4 chunks 1-5 (schema + 3 pollers + read endpoints), then S5 chunks 6-9 (per-game panel, completion verdict card, recent-unlocks Profile chip, global `/steam/achievements` page filling the IA slot opened in S4.5 C-1). **Chunks 1-4 shipped 2026-05-15** — schema + client methods + raw types, schema poller (monthly), player-unlocks poller (daily), global rarity poller (weekly Sunday `30 5 * * 0` Brussels, per-row upsert since rarity drifts over time). All three pollers gated by `achievementCount > 0` to satisfy the schema FK; on-add hooks in `syncOwnedGames` bootstrap newly-added games in-line so first-render shows real data. Hidden-achievement spoiler handling baked into the schema (`hidden` column). → [steam-integration.md](steam-integration.md)
