...
- **Steam Phase S4.5 — fully shipped 2026-05-15** — Chunks A/B (IA + Profile heading) + C-1/C-2/C-3 (identity row + animated tabs + sticky compact header + profile-background backdrop + animated avatar + portaled header band aligned with LoL) + C-4 (`library_hero.jpg` + `logo.png` hero on game.$appid with blurred header.jpg as placeholder/fallback). Phase complete; next Steam surface is S5 (Achievement surfaces MVP). S4.5 numbered as half-step to keep `lol-image-pipeline.md` "after Steam S5" reference stable. → [steam-integration.md](steam-integration.md)
...
- Section layout extraction — pull the sticky compact header + animated tabs + page-slide system + portaled header band shared between `routes/lol/$accountSlug.tsx` and `routes/steam.tsx` into an `_shared/section-layout/` primitive. Surfaced during S4.5 C-1; identity-slot shape informed by C-3's profile-background + animated-avatar work. Unblocked now that S4.5 has fully shipped — both callers stable. → [section-layout-extraction.md](section-layout-extraction.md)
