...
- **Command palette reorganization (F2/F3/S1/S2/F4/F5)** — F1 shipped 2026-05-27 (Accounts group collapsed to one row per account, `⌘↵ matches` chord, companion `Search matches in <id>` row gated on non-empty input). Remaining: F2 (suppress empty-input Matches CTA), S1 (`useCurrentSectionTabs` generalization, must land before F3), F3 (group reorder), S2 (Steam Games group), F4 (Recents label cleanup), F5 (note + lessons writeback). → [command-palette-reorg.md](cross-cutting/command-palette-reorg.md)
...
- **Accent-color system — Chunks 5 + 6 (deferred)** — core cascade + LoL champion/match wiring shipped 2026-05-26. Chunk 5 (broader sweep: scrollbar, focus ring, sparklines, hover sheen) pending per-surface judgement; natural home is [data-viz-densification.md](cross-cutting/data-viz-densification.md). Chunk 6 (Steam game-detail wiring) duplicates Steam-LoL parity Item 6 — blocked on a Steam dominant-color source. → [accent-color-system.md § Shipped + deferred](cross-cutting/accent-color-system.md)
...
- **Page composition (active)** — Chunks 1 (audit) + 2 (top-3 surface decisions) shipped 2026-05-27. Compositional container rule reframed and ratified: *"chrome belongs at the lowest level that visually groups heterogeneous content; don't nest chrome inside chrome."* Surface #1 (Steam game-detail) and Surface #3 (LoL Your Game) both promoted to reference — no change needed, they're the two valid branches of the rule. Surface #2 (LoL profile) yields one Chunk 4 commit: migrate 3 ad-hoc span headers in `profile-lp-history.tsx:540`, `profile-season-history.tsx:213`, `profile-now-playing.tsx:65` to `SectionTitle`. Pending Chunk 3 (visual capture + rule ratification) → Chunk 4 (the one sweep + secondary candidates: LoL recap, champion-detail, trends, Steam achievements/signature) → Chunk 5 (writeback to repo-conventions.md). → [page-composition.md](cross-cutting/page-composition.md)
