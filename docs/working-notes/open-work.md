...
- **Steam Phase S4 + S5 — in progress (started 2026-05-15)** — S4 (achievement data layer) is the prerequisite for S5 (user-facing achievement surfaces). Chunk plan: S4 chunks 1-5 (schema + 3 pollers + read endpoints), then S5 chunks 6-9 (per-game panel, completion verdict card, recent-unlocks Profile chip, global `/steam/achievements` page filling the IA slot opened in S4.5 C-1). **Chunks 1-2 shipped 2026-05-15** — schema + client methods + raw types, then schema poller (monthly cron `0 5 1 * *` Brussels + boot backfill + on-add hook via `syncOwnedGames` diff, mirrors the enrichment poller pattern; empty-achievement games get meta `count=0` so the cron doesn't thrash). Hidden-achievement spoiler handling baked into the schema (`hidden` column) per the brainstorm rule: locked + hidden → `???`, unlocked + hidden → fully revealed. → [steam-integration.md](steam-integration.md)
