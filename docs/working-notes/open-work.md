...
- **Match-depth Phase D remainders** — squad detection **shipped end-to-end 2026-06-16** (`getSquads`/`GET …/squads` + `ProfileSquads` web section); per-duo champion pairs **shipped 2026-06-16** (`Duo.championPairs` + expandable `ProfileDuos` rows). **D.9 rune-page diversity shipped 2026-06-17** (`GET …/champions/:championKey/rune-diversity` + `ChampionRuneDiversity` tile on Champion detail). **D.2 lane-phase differential shipped 2026-06-18** (`GET …/champions/:championKey/lane-phase` + `ChampionLanePhase` tile — owner-minus-laneOpponent CS@10/CS@15/gold@10 diffs computed live from `MatchTimelineCache`, no migration). **D.4 first-blood/first-tower shipped 2026-06-18** (`GET …/objective-firsts` + `ProfileObjectiveFirsts` strip — personal first-blood rate + team first-tower rate, each with win-when-it-happens, from `MatchDetailCache`). **D.5 carry-vs-carried shipped 2026-06-18** (`GET …/carry-profile` + `ProfileCarryProfile` — WR split by whether the owner ranked top-3 vs bottom-2 in team champion damage, neutral framing, from `MatchDetailCache`). **D.3 damage-profile radar shipped 2026-06-18** (`GET …/damage-profile` + `GET …/champions/:championKey/damage-profile` + reusable `DamageProfileRadar` — owner's mean share of team damage/damage-taken/vision/CS as a 4-spoke radar, share-of-team normalization, mounted on **both** Profile and Champion detail, from `MatchDetailCache`). Remaining: LP-overlay graphs per duo (blocked on owner-auth), D.6 (objective participation), D.7 (ARAM dashboard). (Match-list duo highlight shipped 2026-06-15.) → [match-depth-roadmap.md](lol/match-depth-roadmap.md)
