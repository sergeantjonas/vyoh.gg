...
- **Patch notes PN3 (next-up)** — PN1 + PN2 shipped 2026-05-17. PN2: `GET /lol/patches/current/changes` endpoint + `ProfilePatchNotice` callout on `/lol/$accountSlug/` (top-5 champion derivation via existing CDragon name resolver, per-patch dismissal, glyph-coded change lines). Next: patch-notes tab under the account route with full champion list, your champions highlighted, "my champions only" toggle, prev/next patch selector. → [lol-patch-notes.md](lol-patch-notes.md)
...
- **Home deck — next candidate tile** — S8.4 (weekly gaming-total bento) lands a second cross-stream tile on `/`. Next home tile per [home-deck.md](home-deck.md) + [self-portrait-surfaces.md](self-portrait-surfaces.md). Candidates: cross-stream "now playing" synthesis, first-played-meaningfully chip.
- **Steam Phase S8 remaining surfaces** — S8.1 + S8.2 + S8.3 (per-game unlock timeline) + S8.4 (weekly gaming-total bento) shipped 2026-05-17. Remaining S8 exit-criteria surfaces: LoL-vs-Steam evening split, session-length distribution. → [steam-integration.md § S8](steam-integration.md#phase-s8--temporal--cross-stream)
