...
- **Home deck — next candidate tile** — S8.2 shipped 2026-05-17 (`07197df`); chronotype is now cross-stream (LoL + Steam). Next home tile per [home-deck.md](home-deck.md) + [self-portrait-surfaces.md](self-portrait-surfaces.md). Candidates: cross-stream "now playing" synthesis, weekly gaming-total bento.
- **Steam Phase S8 remaining surfaces** — S8.1 (signature chronotype) + S8.2 (home chronotype) shipped 2026-05-17. Remaining S8 exit-criteria surfaces: per-game timeline, LoL-vs-Steam evening split, weekly gaming-total bento, session-length distribution. → [steam-integration.md § S8](steam-integration.md#phase-s8--temporal--cross-stream)
