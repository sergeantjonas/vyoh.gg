...
- **Steam Phase S8 closing arc** — S8.1–S8.5 shipped 2026-05-17 (S8.5 first-played-meaningfully chip lands the seventh `/` tile). Remaining: S8.6 per-stream toggle on chronotype, S8.7 LoL-vs-Steam evening split (exit criterion), S8.8 session-length distribution (exit criterion). Substrate decision: `SteamPlaySession` (forward-only) is canonical; achievement-anchor reconstruction parked. Bento-density decision needed before S8.7. → [steam-integration.md § S8](steam-integration.md#phase-s8--temporal--cross-stream)
