...
- **The visual review has NOT been run.** The commented Beast fixture sits below `DEV_STEAM_MOMENT_OVERRIDE` in [landing-config.ts](../../../apps/web/src/home/landing-config.ts) ready to paste, and the override is `[]` as required, but nobody has looked at `/` in Chrome or Firefox with the beat rendered. The sparkline at 240×48 inside a receipt band is the untested part, though not a novel one: [rank-trajectory-strip.tsx](../../../apps/web/src/home/conclusion/rank-trajectory-strip.tsx) already ships `Sparkline` at 400×48 with `strokeWidth={1.5}` and `h-12` on the same page, so the risk is layout inside `ChapterDetail` rather than the primitive. Do this before or with chunk 2.
