...
| R3 | The launch-window drift beat | `packages/shared` + `apps/api/src/recap` + `apps/web/src/home/recap` | **Scoped 2026-09-02; chunk 1 (shared types + deriver) shipped the same day, chunks 2 and 3 open**, see [R3 chunk plan](#r3-chunk-plan-2026-09-02). Third `steam-moment` type `LAUNCH_RARITY_DRIFT`: the owner's unlocks on a day-one title, rarity when earned against rarity now, one curve. Three chunks, each independently committable, but chunk 3 must land before or with chunk 2. |
...
Each chunk is one commit, each carries its tests, each leaves `verify:cc` green. Chunk 2 depends on chunk 1; chunk 3 depends on chunk 1 only. They are **not** order-free in practice: chunk 2 alone would put live `LAUNCH_RARITY_DRIFT` descriptors on `/` while `momentCopy` still falls a third type through to the cluster branch, so the page would caption a rarity beat *"Recent run on X / Stretch of achievements stacked up"* off a null cluster. Land chunk 3 before or in the same session as chunk 2.
...
- **Chunk 3 must not ship after chunk 2**, for the cluster-prose fallthrough recorded above the chunk list. If the api detector has to land first for a live check, keep it out of `detectAll` until the web branch exists.
