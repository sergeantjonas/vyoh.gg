...
**Status:** Shipped — R1 shipped 2026-08-07, R2 shipped 2026-08-12 (cohort split the same day, cadence split 2026-08-13). **R3 shipped 2026-09-02 as the launch-window beat**, scoped the same day and decided on the third probe reading: launch cohort 99 of 99 series visible across two owner-played day-one titles (Beast of Reincarnation +36.90pp over 28 days on 12 observations, Mortal Shell II +31.50pp over 12 days on 5); the mature cohort's only rare-band movement is Nioh 3 rising off a literal 0.0 plus DOOM: The Dark Ages at +2.1pp in 27 days — no settled rare-band series reached 2× without starting at zero. The mature-library drift beat is **parked**: re-run the probe when a settled rare-band series moves ≥ 5pp or 2× from a non-zero origin. R3 shipped as a third `steam-moment` type, `LAUNCH_RARITY_DRIFT`, in three chunks (shared deriver → api detector → web beat); **all three chunks shipped 2026-09-02, plus the calibration follow-up the same day**. The beat is live on `/`: Beast of Reincarnation renders as `LAUNCH_RARITY_DRIFT` at score 14.86, having beaten its own achievement cluster at 13.46. R3 is closed: the beat is live and the visual review passed in Firefox. R2b shipped 2026-09-06, so the only thing left on the arc is the parked mature-library beat. The plan below is written to be implemented from this note alone. Known limit stands: copy says "when you earned it" only for unlocks bracketed by a sample within 3 days, never for anything older.

## Today's behaviour

Global rarity is a **current value, refreshed weekly, with an append-only observation log behind it**.

- [global-rarity.poller.ts](../../../apps/api/src/steam/achievements/global-rarity.poller.ts) selects every game whose `lastRarityCheckedAt` is older than 7 days, daily at 05:30 Europe/Brussels, 40 games per pass, boot-reconciled. So a displayed percentage is at most ~7 days stale.
- [global-rarity.service.ts](../../../apps/api/src/steam/achievements/global-rarity.service.ts) upserts `SteamAchievementGlobalRarity` in place. That row stays the single current value and stays the only thing the read path touches.
...
| R3 | The launch-window drift beat | `packages/shared` + `apps/api/src/recap` + `apps/web/src/home/recap` | **Shipped 2026-09-02** across all three chunks, plus the R3b calibration follow-up; live on `/`, see [R3 chunk plan](#r3-chunk-plan-2026-09-02). Third `steam-moment` type `LAUNCH_RARITY_DRIFT`: the owner's unlocks on a day-one title, rarity when earned against rarity now, one curve. Three chunks, each independently committable; chunk 3 landed ahead of chunk 2 so the beat exists before anything emits a descriptor. |
| R2b | Probe verdict hygiene | `apps/api/src/scripts` | **Shipped 2026-09-06** — `hasSettledOrigin()` gates the mature cohort's visible count (launch keeps both arms, since its series legitimately start at zero) and every cohort line prints its rare-band count beside the all-band one. Original scope: exclude from-the-floor series (`firstPct === 0`) from the mature verdict and print the rare-band count beside the all-band count, so `Gate CLEARED on settled titles` cannot fire off Isaac creep plus a content drop (see the third reading). |
...
**Follow-up for R2, not R3 — shipped 2026-09-06 as R2b.** The pooled mature verdict should exclude from-the-floor series (`firstPct === 0`) and print the rare-band count separately from the all-band count, so the `Gate CLEARED` line cannot fire off Isaac creep plus a content drop. One flag or a second line; small, and it belongs to the probe.
...
> **Corvus's End** was **0.7%** when you earned it. It's **28%** today.
> ⟋ (one curve: that achievement's global percentage across every observation we hold)
> Bestie 1.4% → 34% · Taurus's End 1.5% → 35% · Closest Companion 3.2% → 38% · …
...
Each chunk is one commit, each carries its tests, each leaves `verify:cc` green. Chunk 2 depends on chunk 1; chunk 3 depends on chunk 1 only. They were **not** order-free in practice — chunk 2 alone would have put live `LAUNCH_RARITY_DRIFT` descriptors on `/` while `momentCopy` still fell a third type through to the cluster branch, captioning a rarity beat *"Recent run on X / Stretch of achievements stacked up"* off a null cluster. Chunk 3 landed first, so that hazard is closed and chunk 2 is now free to land on its own.
...
**Shipped 2026-09-02.** [launch-drift.ts](../../../packages/shared/src/steam/launch-drift.ts) carries the constants, the row types and the three functions; `SteamLaunchDriftUnlock` / `SteamLaunchDriftStats` and the `launchDrift` descriptor field sit in `recap-chapter.ts`; the candidate variant and `toDescriptor` pass-through are wired; sixteen tests in `launch-drift.test.ts` plus the `recap-scoring.test.ts` pass-through case. Four notes for chunks 2 and 3:

- **`momentAccentClass` is exhaustive with no `default`** and a `string` return type, so the third literal did break `apps/web`'s typecheck. The escape hatch applied: `case "LAUNCH_RARITY_DRIFT": return "text-indigo-300";` landed in this chunk with the palette comment chunk 3 specified. Chunk 3 will find [moment-accent.ts](../../../apps/web/src/home/recap/moment-accent.ts) already done and should drop it from its file list. `momentCopy` and `entranceForType` both have fallthroughs and needed nothing.
- **The `LAUNCH_DRIFT_MIN_DELTA_PP` test is quantised to one decimal** before comparing, and so are the absolute tiebreak and `launchDriftBaseSignal`. This is the same precedent [global-rarity.service.ts](../../../apps/api/src/steam/achievements/global-rarity.service.ts) already sets for its own "did it move" test: Steam publishes one decimal but serialises through a float32, so a stored value is `47.900001525878906`, not `47.9`. Subtracting the raw pair drops roughly one move in seventy just under the threshold — `4.1 − 3.1 = 0.9999999999999996`, while `3.3 − 2.3` is exactly 1, so the failure is sparse and would never have shown up in a hand-picked fixture. `launch-drift.test.ts`'s +1.0pp case uses `3.1 → 4.1` specifically so deleting the `Math.round` fails a test.
- **`observedFrom` / `observedTo` come off the distinct-timestamp set, not the ends of the list.** The ascending contract on `observations` still stands and chunk 2's query still supplies it — it just no longer decides the span, so an ordering slip on the api side cannot produce an inverted one.
- **The fixture reproduces the illustrated beat copy only because Munitions Master is a day-13 unlock** (bracketing sample 3.4%). At its day-1 value of 0.1% it would rank first on relative gain — ratio 57 against Corvus's End's 40.6 — and headline the beat instead. Decision 4 stands as written; the point for chunk 3's visual review is that the headline follows unlock *timing*, so the real Beast descriptor may well not open on Corvus's End.
...
1. Import `LAUNCH_WINDOW_MS` from `../steam/achievements/global-rarity.poller` (the same constant the poller uses to pick daily-sampled titles, so "captured while in the window" means the same thing on both sides).
...
**Shipped 2026-09-02.** `detectLaunchRarityDrift` is in [steam-moments.service.ts](../../../apps/api/src/recap/steam-moments.service.ts) and in `detectAll`'s `Promise.all`; `allSteamMomentAppids` already took every `steam-moment` candidate regardless of `momentType`, so [recap-subjects.service.ts](../../../apps/api/src/recap/recap-subjects.service.ts) needed only its two comments extended. Six new spec cases.

**The live check did print nothing, and the reason is calibration, not the detector.** The probe's launch cohort was full (99 of 99 series visible, 2 games, max +36.90pp), so the detector was called directly against the live database with `NO_CURATION` and returns both games:

| game | headline | baseSignal | daysSince | score |
|---|---|---|---|---|
| Mortal Shell II | `Praise him!` 2.6% → 23.8% | 10.6 | 10 | 6.46 |
| Beast of Reincarnation | `Closest Companion` 3.2% → 38.4% | 15 | 20 | 5.57 |

Both clear the floor of 5. Neither reaches `/`, because the same two games *also* produce achievement clusters, and those score 24.4 (Mortal Shell II), 18.1 (Mortal Shell) and 13.5 (Beast) — filling `steamMomentCap: 3` outright. Two findings follow, and both are calibration questions this note closed on paper and the data has now reopened. **Neither was changed in this chunk**, since the constants are spec-pinned:

1. **Decision 5's comparison is arithmetically wrong.** It says the beat "is meant to outrank a cluster on a fresh launch and lose to it three weeks later". It cannot: `CLUSTER_MIN_UNLOCKS × CLUSTER_SIGNAL_FACTOR` is 5 × 4 = **20**, so the *smallest qualifying* cluster outranks the drift beat's *capped maximum* of 15 at equal age. The beat can only ever win by being fresher, and a launch title reliably produces a cluster at the same time. **Resolved in R3b below.**
2. **A same-appid cluster and drift beat are not deduped.** Decision 7 settled the drift-vs-`steam-subject` question ("complementary, no dedupe") but says nothing about drift-vs-cluster, and live data produces both for Beast of Reincarnation and both for Mortal Shell II. Today the cap masks it; raise the cap or age the clusters out and one game renders two moment beats with two different framings. Note there is no machinery to copy for this: `recap-subjects.service.ts` dedupes moment ↔ *subject* across kinds, and nothing in `selectChapters` dedupes two moments of the same kind by appid. This wants a decision before the beat can actually appear.

**Confirmed prediction:** chunk 1's note warned the real descriptor might not open on Corvus's End, because the headline follows unlock timing rather than the note's illustration. It doesn't — the live headline is `Closest Companion` at 3.2% → 38.4%.

### R3b — shared scale and same-appid dedup (2026-09-02)

Both findings above, closed together, because they are one problem: the beat and the cluster were being compared by score on scales that did not agree.

**`LAUNCH_DRIFT_SIGNAL_FACTOR` 0.5 → 4/3.** A capped beat now scores `30 × 4/3 = 40` — exactly a capped cluster's `CLUSTER_UNLOCK_CAP × CLUSTER_SIGNAL_FACTOR`. The first estimate at 0.8 was calibrated against the *smallest* qualifying cluster (20) and was wrong: both clusters actually competing are capped ten-unlock clusters at 40, so 0.8 would have left the beat losing by arithmetic and shipped a feature that never appeared. `30 × (4/3)` is exactly 40 in doubles, so the cap assertion stays exact. Decay is unchanged, so a capped beat now crosses the floor of 5 at about six weeks rather than three.

**Same-appid moment dedup**, in `recap-subjects.service.ts` ahead of the existing moment ↔ subject pass, which therefore only ever sees one moment per appid. Highest `recapScore` keeps the appid; ties go to the earlier detector in `detectAll`. This is *not* the moment ↔ subject rule reused — that one compares across kinds and has no bearing here — and score-based comparison is only defensible because the ceilings were made to match first. If either ceiling moves, this dedup silently becomes a bias again; the factor's comment in `launch-drift.ts` says so.

**`FIRST_TIME_GAME` is exempt from that dedup**, and the first cut wrongly included it. Its `baseSignal` is play-minutes / 15 — uncapped, on no shared ceiling, and systematically under-observed because the poller only ever sees part of the play — so scoring it against a cluster drops the first-time story on precisely the game where it is the story. Worse and more quietly: removing the candidate there also strands the moment ↔ subject suppression, which needs the first-time candidate to still exist to know it should drop the "Playing lately" subject. The existing suppression specs did not catch it because none of them puts a cluster and a first-time on one appid; there is now a spec that does, and it fails if the exemption is removed.

**Live confirmation.** Predicted before the change and matched exactly after it:

| game | cluster | drift | survives |
|---|---|---|---|
| Mortal Shell II | **24.38** | 17.23 | cluster |
| Mortal Shell | **18.11** | — | cluster |
| Beast of Reincarnation | 13.46 | **14.86** | drift |
...
Chrome was not separately eyeballed and does not need to be: the fix is a second `<polyline>` on the same path, and SVG stroke painting is not a surface where the engines diverge — unlike the `backdrop-filter` and scroll-timeline behaviour the arc notes elsewhere gate on engine. Firefox is also the stricter engine for this app's text-over-splash treatments, so a Firefox-verified curve is the stronger result of the two.
...
**Shipped 2026-09-02.** The copy branch, `launchDriftBody`, the entrance case, the receipt block and the `launchDrift` prop are in [steam-moment-beat.tsx](../../../apps/web/src/home/recap/steam-moment-beat.tsx); the aggregator passes it through; twelve tests including the axe scan, plus one aggregator beat-count case. Six files, as planned, but a different six — `moment-accent.ts` came off the list and `steam-moments-aggregator.test.tsx` went on it. Five notes:

- **The prose renders `28%`, not `28.4%`.** `formatRarityPercentEditorial` keeps a decimal below 10% and rounds above it, by its own documented design ("12% reads cleaner than 12.4%"). Every illustration in this note above 10% was written against unrounded values and has been corrected to what the beat actually prints. Chunk 2's live check should expect `0.7% → 28%`.
- **`moment-accent.ts` was already done** in chunk 1, for the exhaustive-switch reason recorded there.
- **The readings line says `1 day`, not `1 days`.** The spec's string was `{days} days`; `driftSpanDays` floors at 1, so the singular branch exists to keep a same-day pair readable. Same shape as the `1 session` branch in the first-time receipt.
- **`observationCount` can exceed the vertices the curve draws.** It counts the game's distinct observation timestamps, while `curve` is the headline achievement's own series and R1 appends a history row only when *that* achievement moved. So "12 readings" beside an 8-point line is correct, not a defect. Spec-pinned; recording it rather than changing it.
- **The visual review was outstanding at the time of this chunk**, and ran after R3b on real data instead of the dev override — see the R3b section for what it found. The commented Beast fixture stays below `DEV_STEAM_MOMENT_OVERRIDE` in [landing-config.ts](../../../apps/web/src/home/landing-config.ts) for reviewing the beat without a qualifying game.
