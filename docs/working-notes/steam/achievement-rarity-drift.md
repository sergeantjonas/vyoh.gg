...
1. **Decision 5's comparison is arithmetically wrong.** It says the beat "is meant to outrank a cluster on a fresh launch and lose to it three weeks later". It cannot: `CLUSTER_MIN_UNLOCKS × CLUSTER_SIGNAL_FACTOR` is 5 × 4 = **20**, so the *smallest qualifying* cluster outranks the drift beat's *capped maximum* of 15 at equal age. The beat can only ever win by being fresher, and a launch title reliably produces a cluster at the same time. Either `LAUNCH_DRIFT_SIGNAL_FACTOR` needs to rise (0.5 → ~0.8 puts a +30pp beat at 24) or the beat needs to stop competing with clusters for the same three slots.
