...
- **4b — Timeline with brush.** Weeks as rows, hours as columns, sessions as bars coloured by game, unlock dots inside them, a visx brush to scrub the window. `@visx/brush` is installed and unused anywhere else, so this is its first consumer; probe in Chromium, Firefox and WebKit before trusting it. The visx timeline with brush, the hour heatmap. Engine-gate anything the brush does that Firefox or WebKit disagree with; probe in a real browser, happy-dom does not lay out.
