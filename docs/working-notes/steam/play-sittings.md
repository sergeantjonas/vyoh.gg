...
**Copy pass — open.** Read every card's rendered copy off the live page at once, as portrait chunk 5 did. Known so far: the strip's hover tooltip and the hero say the same sentence, which is right; the ledger's verdict names days while its footer counts rows, which is deliberate but worth a glance; and `unusual-slot` copy has not been seen live since the 40-cell gate landed.
