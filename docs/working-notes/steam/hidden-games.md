...
**Why the quarantine has to catch the *big* new purchases (owner, 2026-08-25).** *"I don't want everyone to see I had a binge session of an adult game, so by default it should hide it until I allow it to show."* So the high-engagement new game is the **primary** case, not collateral damage: a 40-minute curiosity needs hiding far less than a 10-hour weekend does. Chapter selection reads the public curation and `/` is not viewer-aware, so a quarantined game loses both chapters it would have earned — the "Playing lately" subject and the FIRST_TIME_GAME moment — until the owner rules on it. That silence is the feature.

Two things must therefore never be built: **auto-publishing past an engagement threshold** (it would fire hardest on exactly the games meant to stay back), and **moving the review prompt to `/`** to catch owners who don't visit the Steam section (the current failure direction is *stays private*, which is the safe one). Both are recorded with the reasoning in [dormant-chapter-ranking.md](../cross-cutting/dormant-chapter-ranking.md) § D6.
...
One button, in the place the owner noticed the game: `HideGameButton` on the game-detail chip row, on the library **hovercard** — which both the row and the grid tile already share — and on the wishlist row.

**The wishlist row is the case that needed the `name` prop.** A wishlisted appid usually has no `SteamOwnedGame` row, so the api's `dto.name ?? ownedGameName(appid)` fallback resolves to null and `/status` would list the ruling as a bare "App 1091500" forever. The button forwards the title when the surface knows it, and omits it when it doesn't — a caller's own "Unknown title (app N)" placeholder must not be persisted as data. `name` only affects the `create` branch of the upsert, so sending it on an already-curated row is inert.

It also needed a different escape from the nested-interactive problem. The wishlist row is an external `<a href>` to the Steam store with no hovercard to hide a button in, so the `<li>` became a flex container and the button is a **sibling** of the anchor rather than a descendant.
