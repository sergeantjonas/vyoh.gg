...
**Status:** Active — chunks 0, 1, 2a and 2b complete (2026-08-02). Snapshots are viable; `SteamPlaySession` is not, so cards 4 and 8 and the single-session cohort are deferred to post-hosting. Cards 1, 2 and 6 are live on `/steam` under a **Portrait** header. Chunk 2c (cards 3 and 5) is next, and it owns the chip-overlap cleanup noted below.
...
- **2c — Web.** Cards 3 and 5 folded into the same section, reading the existing `achievements/library-completion` and `platform-mix` endpoints. **Also resolves an overlap 2b left standing:** `LibraryCompositionChip` in the grid below reports owned / ever-launched / untouched, and card 6 reports owned / meaningful / ghost off the same snapshot — the untouched and ghost counts are the same number said twice. Card 5 is already specified as a reframe of `PlatformMixChip`, so 2c is the change where both chips move into the Portrait rather than sitting beside it.
