...
**Status:** Active — **chunk 3 complete** (2026-08-02); chunks 0, 1, 2 and 3 have all shipped and each was verified against the running api rather than the suite alone. Cards 1, 2, 3, 5 and 6 are live on `/steam` under a **Portrait** header and cards 7, 9, 10, 11, 12 and 13 under an **Anti-Portrait** one, every card server-rendering its real claim. **Cards 4 and 8 are the only catalog entries not built**, along with the single-session cohort: `SteamPlaySession` is poller-bound and holds 28 rows, so all three wait for hosting. **The layout pass (chunk L) is complete** (2026-08-02, L1–L5): each half now opens with an editorial hero band — `Souls-like` for the Portrait, `120 never opened` for the Anti-Portrait — over a column-flowed band of compact chips, and cards 1 and 13 are deleted into the two heroes. It came in under the route's paint budget (26 layers / 67 ms against ≤ 35 / ≤ 150 ms) and left the page ~320 px taller than the chips alone. **Chunk 4 (backlog recommendations) is next.**
