...
**Status:** Active — **chunk 3 complete** (2026-08-02); chunks 0, 1, 2 and 3 have all shipped and each was verified against the running api rather than the suite alone. Cards 1, 2, 3, 5 and 6 are live on `/steam` under a **Portrait** header and cards 7, 9, 10, 11, 12 and 13 under an **Anti-Portrait** one, every card server-rendering its real claim. **Cards 4 and 8 are the only catalog entries not built**, along with the single-session cohort: `SteamPlaySession` is poller-bound and holds 28 rows, so all three wait for hosting. **A layout pass (chunk L) runs before chunk 4** — eleven identically-shaped cards make the page long and flat, so each half gets an editorial hero band with denser chips beneath it; L1 has shipped.
...
- **L2 — A compact density for the supporting cards.** A `density` on `CardShell`, three columns at `xl` and two at `md`, so the nine non-hero cards stop each claiming a full-weight tile.
- **L3 — Portrait hero band.** The genre anchor becomes the section's opening statement rather than the first of five equals. Needs [chapter-shadows.ts](../../../apps/web/src/home/recap/chapter-shadows.ts) lifted into `_shared` first — text over the Steam backdrop needs the same hard-inner + halo + glow tiers the recap chapters use, and a second copy would drift.
- **L4 — Anti-Portrait hero band.** Card 13 carries the same treatment for its half.
- **L5 — Re-probe and record.** `/steam` has no row in the paint-budget table, so this establishes a number rather than defends one.
