...
**Status:** Active — **chunk 2 complete** (2026-08-02); chunks 0, 1, 2a, 2b and 2c all shipped. Cards 1, 2, 3, 5 and 6 are live on `/steam` under a **Portrait** header. Card 4 is the one Portrait-half card not built: `SteamPlaySession` is poller-bound and holds 28 rows, so it and card 8 and the single-session cohort wait for hosting. **Chunk 3 (the Anti-Portrait half — cards 7, 9–13) is next.** One decision left open for the owner: whether `LibraryCompositionChip` retires now that card 6 supersedes it.
...
**Still open after 2c:** `LibraryCompositionChip` remains in the grid below reporting owned / ever-launched / untouched, and card 6 reports owned / meaningful / ghost off the same snapshot — the untouched and ghost counts are the same number said twice. Card 5's move was specced ("reframe of the existing platform-mix chip *inside the Portrait*"); retiring the library chip was not, so it is left for the owner to call.
