...
**Status:** Active — **chunks 3a and 3b shipped** (2026-08-02); chunks 0, 1 and 2 all landed before them, each verified against the running api. Cards 1, 2, 3, 5 and 6 are live on `/steam` under a **Portrait** header and cards 7, 9 and 11 under an **Anti-Portrait** one, every card server-rendering its real claim. Card 4 is the one Portrait-half card not built: `SteamPlaySession` is poller-bound and holds 28 rows, so it and card 8 and the single-session cohort wait for hosting. **Chunk 3c (cards 10, 12 and 13) is next and closes the Anti-Portrait half.**
