...
| 1 | **`GET /steam/upcoming`.** `SteamUpcomingItem` (+ `source: "wishlist" \| "owned"`) in `@vyoh/shared`; service merges wishlist with owned-unreleased (`removedAt: null`, `comingSoon: true`), deduped by appid preferring `owned`. Explicit controller return type per convention. | `packages/shared/src/steam/`, `apps/api/src/steam/steam.{service,controller}.ts`, tests |
| 2 | **Upcoming panel on the merged shape.** Bucketing / calendar / bands / TBA pool consume `SteamUpcomingItem`; provenance mark on tiles, "Pre-ordered" framing in the imminent hero. | `apps/web/src/steam/wishlist/upcoming/`, tests |
