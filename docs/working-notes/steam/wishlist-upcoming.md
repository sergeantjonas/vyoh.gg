...
**Status:** Shipped — original arc (chunks 0–6) complete. **The follow-on arc (chunks 0–5) is complete as of 2026-08-12** — the Upcoming view was wishlist-sourced, so pre-ordering a game deleted it from the calendar; it now covers owned-but-unreleased titles too and lives on its own `/steam/upcoming` route. **The reported defect is closed** (pre-orders reach the calendar, the imminent hero and the profile chips), **the route split has landed** (`/steam/wishlist` is the list alone, its tabs retired, `?tab=upcoming` redirected), **the profile chip became a pair** — `Wishlist` for the backlog, `On the radar` for what lands next — and **the hero-meta guard now gates on the upcoming set**, which also stopped a pre-ordered hero from 404ing its own accent, platforms and blurb. See § Follow-on arc for the data precondition, the naming decision, and the per-chunk decisions. The rest of this note describes the original arc as shipped — note that § Route shape's tab model is superseded by the split.
...
| 4 | ~~**Guard + hero-meta.**~~ **Done 2026-08-12.** `SteamUpcomingService.membershipOf()` replaces `isWishlisted()` as the hero-meta gate — the library arm first (coming-soon enrichment row narrowed to a non-removed library row), the wishlist arm second, returning the provenance rather than a boolean. Still a closed owner-controlled set. A pre-order was 404ing its own metadata, so the imminent hero rendered with no accent, platform pills, ESRB badge or blurb; it now reads them off the enrichment row it already has, skipping the store call, both image fetches and the Vibrant pass. | `apps/api/src/steam/{steam,upcoming,wishlist-hero}.service.ts`, `packages/shared/src/steam/wishlist.ts`, tests |
| 5 | ~~**Profile chip pair.**~~ **Done 2026-08-12.** The Wishlist chip was making a claim its own count contradicted — titled `Wishlist`, counting 33 wishlisted games, and headlining a pre-order that is by definition not one of them. Split into two cards on the same 2-column grid (which had an empty cell): `Wishlist` back to backlog framing, and a new **`On the radar`** ([`upcoming-chip.tsx`](../../../apps/web/src/steam/upcoming-chip.tsx)) carrying the release fact, the tracked count and the key art, both links to `/steam/upcoming`. The fact module moved with it as [`upcoming/upcoming-fact.ts`](../../../apps/web/src/steam/upcoming/upcoming-fact.ts) (`pickUpcomingFact` + `formatUpcomingFact`, the phrasing pulled out of `wishlist/format.ts` so one module owns the fact and its sentence). | `apps/web/src/steam/{upcoming-chip,wishlist-chip}.tsx`, `steam/upcoming/upcoming-fact.ts`, `routes/steam/index.tsx`, tests |
...
- **The guard was refusing the exact appid the surface most wanted to ask about.** Scoped as plumbing — widen a membership check to match a widened set — and it was, but the widening also closed a live symptom nobody had looked for: `/steam/wishlist/:appid/hero-meta` gated on wishlist membership, and a pre-order is gone from the wishlist by the time it becomes the nearest release, so the imminent hero for Mortal Shell II 404'd its own metadata. The art still resolved (the proxy takes an appid and falls back to the `0` asset-timestamp sentinel), which is why it read as a styling gap rather than a failed request: no accent, no platform pills, no ESRB badge, no blurb, on the one hero the arc exists to show.
- **Membership returns the provenance, not a boolean.** `membershipOf()` answers `"owned" | "wishlist" | null`, because the two arms have different costs downstream. Proving a title owned means reading its enrichment row, and that row already holds every field the endpoint returns — so the answer to the guard is also the answer to the request. A boolean would have thrown that away and sent an owned title down the projection path: a live `GetItems` call, up to two image fetches, and a Vibrant pass over art the anchor pass already analysed.
- **The library arm goes first, and not only because it wins collisions.** It answers out of our own DB, so a pre-ordered hero still resolves while Steam is down — the arm that can answer unaided should be the one that runs unconditionally. It matches the merge's own precedence (owned wins), so the guard and the payload describe a title the same way.
- **A null stored accent is not re-computed.** `dominantHex` on the row is written by the anchor pass over the same hero art the Vibrant pass here would re-read; null means that pass has not reached the row, not that the answer is unavailable. Falling back to a live extraction would reintroduce the whole cost this path exists to avoid, on the request least able to afford it. The hero uses its neutral token, exactly as it does when the art is unreachable.
- **No memory cache on the row path.** The TTL map exists to spare an upstream round-trip and a CPU pass; a row read has neither cost, and a copy here would go stale against the daily refresh that keeps an announced date current — the one field on this surface that moves right up to launch.
- **Verified live, and the two paths are two orders of magnitude apart.** Mortal Shell II (`2584270`) went from 404 to 200 in **11 ms** with its accent, blurb and platform flags; a wishlisted title on the same list (`3949550`) answers in **990 ms** cold, which is the store call plus the art fetch plus Vibrant. The refusals still precede the spend — an owned-but-released appid 404s in 2 ms, a nonsense one in 1.6 ms.

### Chunk 5 decisions

- **Two cards, because the chip had the route's own bug one level down.** Chunk 2 changed where the chip read from and chunk 3 re-pointed its link, but nobody touched its identity: it still said `WISHLIST`, still counted the wishlist, and by then led with a game the wishlist provably could not hold. Retitling the single card was the alternative considered — the title following whichever claim leads — and rejected: a card that renames itself on data is a strange thing to explain, and the profile grid is `md:grid-cols-2` with three chips, so the fourth cost nothing but the empty cell.
- **"On the radar", not "Upcoming" or a countdown framing.** The route is `Upcoming` and the tab strip says so, but a chip is not navigation and can afford a voice. The deciding argument was the weakest tier: the card's fallback fact is a title with no date at all, and a game nobody has dated is still *on the radar* while it is neither counting down nor inside a launch window. Rejected for that reason: `Countdown`, `Launch window`, `Incoming`. `Release radar` was dropped as too close to Spotify's product name.
- **The verdict splits on provenance.** "Next up" reads as anticipation of a decision not yet made, so a bought title gets "Already yours: X, in 9 days." and "Bought, lands Aug 3: X." The chip is the only surface that knows which it is looking at, and it is the same voice as the imminent hero's "Next up — already yours" eyebrow. The TBA tier deliberately has no owned variant: a pre-order with no date at all is a shape Steam has not shown us (§ Chunk 1 decisions), so there is nothing to phrase and nothing to test.
- **A count with no fact is a real state and says so.** Every picker tier needs a date inside 90 days or a TBA title; a set of nothing but far coarse dates satisfies neither. The card keeps its count and drops to "Nothing lands in the next 90 days." rather than rendering an empty verdict or inventing a fifth tier.
