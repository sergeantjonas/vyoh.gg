...
**Status:** Original arc (chunks 0–6) complete. **A follow-on arc is in progress as of 2026-08-11** — the Upcoming view was wishlist-sourced, so pre-ordering a game deleted it from the calendar; it now covers owned-but-unreleased titles too and lives on its own `/steam/upcoming` route. Chunks 0–3 and 5 are done: **the reported defect is closed** (pre-orders reach the calendar, the imminent hero and the profile chips), **the route split has landed** (`/steam/wishlist` is the list alone, its tabs retired, `?tab=upcoming` redirected), and **the profile chip became a pair** — `Wishlist` for the backlog, `On the radar` for what lands next. Chunk 4 (guard + hero-meta) remains and is plumbing rather than the bug. See § Follow-on arc for the data precondition, the naming decision, and the per-chunk decisions. The rest of this note describes the original arc as shipped — note that § Route shape's tab model is superseded by the split.
