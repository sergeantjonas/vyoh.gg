...
- ~~**9a — Capture + projection.**~~ ✅ shipped 2026-05-25. `screenshotsAllAges Json` + `screenshotsMature Json` (both NOT NULL `@default("[]")`) added to `SteamGameEnrichment`. Projection in `projectEnrichment` drops malformed entries (missing `filename` / `ordinal`) so the read path stays branchless. URL helpers `steamScreenshotThumbUrl` (600x338) and `steamScreenshotFullUrl` (1920x1080) live in [packages/shared/src/steam/screenshots.ts](../../../packages/shared/src/steam/screenshots.ts); they auto-skip the size suffix when filenames already carry one. Backfill re-run via the existing [backfill-steam-enrichment.ts](../../../apps/api/src/scripts/backfill-steam-enrichment.ts) populated 193/196 rows.
- ~~**9b — Game-detail strip migration.**~~ ✅ shipped 2026-05-25. New `GET /steam/game/:appid/screenshots` endpoint returns `{ allAges, mature }`. [game-screenshot-strip.tsx](../../../apps/web/src/steam/game/game-screenshot-strip.tsx) swaps `useGameMedia` → `useGameScreenshots`, composes `{thumbUrl, fullUrl}` from the new bucket via `steamScreenshotThumbUrl` / `…FullUrl` helpers. **All-ages only by default** — mature bucket stays unconsumed until the owner-auth toggle lands (deferred to its own arc; `data.mature` is on the wire ready for that consumer). `useGameMedia` and the appdetails-side service stay in place for the hovercard until Chunk 9c sunsets both.
- ~~**9c — Hovercard rotation migration + appdetails sunset.**~~ ✅ shipped 2026-05-25. [library-tile-hovercard](../../../apps/web/src/steam/library/library-tile-hovercard.tsx) reads from `useGameScreenshots` (same hook as the strip) — same compose-via-helper shape. In the same commit: deleted `apps/web/src/steam/library/use-game-media.ts`, `apps/api/src/steam/screenshot.service.ts` (+ spec), `packages/shared/src/steam/media.ts` and its exports, the `getGameMedia` controller endpoint, the controller-spec's dedicated `getGameMedia` test, the `SteamScreenshotService` provider in `steam.module.ts`, and the `useGameMedia` cases in `use-steam-game-hooks.test.tsx`. Prisma migration drops the now-unused `screenshots` + `screenshotsFetchedAt` columns. **Bug found + fixed mid-arc:** the original `steamScreenshotThumbUrl` / `…FullUrl` helpers in Chunk 9a were doubly-prefixing the URL path (`/{appid}/steam/apps/{appid}/...`) because the stored `filename` already carries the `steam/apps/{appid}/...?t=…` shape. Fixed in commit `2c749de` — helpers now treat `filename` as a sub-`/store_item_assets/` path and inject the size suffix before the extension while preserving any `?t=` query.
