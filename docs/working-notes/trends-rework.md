...
- **Phase T4** ✅ shipped 2026-05-10. Phase-A trio (damage role consistency, vision investment, first-blood gold conversion) reads from `MatchSummary` Phase A schema fields. Phase-B trio (lane phase prognosis, death timing, comeback resilience) reads from new `MatchSummary` timeline-derived fields (csAt10/15, goldAt10/15, teamGoldDiffAt15, deathTimings). Sync paths now eagerly fetch the timeline alongside match details so new matches populate Phase B fields at insert time; historical paging stays untouched. One-shot bulk backfill (`src/scripts/backfill-timeline-metrics.ts`) populated the most recent 200 matches per account so tiles have an immediately useful sample. Static role baselines live at `lol/_shared/role-baselines.ts`.
...
- **2026-05-10** — Phase T4 Phase-B trio shipped (lane phase prognosis, death timing, comeback resilience). Six timeline-derived fields added to `MatchSummary` and `Match`: csAt10, csAt15, goldAt10, goldAt15, teamGoldDiffAt15, deathTimings (Int[]). New `riotTimelineToSummaryMetrics` helper resolves the user's participantId (from info.participants or via metadata fallback), reads the frame whose timestamp first reaches the requested minute, and harvests CHAMPION_KILL events for death timings. `backfillMissingMatches` gained a `fetchTimeline` opt set true on user-driven sync paths (head sync via cron, manual sync, list-window backfill). Historical paging stays off. The raw timeline is persisted to `MatchTimelineCache` in the same upsert pass so downstream views (build order, kill plot, lane phase) reuse it without a re-fetch. Bulk one-shot script (`src/scripts/backfill-timeline-metrics.ts`) populated the most recent 200 matches per account. Lane phase prognosis sidesteps SUPPORT (farming isn't the role's job, surfaces a pointer to vision investment instead). Death timing buckets across 12 three-minute bins and surfaces clusters at ≥25% density. Comeback resilience filters to matches down 5k+ team gold at 15 min and compares against a 30% population reference. All three gate sample on a "timeline projected" sentinel (csAt10 > 0 or teamGoldDiffAt15 ≠ 0) so historical rows missing the projection don't bias the average.
