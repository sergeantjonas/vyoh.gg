...
**Web Vitals dashboard.** Capture LCP, INP, CLS per route, plot trends. Existing pub/sub plumbing in [web-vitals.ts](../../../apps/web/src/lib/web-vitals.ts) (called from [__root.tsx](../../../apps/web/src/routes/__root.tsx)) is the entry point; the console reporter and the dev-only [perf-overlay.tsx](../../../apps/web/src/components/perf-overlay.tsx) subscribe, nothing persists or plots. **Sequenced behind the post-launch RUM backend (2026-09-06 decision):** before launch the only visitor is the owner's own dev box, so a dashboard would plot one machine; the RUM backend ([frontend-2026-gaps.md Gap 3](frontend-2026-gaps.md)) is what gives it data, and the dashboard is its read side. Could be an internal `/perf` route or a public one — public version is a case-study anchor.
...
9. ⚠️ **Web Vitals dashboard + bundle budget CI** — bundle budget shipped (`size-limit` runs as the "Bundle size budget" job in `ci.yml`); the per-route trend dashboard is sequenced behind the post-launch RUM backend, since there is no traffic to plot before launch.
