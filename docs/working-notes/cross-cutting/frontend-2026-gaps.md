...
**Shipped 2026-06-23 (commit 1 of 2):** App-root tier now wraps `<RouterProvider>` in [main.tsx](../../../apps/web/src/main.tsx) (since the Start migration: [router.tsx](../../../apps/web/src/router.tsx)) with a static, provider-independent reload screen (`AppErrorFallback fullScreen`), catching router/provider crashes the in-tree `<Outlet>` boundary can't see. A `WidgetBoundary` primitive plus shared `AppErrorFallback` / `WidgetErrorFallback` live in [error-boundary.tsx](../../../apps/web/src/components/error-boundary.tsx); the widget tier is applied to the command palette and splash backdrop (fail-silent, `fallback={null}`). The pre-existing `<Outlet>` page-content boundary in [__root.tsx](../../../apps/web/src/routes/__root.tsx) was refactored onto the shared `AppErrorFallback` (visually identical). **Commit 2** added a `ChartBoundary` preset (compact "This chart is unavailable." fallback) wrapping all 11 Recharts/visx chart leaves at their call sites: LP history, damage radar, gold-lead, lane-phase, map overlay, review gold-diff, champion position + death-matchup heatmaps, KDA trend, the champion-page win-rate sparkline, and the live team-composition radar. One chart crashing now fails small (compact fallback in place) without taking its sibling charts or the rest of the route-content region down. `ChartBoundary` wraps at the call site so it also catches the chart component's own data-prep, not just its render subtree. Route tier SHIPPED 2026-07-27 with Start chunk 5 — router-level `defaultErrorComponent` + `defaultPendingComponent`, so a rejected loader fails the region instead of the document.
...
- **Route-chunk prefetch on intent is on.** [router.tsx:116](../../../apps/web/src/router.tsx#L116) sets `defaultPreload: "intent"`. This is necessary but not sufficient (Gap 15 covers the data half).
...
- **Manual code-split annotations exist where they matter.** [apps/web/src/router.tsx:17](../../../apps/web/src/router.tsx#L17) lazy-loads `sonner` (toaster used post-mount). [apps/web/src/components/command-palette.tsx:4](../../../apps/web/src/components/command-palette.tsx#L4) lazy-loads the dialog body so `cmdk` doesn't ship in the initial chunk. [apps/web/src/lol/matches/match-event-timelines.tsx:31](../../../apps/web/src/lol/matches/match-event-timelines.tsx#L31) lazy-loads the map overlay. KB §8.3 rule of thumb (>50KB or behind interaction) is being followed.
...
- **Route prefetch on intent** ([apps/web/src/router.tsx:116](../../../apps/web/src/router.tsx#L116) — `defaultPreload: 'intent'`) — happy-dom has no hover intent timing or chunk-load behaviour. The feature exists; nothing tests that the prefetch actually fires.
