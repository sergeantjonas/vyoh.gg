...
**Status:** Superseded 2026-06-01 — the Canvas2D implementation described below shipped 2026-05-31 (Chunks 1–6) but was **replaced by the CSS atmosphere substrate** ([[project_atmosphere_substrate]], shipped 2026-06-01, arc [atmosphere-arc.md](atmosphere-arc.md) A-2). The hero is now CSS radial gradients driven by `MotionValue`s in [atmosphere-layer.tsx](../../apps/web/src/home/atmosphere/atmosphere-layer.tsx); [`AmbientHero`](../../apps/web/src/home/ambient-hero.tsx) is now a pure palette-claim registrar (`return null`) with **no canvas, no rAF draw loop, no `OffscreenCanvas`**. The canvas drift (60s sine phase) and cursor parallax (Chunk 5) were **deliberately not migrated** — "polish-on-polish at amplitudes (0.05 / 14 px) where loss is imperceptible" (see the comment block above `AmbientHero`). **The Canvas2D / rAF / WebGPU / OffscreenCanvas-worker chunk plan below is historical** — kept for the design rationale (time-of-day palettes, intensity reactivity, the Canvas2D-vs-WebGPU analysis), not as a description of the live implementation. The CSS-platform arc's C7 (OffscreenCanvas worker) was closed obsolete on this basis ([css-platform-2026.md](css-platform-2026.md) C7 §). Reacts to **time of day in Europe/Brussels** and **recent activity intensity** across LoL + Steam, via the atmosphere claim system. Composes with (not replaces) the existing `OrbMark`, which sits inside `LandingHeading` above the hero strip.
