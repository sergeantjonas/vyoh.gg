...
8. **Hero scroll-hint enrichment + per-band recurrence.** [`<HeroScrollHint>`](../../apps/web/src/home/hero-scroll-hint.tsx) currently ships as an anonymous bobbing chevron — a stream-agnostic "more below" affordance that fades on first 80px of scroll, now clickable (2026-06-01) so a click advances `<main>` by one viewport. It replaced the earlier `min-h-[85vh]` hero trick that leaked the next section into view as a scrollability signal (the leak fought the [bento + steam-band whileInView gates](../../apps/web/src/components/bento/bento-grid.tsx) and couldn't survive band reordering anyway). Two open dimensions, both deferred to this arc:

   - **Enrichment.** Once `useDominantStream` lands (A-5), the chevron can be enriched with the leading band's eyebrow text (e.g. `"Steam · latest unlock  ↓"` when Steam leads this week, `"League · standout match  ↓"` when LoL leads, rhythm-band label on flat weeks). Default: stay anonymous through A-9, revisit during low-activity-fallback polish, so the chevron doesn't compete visually with band entrances during A-3 / A-4 / A-6 development.
   - **Per-band recurrence.** Today the chevron only appears at the hero. The user (2026-06-01) noted this under-signals scrollability past the first band — once A-3 through A-8 establish the multi-band stack, the chevron should likely *recur* near the bottom of each non-terminal band as a "more below" cue, fading as the next band enters view. Open questions: do we render a single chevron driven by "distance to the next band's leading edge" against `useScroll`, or one chevron per band wired to its own `useScroll({ target, offset })`? Does the click target advance by `main.clientHeight` (current behavior) or snap to the next band's top? Does the chevron disappear entirely on the final band (footer), or morph into a "back to top" affordance there? Pick up alongside A-5 enrichment so per-band copy + per-band recurrence land together.
