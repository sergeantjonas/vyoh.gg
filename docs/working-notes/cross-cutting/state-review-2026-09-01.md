...
**Status:** Reference — docs-and-in-flight sweep (Phases 0, 1, 2 and 6 of the state-review procedure) run on `main` @ `045217d6`. Phases 3–5 (architecture, code quality, project setup) were deliberately not run this pass; see § 7. **Chunk 1 (docs reconciliation) shipped 2026-09-01 in the commit that adds this note** — F-1, F-3, F-4 and N-1 → N-5 closed; F-0 landed earlier as `045217d6`. The budget re-baseline (F-2, with measured attribution) and the shared coverage tests (F-5, `2b963357`) shipped the same day. The LP-overlay-per-duo recommendation shipped 2026-09-01 in two commits (owner-only endpoint + duo-row LP split, then the duo picker ringing the LP-history chart), closing match-depth Phase D. The previous full sweep is [state-review-2026-07-25.md](state-review-2026-07-25.md); its findings F-1 through F-18 are all closed except F-14 (partial by decision), so this review does not re-litigate them.
...
**F-2 · Initial-JS budget is 2.54 kB from tripping, unexplained. CONFIRMED — FIXED 2026-09-01.** Attribution by like-for-like `size-limit` runs (worktree at `16cb4e02` vs `HEAD`): 244.38 → 247.46 kB, +3.1 kB, all of it owner-auth and curation wiring in the root graph (`owner-badge`, `logout-button`, `review-dot`, `use-admin-steam-games`, `owner-request`, plus TanStack Query's mutation cache). The champion accent table's apparent +7.2 kB was a relocation from the entry chunk into the `champion-icon` chunk when `champion-theme` moved to `@vyoh/shared` (−6.6 kB on the entry; net +0.6 kB). Ceiling raised to 255 kB with the reason in `.size-limit.cjs` and `perf-baseline.md`; the owner-only lazy split is filed in `quick-wins.md`. Original finding follows. Evidence: `size-limit` output above; [perf-baseline.md:194](perf-baseline.md) recorded 241.65 kB on 07-27. **Why it matters:** the next feature chunk fails the "Bundle size budget" CI job (`ci.yml:111–127`) with no diagnosis on file, and the fix under time pressure will be "raise the number". **Fix:** attribute the +5.8 kB (build at `467e29f9` and at `HEAD`, diff the manifest's static-import set), then either trim or raise the ceiling *with the reason recorded* in both `.size-limit.cjs` and `perf-baseline.md`. **Effort:** one sub-session.
