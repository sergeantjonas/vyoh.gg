...
| [page-composition](page-composition.md) | Decide section-structure (IA — when to group cards under dividers vs flat list) and container convention (chrome around section bodies vs bare) per surface. Sweep ~10 product surfaces with per-surface decisions; codify ruleset alongside the typography arc's CardTitle/SectionTitle rule. | Active — Chunks 1 (audit) + 2 (top-3 surface decisions) shipped 2026-05-27. Compositional container rule ratified: chrome belongs at the lowest level that visually groups heterogeneous content; don't nest chrome. Steam game-detail + LoL Your Game both promoted to reference (no change). One Chunk 4 commit identified: 3 ad-hoc span headers on LoL profile → `SectionTitle`. Chunks 3–5 pending. |
