...
1. **nav-condensation-arc** — three-layer chrome → two; unblocks `cross-section-nav-arc` and composes with the active-tab layoutId morph.
2. **cross-section-nav-arc** — only after nav-condensation 1.1 lands; composes the merged strip with the section-shell VT pattern.
