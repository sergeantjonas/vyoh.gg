...
| [anchor-positioned-overlays](anchor-positioned-overlays.md) | CSS Anchor Positioning for command-palette result peek + hover-card follow-on-scroll; feature-detect + Oddbird polyfill fallback | 🟡 In flight — Chunks 1 + 2 landed 2026-05-27 (`ensureAnchorPositioning()` feature-detect + Oddbird lazy-loader; `<CommandPalettePreview>` shell wired to focused cmdk row). Chunks 3–7 pending. |
