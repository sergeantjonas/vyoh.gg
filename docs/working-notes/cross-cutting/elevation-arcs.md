...
| [og-image-pipeline](og-image-pipeline.md) | Per-route OG images (Satori) for shareable match/champion/profile/Steam deep-links; SEO + share-delight win | 🟡 Match endpoint shipped (retrofitted to 1200×630 on 2026-06-07 as Chunk 0); Champion / Profile / Home / Steam endpoints pending. All endpoints dynamic per request, assets via image proxy. |
...
Curated 2026-05-28 to remaining picks only — the original 1–15 ranking from 2026-05-23 covered seven arcs that have since shipped. When updating after the next shipping wave, drop completed items rather than annotating around them. Refreshed 2026-06-07 to drop motion-choreography-arc + atmosphere-arc (both shipped) and personal-record-moments (shipped); self-portrait-recap-arc's page-shape work shipped same day with the remainder gated on observational data.

1. **og-image-pipeline** — once shareable URLs exist, OG cards convert; pairs with [self-portrait-surfaces.md](self-portrait-surfaces.md). Highest-leverage planned arc on this list.
2. **detail-panel-arc** — biggest structural lift on this list; defer until the simpler arcs have absorbed their feedback.
3. **optional-ui-audio** — bold; consider after the visual baseline lands so the audio doesn't carry the whole "wow".
4. **self-portrait-recap-arc — observational follow-on** — measure dry-spell fill rates before scoping the next Lane A/B detector batch; not a fresh-pick item but worth watching for the next opportunity.
