...
| [page-composition](page-composition.md) | Decide section-structure (IA — when to group cards under dividers vs flat list) and container convention (chrome around section bodies vs bare) per surface. Sweep ~10 product surfaces with per-surface decisions; codify ruleset alongside the typography arc's CardTitle/SectionTitle rule. | Planned — opened 2026-05-27 after editorial-typography sweeps surfaced LAYOUT-level inconsistencies (Steam game-detail flat-cards vs LoL profile grouped-sections; match-detail chart sections bare vs Steam in-chrome). |
