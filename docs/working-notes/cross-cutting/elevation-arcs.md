...
| [motion-choreography-arc](motion-choreography-arc.md) | App-wide editorial motion vocabulary (Linear/Resend register) — `<EditorialHeading>` primitive, section stagger variants, `whileInView` adoption, reduced-motion contract. Foundation arc that [atmosphere-arc](atmosphere-arc.md) consumes; both succeed [landing-showcase-arc](landing-showcase-arc.md). | ✅ M-1 through M-6 shipped 2026-05-31. M-7 (propagation to `/lol` and `/steam` routes) tracked as a separate followup arc, not part of this one. |
...
| [self-portrait-recap-arc](self-portrait-recap-arc.md) | Always-on Wrapped: `/` becomes a scroll-paced editorial recap of recent activity. Multi-band **chapters** (subject = Ahri + Steam games; moment = RANK_UP, OFF_META, MARATHON, etc.) over the shipped atmosphere substrate. Apple-style pin + progressive reveal per chapter; recency-decayed scoring with no fixed window. Retires ADR-2 (no-recognizable-imagery); ships hard-coded curation, deferred-trailer ADRs. | ✅ Page-shape R-1 through R-15 shipped 2026-06-07. R-7i Lane A's first detector (`FAVORITE_CHAMPION_OF_PERIOD`), R-7i Lane B's first detector (`LIFETIME_PEAK_RANK`), R-10 (Steam beat-4 trailer slot promotion), and R-11 (reduced-motion + Safari engine-gate verification) all landed the same day. Remaining work is observational: secondary Lane A detectors + remaining Lane B candidates depend on watching dry-spell fill rates from real activity patterns. No scoped chunks open. |
...
| [og-image-pipeline](og-image-pipeline.md) | Per-route OG images (Satori) for shareable match/champion/profile/Steam deep-links; SEO + share-delight win | ✅ Shipped 2026-06-07. All five endpoints (match retrofitted to 1200×630, champion, profile, home, Steam game) live; all dynamic per request, assets resolved via the canonical image resolver (`LolImageService`/`SteamImageService`); `index.html` default OG meta + `twitter:card: summary_large_image`. Owner share affordance stays out of scope. |
...
| [detail-panel-arc](detail-panel-arc.md) | Detail pages (match / champion / Steam game detail) become right-aligned `max-w-4xl` non-modal slide-over panels with URL-as-state; list stays mounted, row→panel VT morph for click-from-list, virtualizer-scroll-to-row for cold deep-link arrivals | ✅ Shipped 2026-06-07 → 2026-06-09 (Chunks 2.1 + 2.2 + 2.3). Chunk 2.4 (mobile drawer) deferred until mobile-first work picks up. Late-arc Chrome compositor diagnosis spun off into [panel-compositor-load.md](panel-compositor-load.md). |

---

## Pick order suggestion

Curated 2026-05-28 to remaining picks only — the original 1–15 ranking from 2026-05-23 covered seven arcs that have since shipped. When updating after the next shipping wave, drop completed items rather than annotating around them. Refreshed 2026-06-07 to drop motion-choreography-arc + atmosphere-arc (both shipped), personal-record-moments (shipped), and og-image-pipeline (shipped same day); self-portrait-recap-arc's page-shape work shipped same day with the remainder gated on observational data. Refreshed 2026-06-09 to drop detail-panel-arc (Chunks 2.1 + 2.2 + 2.3 all shipped; 2.4 mobile drawer deferred until mobile-first work picks up).

1. **Compositor-load universal audit** — apply the panel-arc diagnosis to the rest of the app: `content-visibility: auto` sweep on long lists, frosted-tile density audit, layer-promotion triggers on shared row classes, ambient-layer review, plus a checked-in perf-probe and a layer-count budget in repo-conventions. → [panel-compositor-load.md § Universal optimization pass](panel-compositor-load.md#universal-optimization-pass).
2. **optional-ui-audio** — bold; consider after the visual baseline lands so the audio doesn't carry the whole "wow".
3. **self-portrait-recap-arc — observational follow-on** — measure dry-spell fill rates before scoping the next Lane A/B detector batch; not a fresh-pick item but worth watching for the next opportunity.
