...
## A1 — Global fallback exception filter — shipped 2026-09-06

**Shipped** as [fallback-exception.filter.ts](../../../apps/api/src/fallback-exception.filter.ts), a `@Catch()` registered *before* the Riot filter in `main.ts` because Nest matches global filters last-registered-first. It passes Nest `HttpException`s through untouched (a 5xx one is logged), maps `SteamClientError` the way the Riot filter maps `RiotError` (404/429/504 pass through, the rest → 502) and `SteamRateLimiterTimeoutError` → 503, maps Prisma P2025 → 404 and P2002 → 409 with every other Prisma code → 500, `PrismaClientInitializationError` → 503, and answers 500 to anything else while logging the stack with the route. Client bodies are `{ statusCode, message }` except a passed-through `HttpException` body, and never carry an upstream or database message. A `conventions.spec.ts` lint pins the registration order, and the filter sets a JSON `Content-Type` because the image and OG routes declare an image type up front. Spec mirrors the Riot filter's. Original scope follows.
