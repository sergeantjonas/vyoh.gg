...
| 13 | seo | ✅ refreshed | 2026-05-24 | Project-first sweep + KB refresh. Project outputs: Round 8 in frontend-2026-gaps.md (Gaps 28–33: robots.txt silent on AI tokens, zero JSON-LD, sitemap missing `lastmod`, per-route `head()` only on match-detail, `twitter:card` still `summary`, no `max-image-preview:large`), 3 SEO-hygiene quick-wins (AI crawler tokens, sitemap `lastmod`, robots meta), new "SEO — evaluated alternatives" section in library-shortlist.md (react-helmet-async/next-seo rejected, schema-dts/gray-matter parked with triggers, framework-coupled sitemap generators rejected, absolute-URL helper as build-our-own pattern). KB outputs: §3 new `Person`-vs-`Organization` decision for personal sites, §5 AI-crawler-policy-by-site-shape table with portfolio-row default-inversion callout, §8 "What's shippable today on a pure CSR SPA" subsection (forward-compat migration-prep pattern for per-route `head()`), §15 TanStack Router (non-Start) `head:` shape clarified separately from Start, §15 absolute-URL helper as load-bearing pattern, README new decision-shortcut rows (AI crawler policy / Person-vs-Organization / per-route metadata before SSR). |
...
| 20 | data-visualization (new — see [kb-expansion](frontend-2026-kb-expansion.md)) | ✅ **compiled** | 2026-06-14 (Phase 1 + Phase 2) | **Both phases landed 2026-06-14.** Phase 1 project outputs: Round 9 in [frontend-2026-gaps.md](frontend-2026-gaps.md) (Gaps 34–36: custom heatmaps pointer-only, no table-fallback on any chart, undocumented 3-tier charting boundary + non-gaps recording Recharts-3 `accessibilityLayer`-on-by-default, deliberate palette hex, no SVG-DOM cliff), 1 quick-win (descriptive `aria-label` on the 7 Recharts charts), new "Data visualization" decision section in [library-shortlist.md](library-shortlist.md). Phase 2 KB output: `~/.claude/knowledge/frontend-2026/20-data-visualization.md` (`last_verified: 2026-06-14`) — grammar-vs-components-vs-primitives framing axis, full library landscape (Recharts default / visx escape-hatch / hand-rolled / Observable Plot+ECharts+Chart.js+nivo deferred-with-triggers), d3-as-utilities pattern, SVG→Canvas→WebGPU cliff ladder, chart-a11y core (table fallback as default, navigable-points as the costlier option, canvas-must-have-table, WCAG 1.4.1/1.4.11), perf (library-tweens-vs-Motion, ResizeObserver thrash, LTTB downsampling), OKLCH color-scale grammar, tables-as-viz. README integration: domain-index row 20, load-when-X row, "Which charting library?" decision-shortcut row, 17→18 file count, "What changed" expansion note. Web-verified this session: Recharts 3.0 `accessibilityLayer` default-true, **visx 4.0.0 just shipped (~2026-06-12)** so the project's `^3.x` pin is now a behind-major (noted in the file), Observable Plot active, ECharts 6.0 shipped. |

## Suggested order

1. **17 — cross-platform-edge-auth** *(Tier 2)*
   Pick up opportunistically when [owner-auth.md](../ops/owner-auth.md) gets reopened or the [hosting.md](../ops/hosting.md) decisions firm up. Don't do it speculatively.

Tier 1 is complete (15 / 05 / 07 all ✅ refreshed 2026-05-23). Tier 2 testing (10) and light SEO (13) are now ✅ refreshed 2026-05-24. Only Tier 2 cross-platform-edge-auth remains in the active queue; everything else is deferred or spec-driven.
