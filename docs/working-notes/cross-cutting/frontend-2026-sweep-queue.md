...
| 20 | data-visualization (new — see [kb-expansion](frontend-2026-kb-expansion.md)) | 🟡 **Phase 1 done** — Phase 2 pending | 2026-06-14 (Phase 1) | **Phase 1 landed 2026-06-14** (checkpoint per the "stop before Phase 2" rule). Project outputs: Round 9 in [frontend-2026-gaps.md](frontend-2026-gaps.md) (Gaps 34–36: custom heatmaps pointer-only, no table-fallback on any chart, undocumented 3-tier charting boundary + non-gaps recording Recharts-3 `accessibilityLayer`-on-by-default, deliberate palette hex, no SVG-DOM cliff), 1 quick-win (descriptive `aria-label` on the 7 Recharts charts), new "Data visualization" decision section in [library-shortlist.md](library-shortlist.md) (Recharts/visx/hand-rolled three-tier rule; Observable Plot rejected, ECharts deferred past 1k-point cliff). **Phase 2 (KB file `20-data-visualization.md`, 5–8k words + README rows) not started** — next chunk; recommend a fresh/compacted session. |
