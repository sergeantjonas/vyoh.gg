...
Cap-height/alphabetic trimming (Baseline 2025 — inside floor) for `SectionTitle` / `CardTitle` / recap display numerals. Kills the invisible half-leading that makes optical alignment with chips and borders approximate today. One-line-per-primitive change at the primitive level (same single-change-point principle as the header-primitive migration); visual diff review required since every header shifts by a few px.
