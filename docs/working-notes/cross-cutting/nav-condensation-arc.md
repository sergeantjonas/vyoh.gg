...
**Perf:** the static hero carries **no** gate (one image + a CSS drift). The scroll/nav morphs are compositor-friendly (Motion layout + VT snapshot on `<main>`), but a host-Chrome INP + scroll-jank re-baseline against [perf-baseline.md](perf-baseline.md) on this highest-traffic route is still **open** before treating the morphs as perf-validated.
