...
**Still pending — M2 (scroll-collapse morph):** the hero collapses into the compact strip identity via shared `layoutId` on scroll (gated to profile-index + scroll-top), reverses on scroll-up; other tabs always show the compact strip. Reduced-motion + a host-Chrome perf re-baseline (layout morph on the highest-traffic route) gate it before "shipped." The static hero above carries **no** perf gate (one image + a CSS drift).
