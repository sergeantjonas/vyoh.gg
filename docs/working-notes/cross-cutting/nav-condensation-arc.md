...
- **Detail pages restore the section nav. SHIPPED (2026-05-30).** Match-detail now renders the full section nav (Profile · Matches · Trends · Champions) in the merged strip with **Matches highlighted** — [`$accountSlug.tsx`](../../apps/web/src/routes/lol/$accountSlug.tsx) computes `lolTabs`/`lolLive` unconditionally instead of the old `isMatchDetail ? []` guard. The detail page's own Recap / Your game / Review / Timeline tabs were demoted from sticky chrome to an **inline tab bar at the top of the detail content** (always in-flow now; the `heroScrolledPast && "invisible"` toggle is gone). **DECISION (owner, 2026-05-30): the `ChampionStickyStrip` is kept, but its compact copy of the detail tabs was removed.** The strip still pins champion icon + W/L + KDA + LP as scroll context while deep in a match — it just no longer duplicates the tab bar (that double-sticky-chrome was the clutter this arc condensed). Tradeoff accepted: switching detail sub-tabs while scrolled deep now means scrolling back up; the section nav above stays sticky. The breadcrumb (`← Matches`) was already inline and stays as-is; the richer `· Match · date vs X` breadcrumb is **not done** (deferred polish, non-blocking).
...
- Detail-page section-nav-restored test — **shipped** as a logic test in [`account-tab-helpers.test.ts`](../../apps/web/src/lol/account/account-tab-helpers.test.ts): on every match-detail subpath (`/matches/<id>`, `/recap`, `/your-game`, `/review`, `/timeline`) the active-state map is `[Profile:false, Matches:true, Trends:false, Champions:false]`. This guards the user-facing "Matches highlighted on detail" guarantee.
- Detail-page inline-tabs demotion — **no new test, by decision.** The demotion is a route-composition change in [`$matchId.tsx`](../../apps/web/src/routes/lol/$accountSlug/matches/$matchId.tsx); the moved `MatchDetailTabs` component keeps its full coverage in [`match-detail-tab-nav.test.tsx`](../../apps/web/src/lol/matches/match-detail-tab-nav.test.tsx). A full-route render test (mocking `createFileRoute` + ~10 data hooks) was judged disproportionate and brittle for asserting "rendered once, inline." Add it if a regression in the strip composition ever surfaces.
