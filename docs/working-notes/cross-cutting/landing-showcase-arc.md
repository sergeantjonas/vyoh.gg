...
**Status:** Active 2026-05-31 — picked up at the top of the [elevation-arcs.md](elevation-arcs.md) pick order. Chunks 1 (editorial display headline) + 2 (static CSS ambient hero) + 3 (Canvas2D promotion with rAF drift, visibility pause, reduced-motion/low-power → static fallback, lazy-loaded canvas chunk) landed. Remaining: Chunk 4 (activity-intensity reactivity from LoL matches-in-24h + Steam playtime today) and Chunk 5 (composition pass with bento + perf-baseline writeback). Consumes [ambient-home-hero.md](ambient-home-hero.md) verbatim for the hero rungs.
