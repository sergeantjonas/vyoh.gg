...
| 11 | Sheen on Steam tiles + rows | [sheen-overlay.tsx](../../../apps/web/src/components/sheen-overlay.tsx) (one component since 2026-09-06; was inline in library-tile, steam-game-row and hundred-percent-hall) | ❌ Missing | No `@media (prefers-reduced-motion)` block drops the `transition-[--sheen-extent,opacity]` 900ms hover. Standard wants the gradient stop pinned at midpoint (~33%) with no transition. |
