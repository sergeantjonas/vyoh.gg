...
- **R-7i. LoL dry-spell top-up — UNPARKED, pending design.** R-7b.1 parked the symmetric question ("should LoL have a top-up?") on the bet that more detectors would fill the LoL slot probabilistically. Six detectors later (RANK_UP, KDA_OUTLIER, RETURN_FROM_HIATUS, STREAK_5W/5L, MARATHON, OFF_META_PICK) that bet has empirically not converged for a 30-day dry spell — if nothing notable happened in the last 30d of ranked play, the LoL block goes thin/empty regardless of detector count. Owner wants explicit fill. Two lanes, not mutually exclusive:
  - **Lane A — additional detectors that don't require recent excitement.** Things that can fire from any 30d window of any activity: `FAVORITE_CHAMPION_OF_PERIOD` (most-played champ over the window, framed as "you spent the month on Annie"), `LANE_SHIFT` (primary lane changed vs prior period), `MASTERY_TIER_UP`, `PEAK_RANK_OF_PERIOD` (highest rank held during the window even if no climb). Lower-magnitude `baseSignal` than rank/KDA outliers — these are "filler" tier by design, decay below floor at first sign of a real moment. Categorically still moments, no architectural change.
  - **Lane B — explicit dormant top-up source, analogous to `collectDormantTopUp`.** When active LoL moment count < cap, pull from a fallback pool. Candidates for the pool: cached lifetime-peak ranked snapshot (framed retrospectively — "You climbed to Plat IV last season"); long-tail champion mastery milestones older than 30d; the head row of a `lol-season` aggregate descriptor narrowed to a single editorial framing per call. Original park rejected aggregate `lol-season`-shaped descriptors as a category mismatch — un-park-able if the chapter framing carries clearly retrospective register ("Looking back —") so the reader doesn't expect recent-moment energy.
  - **Decision needed before implementation.** Lane A is structurally cleaner (no new feed type) but uncertain whether the new detectors actually fire often enough to close the gap. Lane B is deterministic but reopens the category-mismatch debate the original park flagged. Suspect the answer is "ship Lane A first as additive detectors, measure, ship Lane B only if dry spells persist after Lane A lands". Stub a measurement in `RecapSubjectsService` first so we can tell whether Lane A alone is sufficient before committing to Lane B.
...
- **Small-screen layout collision (NEW, 2026-06-04).** On viewports below ~720px tall, beat content overlaps the chapter masthead because the inner content is absolutely-positioned-equivalent (via the counter-translate inner `<m.div>` and h-dvh geometry) without accounting for the masthead's height. The mastheader and beat content render in the same 100dvh box without flex/spacing that adapts. Needs a chunk: introduce a layout primitive that reserves masthead height (probably `grid` rows: `auto` for masthead + `1fr` for beat content) so the beat content's vertical center accounts for the masthead. Likely also requires tuning `h-dvh` → `h-[100dvh-var(--masthead-h)]` or equivalent, with the masthead height published as a CSS var by the chapter root. Defer specific layout decisions until R-13 lands so the fix sits on the final beat model.
