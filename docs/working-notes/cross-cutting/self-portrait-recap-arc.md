...
**R-10. Trailer polish (ADR-5 promotion). ✅ SHIPPED 2026-06-07.** Reused the existing microtrailer pipeline (`steamMicrotrailerUrl` + `steamMicrotrailerPosterUrl`) rather than introducing a parallel `movies[]` fetch (the open decision in the original chunk plan resolves toward "reuse pipeline" — the 6-second silent loop is editorially honest as a chapter closer; the masthead's click-through to `/steam/game/$appid` opens the full-trailer surface for users who want the longer cut). `SteamGameRecap` extended with the four microtrailer fields (`microtrailerWebm/Mp4/Poster/Name`) carried verbatim from `SteamOwnedGame`'s enrichment row through `deriveSteamGameRecap`. `SteamChapterCloserMedia` (the slot introduced in R-13 chunk 2) now branches on three gates: (1) trailer fields present, (2) `useReducedMotionConfig()` returns false — `Config` not bare `useReducedMotion` so a parent `<MotionConfig reducedMotion="always">` wins, (3) `navigator.connection?.saveData !== true` AND `effectiveType` is not `slow-2g`/`2g`/`3g`. All three pass → renders a looping silent `<video>` with `playsInline` + `muted` + `loop` + `preload="metadata"` + the Steam-supplied microtrailer name as `aria-label`. Any fail → falls back to the prior screenshot lightbox strip unchanged. The trailer's `<video>` lives inside a child component (`TrailerLoop`) so the ref + autoplay/pause effect don't re-mount when the outer gate flips mid-session. Autoplay lifecycle keys on the per-beat `active` signal (`useChapterBeatNudge` via the multi-beat parent, surfaced as the slot's `active` prop) — `play()` fires on flip-to-true, `pause()` + `currentTime = 0` on flip-to-false so scrubbing past then back replays from the first frame, not from mid-loop. Mobile-data heuristic uses the Network Information API (non-standard, present on Chromium-family browsers; Safari + Firefox return `undefined`, defaulting to "allow" — the right call for desktop). Specs added: closer-media component cases (renders trailer when motion+data permit / falls back under no-trailer / falls back under reduced motion / falls back under saveData / play+pause lifecycle on active flips / null-out when no trailer and no screenshots) + zero-state recap test extended with the four new fields.
