...
**R-2. Ahri subject chapter (hardcoded data).** First end-to-end chapter. Hardcoded skin list (default placeholder). Pulls real match data via existing hooks (`useMatches` filtered to Ahri). Renders all four bands inside one pin. Includes the splash-rotation beat with blur-bloom transition. Tests against a fixture match list. **First visible chapter.**

**R-3. Steam subject chapter (hardcoded appid).** Second chapter type, hardcoded to a known appid (e.g. latest-unlock game). Pulls game hero image, unlocks, playtime. Screenshots rotator (trailer deferred per ADR-5). **Two chapters visible; eyeball the chapter-to-chapter crossfade.**

**R-4. `useChapters()` selection logic.** Scoring function, recency decay, floor threshold, age-bucket copy framing. Drives chapter list off real data instead of hardcoded entries. `landing-config.ts` curation overlay (`HIDDEN_APPIDS`, `PINNED_CHAPTER`, `CHAPTER_COPY_OVERRIDES`). Steam app-type filter. Test coverage for selection edge cases (empty, hiatus, single chapter, multi-chapter, override). **Page becomes data-driven.**
