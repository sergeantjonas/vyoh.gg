...
**Status:** Shipped 2026-06-07. Page-shape arc R-1→R-15 fully landed, including R-7i Lane A's first detector (`FAVORITE_CHAMPION_OF_PERIOD`), R-7i Lane B's first detector (`LIFETIME_PEAK_RANK`), R-10 (Steam beat-4 trailer slot promotion), and R-11 (reduced-motion + Safari engine-gate verification). Remaining work parked / measure-then-decide: R-7i's secondary Lane A detectors and remaining Lane B candidates ride on observation of dry-spell fill rates from real activity patterns. No open chunks. Supersedes [atmosphere-arc.md](atmosphere-arc.md) A-3 onward (A-1 / A-2 / A-2a remain as the shipped substrate). Builds on top of [motion-choreography-arc.md](motion-choreography-arc.md) for entrance vocabulary. Resolves [landing-showcase-arc.md](landing-showcase-arc.md) D4-2's stripped-image interim state by re-introducing recognizable imagery via the substrate-supported claim system inside an editorial framing.
...
- **R-7i. LoL dry-spell top-up — Lane A first detector SHIPPED 2026-06-07.** R-7b.1 parked the symmetric question ("should LoL have a top-up?") on the bet that more detectors would fill the LoL slot probabilistically. Six detectors later (RANK_UP, KDA_OUTLIER, RETURN_FROM_HIATUS, STREAK_5W/5L, MARATHON, OFF_META_PICK) that bet has empirically not converged for a 30-day dry spell. Owner unparked.
  ...
  - **Lane B — explicit dormant top-up source, analogous to `collectDormantTopUp`.**
    - **`LIFETIME_PEAK_RANK`. ✅ SHIPPED 2026-06-07.** Unparked the same day as FAVORITE landed, in response to empirical confirmation that Lane A alone doesn't close the gap when ranked play has been genuinely dormant — every Lane A detector (including FAVORITE) still requires ≥1 ranked match inside the 30d window, which a true dry spell doesn't satisfy. The retrospective register the original park called for ("Looking back —") makes the category mismatch concern fall away: the chapter doesn't pretend to be a recent moment, it's explicitly a "lifetime peak" surfaced today.
    - `LolMomentsService.detectLifetimePeak(now)` is the first windowless detector in the service. **Source is `RankSnapshot`, not `Match.snapshot*`.** The Match table's snapshot fields exist in the schema but aren't currently backfilled (intended for a per-match snapshot lift that never landed — empirical owner-data probe on 2026-06-07 confirmed 198 owner ranked matches, 0 with snapshot fields populated). `RankSnapshot` is the canonical rank-tracking table; `MatchSyncService` calls `LolService.captureRankSnapshot()` after every ranked match completes. The detector reads all owner snapshot rows with `queueId` in `RANKED_SOLO_5x5` / `RANKED_FLEX_SR` (Riot's API vocabulary, distinct from `Match.queueType`'s editorial strings), sorts desc by `normalizeLp(tier, rank, leaguePoints)`, and picks the head. To surface a champion + matchId for the aggregator/chapter (RankSnapshot carries neither), it joins on `match.findFirst` ordered by playedAt desc — the editorial framing of "looking back, but here's what you're playing now" is honest under the retrospective register. When no ranked matches exist at all, `championAlias` falls back to `"Ahri"` so `routes/index.tsx`'s `Boolean(championAlias)` filter still admits the chapter (Ahri is the page's structural anchor LoL subject anyway).
    - `daysSince` anchors at `0` regardless of when the peak was actually hit — the chapter is being SURFACED today even though its CONTENT is retrospective. The actual peak date lives on `lifetimePeak.achievedAt` so the prose can ground the year ("Season YYYY"). Anchoring on real daysSince would let recency decay sink the score below floor for any peak older than ~30d, defeating the dry-spell-filler purpose.
    - `baseSignal = LIFETIME_PEAK_BASE_SIGNAL = 10` — at `daysSince=0` the recap score is 10, above the floor (5) and well below RANK_UP's 35 / KDA_OUTLIER's 21+, so real recent events still outscore and order above the retrospective when both fire.
    - Descriptor extended with `lifetimePeak: { tier, rank, leaguePoints, achievedAt }` (`LolLifetimePeakStats` in `@vyoh/shared`) and a new momentType `"LIFETIME_PEAK_RANK"`. The candidate carries `championAlias` from the peak match (used by the aggregator for the splash backdrop and by the chapter prose for the "championed by X" clause) but the masthead itself is the rank, not the champion.
    - Chapter renders "Looking back" eyebrow + rank-tier emblem + `formatRankTitle(tier, rank)` masthead + "Lifetime peak — *Emerald I*, championed by *Ahri* · *Season 2024*" prose. Cyan accent (`text-cyan-300`) sets the cool retrospective register apart from RANK_UP's warm amber — both are about rank achievements, but RANK_UP is "you climbed recently" (warm, current) and LIFETIME_PEAK is "you peaked at some point" (cool, retrospective). No per-match receipt — the chapter is a single-line retrospective marker; the matchId link carries the click-through.
    - Specs added: detector cases (no puuids / no RankSnapshot rows / highest normalized LP picked regardless of age / daysSince anchored at 0 / capturedAt → achievedAt ISO / Ahri championAlias fallback when no ranked matches / queueId vocabulary RANKED_SOLO_5x5+RANKED_FLEX_SR) + chapter cases (looking-back eyebrow + rank masthead + Season caption / championed-by clause omitted when championAlias is empty / no per-match receipt / null-lifetimePeak fall-through to OFF_META default).
    - **Lifetime-peak is now the always-on LoL top-up** — equivalent to Steam's `collectDormantTopUp` in spirit, just emitted from inside the detector pipeline rather than as a separate selector pass. The remaining Lane B candidates (long-tail champion-mastery milestones, `lol-season` aggregates) stay parked behind the same observation gate as the remaining Lane A detectors: if LIFETIME_PEAK alone closes the dry-spell gap on real activity patterns, the additional shapes don't ship.

- **R-7b.1. Recap ordering + dormant top-up — ✅ SHIPPED 2026-06-03.** Two cross-cutting tweaks landed after R-7b's first review surfaced the "Standout sits between RE4 and (future) Steam moments" awkwardness and the "RE3/Pragmata/RE2 fall out of the page" dry-spell problem. **(1)** Cross-kind order in `selectChapters` flipped from `steam-subject → lol-moment → steam-moment` to `lol-moment → steam-subject → steam-moment` — now the page reads Ahri-anchor → LoL block → Steam block → conclusion with one platform jump instead of Steam → LoL → Steam thrashing. **(2)** Per-kind caps bumped: `steamSubjectCap` 3 → 5, `lolMomentCap` 3 → 5, `steamMomentCap` 2 → 3 (page felt thin at 4–5 total chapters). **(3)** Dormant fallback promoted from "fires only when active is empty" to "always tops up remaining Steam slots": `RecapSubjectsService.getChapters` now counts active steam-subjects, computes slack against `STEAM_SUBJECT_HARD_CAP = 5`, and pulls dormant lifetime-ranked rows (5h floor) into the trailing slots, excluding appids already in the active block. Active rows still sit first inside the Steam block ("Playing lately on RE4"); dormant rows trail ("Earlier this year on RE3"). Same descriptor kind, daysSince-bucket eyebrows on the web side carry the editorial register split. The asymmetric concern ("should LoL have a similar top-up?") is parked at the end of R-7 — for now the answer is "let more detectors do the filling"; aggregate "lol-season"-shaped descriptors would be a category mismatch with the moment slot. `collectDormantFallback` renamed to `collectDormantTopUp(now, take, excludeAppids)`; `DORMANT_CAP` constant removed (replaced by the dynamic `take` parameter). Tests updated: `dormant fallback` describe block renamed to `dormant top-up`, two existing tests rewritten to reflect always-on behavior, two new tests added (top-up trailing position, appid-exclusion). **Two defects in this lane were confirmed 2026-08-25** — it cannot distinguish a completion from an abandonment, and a single round re-tops it (no score floor, and the brief-launch guard is bypassed by the unlock half of `freshest`). The recency sort itself is *correct* and stays: `/` is a current-activity page, so magnitude belongs in a separate "Greatest hits" kind, per the `baseSignal` comment. Analysis in [dormant-chapter-ranking.md](dormant-chapter-ranking.md).
...
**R-10. Trailer polish (ADR-5 promotion). ✅ SHIPPED 2026-06-07.** Reused the existing microtrailer pipeline (`steamMicrotrailerUrl` + `steamMicrotrailerPosterUrl`) rather than introducing a parallel `movies[]` fetch (the open decision in the original chunk plan resolves toward "reuse pipeline" — the 6-second silent loop is editorially honest as a chapter closer; the masthead's click-through to `/steam/game/$appid` opens the full-trailer surface for users who want the longer cut). `SteamGameRecap` extended with the four microtrailer fields (`microtrailerWebm/Mp4/Poster/Name`) carried verbatim from `SteamOwnedGame`'s enrichment row through `deriveSteamGameRecap`.

**Composition: trailer + screenshots, not trailer OR screenshots.** First draft replaced the screenshot strip with the trailer when both were available — owner pushed back; the two carry different editorial weight (trailer = "this game in motion"; screenshots = "moments worth lingering on" + the lightbox click-through). Final shape renders the trailer ABOVE the strip when both are present, with the strip's per-thumb cascade nudged later by 0.25s so the trailer lands first and the strip follows as a hero-then-supporting reveal.

`SteamChapterCloserMedia` (the slot introduced in R-13 chunk 2) now gates only the trailer on three conditions: (1) trailer fields present, (2) `useReducedMotionConfig()` returns false — `Config` not bare `useReducedMotion` so a parent `<MotionConfig reducedMotion="always">` wins, (3) `navigator.connection?.saveData !== true` AND `effectiveType` is not `slow-2g`/`2g`/`3g`. When the trailer gate fails (any of the three), the screenshot strip still renders on its own — the original closer slot's behavior. When the trailer gate passes, both render stacked. The trailer renders a looping silent `<video>` with `playsInline` + `muted` + `loop` + `preload="metadata"` + the Steam-supplied microtrailer name as `aria-label`, lifted into a child component (`TrailerLoop`) so the ref + autoplay/pause effect don't re-mount when the outer gate flips mid-session. Autoplay lifecycle keys on the per-beat `active` signal (`useChapterBeatNudge` via the multi-beat parent, surfaced as the slot's `active` prop) — `play()` fires on flip-to-true, `pause()` + `currentTime = 0` on flip-to-false so scrubbing past then back replays from the first frame, not from mid-loop. Mobile-data heuristic uses the Network Information API (non-standard, present on Chromium-family browsers; Safari + Firefox return `undefined`, defaulting to "allow" — the right call for desktop). Specs added: closer-media component cases (trailer + screenshots stacked when both available / trailer-only when screenshots empty / screenshots-only when no trailer / screenshots-only under reduced motion / screenshots-only under saveData / play+pause lifecycle on active flips / null-out when both empty) + zero-state recap test extended with the four new fields.
