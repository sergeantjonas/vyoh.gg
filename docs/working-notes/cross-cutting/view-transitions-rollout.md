...
1. **AnimatePresence → VT-driven section slides** (own arc — recommend a new working note `cross-cutting/section-shell-vt-migration.md` plus a Tier 1 entry in [elevation-arcs.md](elevation-arcs.md)). The current `SectionShell` pattern wraps the `<Outlet />` in `<AnimatePresence mode="popLayout">`, which is structurally incompatible with per-element VT naming because TanStack Router's `<Outlet />` always renders the current route — both old and new m.divs render the destination during a transition, producing duplicate named elements unless `slideKey` is coarsened. We've worked around that twice now (champion list↔detail, match list↔detail). Per [03-motion.md §3 + §6.6](~/.claude/knowledge/frontend-2026/03-motion.md), the 2026-aligned pattern for route-level transitions is VT directly; AnimatePresence should be reserved for component-level mount/unmount. The migration: replace SectionShell's AnimatePresence wrap with `router.defaultViewTransition: true` (or per-Link `viewTransition: { types: [...] }`) and define the slide direction via CSS keyframes on `::view-transition-old/new` keyed by transition types. Unblocks per-element VT morphs structurally and aligns with the KB's recommended pattern. Higher risk — touches every section navigation — but ships a more robust foundation than continuing to coarsen slideKey for every new list↔detail surface.

2. **Multi-element morph refinement** (deferred from Chunk 3 above). Requires the `ChampionCardChrome` restructure described in the Stage 2 attempt write-up. Worth bundling with the SectionShell migration above since both touch the same visual surface (cards) and the same VT mechanism — fixing them together gives one focused review pass.

3. **Steam library ↔ game detail morph** (was Chunk 5). Steam library at `/steam/library` and game detail at `/steam/game/$appid` are *already shipped* — applying the same single-element pattern is a straightforward port: `view-transition-name: steam-game-${appId}` on `library-tile.tsx` (via ref in onClick) and on the game-detail hero. Steam uses the same `SectionShell` wrapper as LoL, so the same `slideKey` coarsening trick applies — extend the `isInChampionsSubtree` / `isInMatchesSubtree` pattern to add `isInSteamLibrarySubtree` (or coarsen at the `steam.tsx` layer if Steam has its own slideKey derivation). Worth doing before #1 above as the third reuse of the pattern, because it also informs whether the SectionShell migration needs to handle a third section's quirks.

4. **Visual polish on the shipped morphs.** Two deferred items: (a) add per-morph CSS defaults to `view-transitions.css` (320 ms / iOS-spring easing / `mix-blend-mode: normal` overrides) — was omitted from the shipped attempts to eliminate global-CSS as a regression suspect. Add back only after the SectionShell migration so it's tested against the right architecture. (b) Evaluate whether the 700 ms `MORPH_SETTLE_MS` body-hold in `$championKey.tsx` is still needed on VT browsers (the rect-morph fallback still needs it; VT may not).

5. **Telemetry / observation period.** Before Chunk 6 (removing the rect-morph fallback for VT-covered flows), get at least a quarter of stable VT usage observed. The fallback also covers reduced-motion users and any browser still lacking VT support, so do not delete prematurely.
