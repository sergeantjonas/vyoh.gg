...
3. **Steam library ↔ game detail morph** (was Chunk 5). Apply the shipped single-element pattern (`view-transition-name: steam-game-${appId}`) when the Steam detail route lands. Naming convention is documented in `view-transition-nav.ts`.
