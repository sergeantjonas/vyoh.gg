...
- **The detail routes** (`matches/$matchId`, `champions/$championKey`, `library/$appid`) reach the server render as of 2026-09-03. `SlidePanel` renders its content in place before hydration and portals it after, via `PanelLayer` in [slide-panel.tsx](../../../apps/web/src/_shared/slide-panel.tsx). Radix's `Dialog.Portal` emitting nothing server-side was confirmed against `renderToString` rather than inferred, and all six panel URLs were then checked in headless Chromium against the **dev server** for a discarded server tree, with six untouched routes as controls. That is weaker than the containerised probe § "The production image is a different environment" asks for — a first pass caught nothing but stale Vite dep 504s — so re-run it against the container before the next detail route lands.
