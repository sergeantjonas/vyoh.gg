...
- **The detail routes** (`matches/$matchId`, `champions/$championKey`, `library/$appid`) reach the server render as of 2026-09-03. `SlidePanel` renders its content in place before hydration and portals it after, via `PanelLayer` in [slide-panel.tsx](../../../apps/web/src/_shared/slide-panel.tsx). Radix's `Dialog.Portal` emitting nothing server-side was confirmed against `renderToString` rather than inferred, and all six panel URLs were then checked in headless Chromium against the **dev server** for a discarded server tree, with six untouched routes as controls. That was weaker than the containerised probe § "The production image is a different environment" asks for, so **the container probe ran 2026-09-06**: the production web image (built from `apps/web/Dockerfile`, run on the host network against the dev api) served all three panel routes with their bodies (1,941 / 546 / 3,765 characters of `<main>` text) and the dock fallback (128px LoL, 104px Steam), zero hydration errors, zero responses ≥ 400, and the palette opened on ⌃K on all nine probed routes — the positive check that the bundle ran. The six controls were clean too except `/steam`, which throws React #418 in the container and reproduces on the dev server: `SteamIdentityHero` renders its skeleton on the server and the hero art on the client. Pre-existing, and closed the same day — see the last paragraph of this entry.

  The "~66 characters" this note previously recorded was wrong, and measuring first is what caught it: the parent layouts already rendered, so the three URLs were serving 111–1451 characters before any of this. The markup change alone moved them 16–149 characters, because a panel with no data renders a skeleton. **What made the difference was the data**, and on `matches/$matchId` it was not the loader that blocked it — it was `bodyReady`, seeded from `supportsViewTransitions()`, which answers `false` on every server render and gated the body behind a morph-settle that a cold arrival has nothing to wait for. Fixing that one seed took the route to 2304 characters *and* cleared the hydration mismatch, which had the same cause: Chrome and Safari answered `true` on the render that hydrates.

  Two latent mismatches surfaced in the same subtree once it started rendering, both invisible while it never did: a `viewTransitionName` computed from a render-body capability read in [match-hero.tsx](../../../apps/web/src/lol/matches/match-hero.tsx), and a Motion `initial` transform that `renderToString` does not emit. `useViewTransitionsSupported()` in [view-transition-nav.ts](../../../apps/web/src/lib/view-transition-nav.ts) is the render-safe reader for the first; it answers `false` for the server and hydrating renders while a component mounted by a later click still reads the true value synchronously, so the rect-morph entrance keeps working.

  **`library/$appid` — done 2026-09-05.** The body gates on the game's library row, which the panel read from the 664 kB owned-games list the priming rule keeps client-side, so no amount of per-query priming could make it render: the row got its own `GET /steam/game/:appid` (3.9 kB / 14 ms, one snapshot row through the same projection as the list, 404 for an unowned or hidden appid). The loader primes it on the server only, fatal on an outage but tolerating a 404 — the server primes the public projection, so the owner's hidden games are exactly the rows that 404 there, and the miss travels out as loader data so the hydrating render agrees with the server (a failed query is not dehydrated) — with the description (7.7 kB) and achievement list (7 kB) tolerated beside it; screenshots and the unlock timeline stay client-side as media and chart geometry. The component reads the list's row first and the single row second, so a click from the library never fires the second request. Measured by stripping tags from `<main>` in the served HTML: 124 → **3387** characters; headless Chromium against the dev server reported no hydration error and the dialog left `<main>` after hydrating (the dev server again, not the container; the container probe followed on 2026-09-06, see the detail-routes entry above).

  Priming the description found a third way to lose the server tree: the about block ran `DOMParser` inside a render-time `useMemo`, unreachable on the server only while the description was never in its cache. Once it was, the render threw, React switched the whole route subtree to client rendering, and the document served the pending spinner under a 200 — no mismatch, no console error, just 111 characters. The post-processing is now a string pass over the sanitizer's already-normalised output. The two verdict cards built on a relative timestamp (`TimeTo100Card`, `LastProgressedCard`) are gated on `useHydrated()` for the reason § "A server/client branch during render" gives.

  **`champions/$championKey` — done 2026-09-05.** The body is derived from `useCachedMatchesWindow(account, 2000)` — 361 kB, of which the main champion alone is 182 kB — so no per-query priming could make it render without breaking the size rule. The api now computes the same `detail` and `overall` aggregates over the same window and queue filter and returns them with the champion extras (2.8 → 12.6 kB, 12 ms): `computeChampionDetail`, `buildWinRateSeries` and `computeTrendSummary` moved to `packages/shared` with the `ChampionStats`/`RolePosition` types they depend on and a shared `CHAMPION_DETAIL_WINDOW`, so both sides run one function — and that function now calls `excludeRemakes()` like every other aggregate, which it had not on the web, so `detail.games` and the delta tiles no longer count remakes that `overall` already left out. The loader primes the extras on the server only, fatal — a champion with no matches still answers with `detail: null` and the empty state. The page prefers its own window when it holds one (the champion table shares it, so a click never waits) and reads the extras otherwise; the window-driven heatmaps and tilt indicator render nothing until the window lands, since a server render would otherwise emit their "0 games" empty states into the document. Measured the same way as the game panel: 200 → **648** characters; headless Chromium against the dev server reported no hydration error for a 193-game champion and a one-game champion, and the dialog left `<main>` after hydrating.
  ...
  **Closed 2026-09-06:** the panel used to dock at `top: 0` for the pre-hydration frame, because `--account-header-h` is only ever written from JS; measured 128px on LoL and 104px on Steam, so one hardcoded fallback could not serve both. Each section root now declares its height through `SectionShell`'s `headerDockPx`, the panel carries it inline as `--account-header-h-fallback` (on the panel rather than the shell root so it survives the post-hydration portal), and the `top` reads the measured var first, the declared one second, 0 outside any section. The container probe from § "The production image is a different environment" ran the same day for all three detail routes and came back clean; the result is recorded above.

  **The same probe found `/steam` discarding its tree, closed 2026-09-06.** A third instance of the root-warmed-query class from § "What 5 turned out to be": `SteamIdentityHero` picks its backdrop from the live game when player state says the owner is in one, and `PresenceMounts` polls player state from the non-code-split root, so on a cold arrival the answer landed before the route chunk hydrated. With the owned-games list still absent, `isSteamGameAppType(null)` reads the live app as a real game, so the hydrating render carried the hero art the server render never had — and the hero URL in the diff, `/hero/noflip/2638890/0.webp`, was the live branch with no asset stamp. It reproduced in 4 of 14 probe runs, exactly the runs while the owner was playing, and a Playwright probe that fulfils `/steam/player-state` with a live game on the client reproduced it 4 of 5. Gated, not primed: the subject also depends on the 664 kB owned-games list, so no loader can make the two renders agree, and the hero's server output was already the skeleton. `useHydratedSync()` withholds all three query results from the hydrating render; the same probe now passes 5 of 5 against the dev server and the container, and the hero's test renders it to a string with a live game in every mock and asserts the skeleton.
