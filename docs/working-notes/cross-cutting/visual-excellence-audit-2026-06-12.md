...
**Cross-engine fix (2026-06-13):** the initial swap set only `outline-color` on `*`, which was insufficient — owner reported a grey/white ring. A Playwright Chromium+Firefox probe (tab-through, read computed `outline*` on each `:focus-visible` element) pinned two causes: (1) the browser's native focus ring is `outline-style: auto`, which paints a UA grey ring and *ignores* a custom `outline-color`, so bare elements (links/buttons with no own focus utility) never showed the theme hue; (2) **in Firefox the universal `outline-color` lost to `currentColor` on some elements** (wordmark, back-home link), drawing a *white* ring (their text color) — this was the owner's "grey," since the owner runs Firefox. Both fixed by writing the full themed outline directly on the focused element in `@layer base`: `*:focus-visible { outline: 2px solid var(--theme-ring); outline-offset: 2px; }` — color included so it can't fall back to currentColor. Kept in `base` so component `focus-visible:outline-*` utilities (the `outline-none` opt-outs, bespoke widths) still win via the utilities layer. Re-probe confirmed every focused element resolves `outline-color` to `--theme-ring` in both engines (red `oklch(0.62 0.2 25 / 0.7)` under a forced red theme; default-blue on unclaimed routes). **Verification method worth reusing:** headless splash doesn't load, so `--theme-color` stays the default blue in-probe — force `--theme-color` via `documentElement.style` to preview the themed ring.

**Second focus-ring system (2026-06-13):** after the outline fix the owner noticed the *nav* still drew a grey ring. The probe revealed the app has **two** focus-ring mechanisms and V6 had only themed one. (a) `outline`-based (bare elements) — themed above. (b) **`ring-*` box-shadow** — used by ~25 shadcn/Radix primitives (`Button`, `Input`, `Select`, `Checkbox`, `navigation-menu`, `accordion`, `sparkline`, `slide-panel`, many Steam surfaces). These set `focus-visible:outline-none` (suppressing the themed outline) and draw their own ring via `focus-visible:ring-ring/50` — the **neutral grey `--ring` token** (`oklch(0.556 0 0)`), which never joined the theme cascade. Confirmed all `ring-ring` usages are focus-only and `--ring` is a pure focus token (also feeds the 3 `border-ring` focus borders), so unifying it is safe. Fix: made `--ring` theme-derived in both `:root` and `.dark` — `--ring: oklch(from var(--theme-color) l c h)` (solid, no baked alpha, so the `/50` call-site modifiers apply cleanly). One 2-line token change themes all ~25 box-shadow focus surfaces + the focus borders at once. Re-probe (Steam nav link, both engines): box-shadow went from grey `oklab(0.556 0 0 / 0.5)` → red `oklab(0.62 0.181 0.084 / 0.5)` under a forced red theme. Both focus systems now follow the route hue. V6 closed.
