...
- **Chunk 5 — broader sweep.** Partially landed alongside [data-viz-densification.md](../archive/data-viz-densification.md) Part 1 (2026-05-27): the new `Sparkline` primitive defaults `stroke="var(--theme-strong)"`, and per-surface overrides pick role-appropriate tints (emerald/red on win-rate / gold-lead, tier color on LP delta, foreground/60 on Steam playtime). Recharts sparkline strokes therefore now belong to the theme cascade in spirit, even though they were implemented as inline SVG rather than Recharts.
  ...
  **Third wave landed 2026-06-19** — Recharts **chart series** joined the cascade. The `CHART_SERIES` slot in [chart-palette.ts](../../../apps/web/src/lib/chart-palette.ts) was pointing at `var(--accent, #34d399)`, but `--accent` is shadcn-reserved and **unset on every LoL route** (the splash backdrop is a separate mechanism), so it resolved to the neutral grey token — the emerald fallback was dead code and the damage-profile radar's hero polygon rendered dark grey. Re-pointed `CHART_SERIES` at `var(--theme-fg, #34d399)` (the contrast-locked tier — legible for any subject hue), retiring a short-lived `CHART_SERIES_STATIC` author-time-emerald stopgap. Both consumers (`trend-kda` line, `DamageProfileRadar` polygon) now theme per subject — champion hue on champion/match detail, signature-champion hue on the profile (via the splash `useThemeColor`). A 2026-06-19 audit then swept the viz layer for stray hardcoded series colours: the only one left was [match-review-view.tsx](../../../apps/web/src/lol/matches/match-review-view.tsx)'s gold-diff `LinePath` (`stroke="#60a5fa"` — the canonical "hardcoded sky-blue" this chunk was named for), re-pointed at `CHART_SERIES`. (Its companion semantic colours — the kill/death/objective event dots, and the green/red zero-split on the dedicated `match-gold-lead` chart — were deliberately left; gold-diff is rendered semantic there, as a trajectory backdrop here.)

  **Per-site judgement — resolved by the 2026-06-19 audit:** scrollbar thumb stays neutral grey (`color-mix(var(--foreground) 18%)`, [index.css](../../../apps/web/src/index.css)) — colour-flipping on every nav reads as noise. Focus ring is **already themed** — `--ring: oklch(from var(--theme-color) l c h)` shipped in the 2026-05-26 wave (the "needs scoping" concern was moot; it re-tints from the default blue and doesn't over-extend). No tile hover sheen exists (cards rely on frosted chrome); the only sheen is the intentionally-white tab glint (`.theme-bar-glint`). **Chunk 5 is effectively closed** — the cascade reaches charts, sparklines, tab/progress chrome, nav/orb, focus rings, and meta-color; what's left neutral is neutral by decision.
