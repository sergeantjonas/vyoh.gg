...
- [`apps/web/src/router.tsx`](../../../apps/web/src/router.tsx) (was `main.tsx` until the Start migration, 2026-07-26) — `defaultViewTransition.types` callback routes every navigation through [`getNavigationType()`](../../../apps/web/src/lib/navigation-type.ts), emitting one of `slide-left`, `slide-right`, `intra-section`, `cross-section`, `account-swap`, or `false`. Also sets `body[data-vt-shell]` BEFORE `startViewTransition` so the OLD snapshot already reflects the gate, and resets `mainScrollRef.scrollTop = 0` for slide types before the OLD capture so the slide is purely horizontal instead of diagonal.
