...
- **Title-as-link, not bottom-CTA.** The chapter's masthead (champion name + title, or Steam logo + tagline) IS the entry point to the deep-stats / game-detail page — wrap it in a `<Link>` with a group-hover-revealed `→` indicator (mirrors the standout block's "open →" affordance). Replaces the prior bottom-band CTA button that read as a UI module pasted onto the editorial spread, AND frees the bottom of the pin so the `NextChapterCaret` (or future "back to top" caret state) doesn't collide with a closer button. Ahri removes its `ChapterCloser` band entirely; Steam keeps the closer (screenshots strip) but drops the CTA. **When adding a new chapter type, default to title-as-link** — only add a bottom CTA if the masthead can't reasonably carry the destination (rare).
