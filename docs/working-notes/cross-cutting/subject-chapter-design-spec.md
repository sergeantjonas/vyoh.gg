...
- **Pin padding starts at `pt-[6dvh]` and tightens with chapter density.** Default `pt-[6dvh]` (R-3 feedback round drop from 10dvh — 10 felt generous up top). Denser chapters tighten further: Steam runs `pt-[3dvh]` because its bottom band (screenshots + CTA) needs every available pixel inside the 1-viewport pin once the standout description + recent unlocks strip are factored in. Ahri runs `pt-[6dvh]` because its lighter footprint allows the breathing room. **Rule of thumb:** when adding a new content line that pushes the bottom band visibly toward the viewport edge, tighten `pt-[...dvh]` before reaching for `pinViewports={2}` (the 2-viewport pin felt like dead air in the R-2 build and was rejected).
