...
| `useChapterNudge` | Polite one-shot IntersectionObserver nudge — smooth-scrolls the chapter top into viewport-top alignment when the user is committedly entering, then flips `nudged` after a settle window. Threshold tuned at 0.35 so it fires *after* the previous chapter is decisively scrolled past (prevents next-chapter bleed). Don't re-implement per-chapter. | `use-chapter-nudge.ts` |
