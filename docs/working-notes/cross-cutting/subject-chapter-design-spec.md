...
- **Pin padding starts at `pt-[6dvh]` and tightens with chapter density.** Default `pt-[6dvh]` (R-3 feedback round drop from 10dvh — 10 felt generous up top). Denser chapters tighten further: Steam runs `pt-[4dvh]` (was 1dvh under the bottom-CTA-era; relaxed after CTA moved to title-as-link freed ~50px). Ahri runs `pt-[6dvh]` because its lighter footprint allows the breathing room. **Rule of thumb:** when adding a new content line that pushes the bottom band visibly toward the viewport edge, tighten `pt-[...dvh]` before reaching for `pinViewports={2}` (the 2-viewport pin felt like dead air in the R-2 build and was rejected). Eyebrow can sit right under the sticky nav (~52px tall) without visual collision — the nav's `backdrop-blur-md` blurs the splash behind it, so eyebrow text passing under the nav's bottom edge reads cleanly.
- **Title-as-link, not bottom-CTA.** The chapter's masthead (champion name + title, or Steam logo + tagline) IS the entry point to the deep-stats / game-detail page — wrap it in a `<Link>` with `cursor-pointer` + a subtle `hover:opacity-95` so the affordance is felt without painting an explicit `→` indicator (a hover-revealed `→` was tried but read as dead space, especially on flex-wrap rows where it landed on its own line). Replaces the prior bottom-band CTA button that read as a UI module pasted onto the editorial spread, AND frees the bottom of the pin so the `NextChapterCaret` (or future "back to top" caret state) doesn't collide with a closer button. Ahri removes its `ChapterCloser` band entirely; Steam keeps the closer (screenshots strip) but drops the CTA. **When adding a new chapter type, default to title-as-link** — only add a bottom CTA if the masthead can't reasonably carry the destination (rare).
- **Don't strip the chapter band internal padding.** Opener `pt-12 pb-6`, closer `pt-6 pb-12`, detail/stats `py-4` — these aren't just whitespace, they're the editorial breathing room that makes a chapter read as a magazine spread instead of a packed dashboard. Tried zeroing them out in an R-3 round to reclaim viewport-bottom space for the caret; the result felt drastically worse. If a chapter is genuinely too tall, the lever is either tightening the pin's `pt-[Ndvh]` or trimming content, not collapsing the bands.
