...
- **Descriptive `aria-label` on the 7 Recharts charts** — `recharts@3.8.1` enables `accessibilityLayer` by default (keyboard + per-point aria already work), but the charts have no *name*: an SR announces an unlabelled interactive region. Add a one-line `aria-label` (or `role="img"` + `aria-label` on a wrapper) naming what each chart shows — match gold-lead, LP history, KDA trend, etc. The custom SVG charts already do this; only the Recharts surfaces are missing it. From the 2026-06-14 data-viz audit (Round 9, Gap 34 / bundle BC). Does **not** close the table-fallback gap (#35) — that needs the shared primitive.
