...
- **Champion-detail `DeltaTile` renders unsigned negatives** — [`$championKey.tsx:99,422,427`](../../../apps/web/src/routes/lol/$accountSlug/champions/$championKey.tsx#L99) formats both KDA delta and WR delta via `Math.abs(v)` with only a `"+"` prefix branch for positives; color is the sole negative signal, which reads ambiguously (e.g. a Darius row at 0% WR shows the account-average delta as a red `56%` with no `−`). Unique outlier — `match-hero.tsx:29`, `champion-build-path.tsx:157`, `champion-patch-history.tsx:45` all prepend `+` or `−` explicitly. Fix: either consume `formatLpDelta`'s signed pattern (drop `Math.abs`, prepend `+` for positives, let native `-` carry negatives) or add a small `formatPpDelta` helper next to it. Pairs with the pp-delta consolidation above.
