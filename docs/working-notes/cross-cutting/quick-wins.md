...
## Steam library-card chips (from GetItems harvest 2026-05-24)

All five rows below come from the [steam/library-card-enrichment.md](../steam/library-card-enrichment.md) field harvest. Each is one commit after the umbrella Chunk 0 (schema-flags refactor) lands. Pick in any order.

- **Steam Deck compat chip on game tile + detail header** — render `steamDeckCompat` from enrichment (`3` Verified / `2` Playable / `1` Unsupported, skip `0`). Mirrors Steam's own Deck badge visual language. Filed as [library-card-enrichment.md Chunk 2](../steam/library-card-enrichment.md). One commit (Prisma column + projection + chip component).
- **Platform pills (Windows / Mac / Linux / VR) on game detail** — small OS-icon row distinguishing "I haven't played on Mac" (already in `playtime_mac_forever`) from "game doesn't support Mac" (new). Filed as [library-card-enrichment.md Chunk 3](../steam/library-card-enrichment.md).
- **Review summary chip ("Very Positive · 56k") on game detail** — Steam-native vocabulary makes the game-detail page feel storefront-grade instead of stub-grade. Color scale = Steam's own (Overwhelmingly Positive → emerald, Very Positive → green, …, Mixed → amber). Filed as [library-card-enrichment.md Chunk 4](../steam/library-card-enrichment.md).
- **ESRB / PEGI rating chip on game detail** — descriptor list collapses into `+N descriptors` Popover. **Not** an NSFW signal (Subverse returns `game_rating: null` because AO-rated games skip ESRB submission); purely editorial chrome. Filed as [library-card-enrichment.md Chunk 5](../steam/library-card-enrichment.md).
- **Short description as library-tile + game-detail subtitle** — `basic_info.short_description` is a single sentence per game (~150-300 chars). Highest density-of-information win in the harvest. Filed as [library-card-enrichment.md Chunk 1](../steam/library-card-enrichment.md).
