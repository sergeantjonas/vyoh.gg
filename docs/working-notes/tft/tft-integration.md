...
Sibling docs: [self-portrait-surfaces.md](../cross-cutting/self-portrait-surfaces.md) (parent reframe — TFT panels are part of the self-portrait engine), [steam-integration.md](../steam/steam-integration.md) (mirror structure, more committed cadence).
...
- **Backfill story.** TFT match-v5 retention mirrors LoL — likely the same 2-year horizon. Reuse the historical-backfill SSE pattern from [riot-investigation-2026-05-07.md](../lol/riot-investigation-2026-05-07.md).
- **Asset pipeline.** Champions, traits, items, augments are versioned per set. Mirror the build-time-champion-assets pattern; do not hotlink Riot's CDN. (Caveat: if the [lol-image-pipeline.md](../lol/lol-image-pipeline.md) runtime-proxy pivot has shipped by then, TFT lands on the proxy directly — no bundled assets.)
- **Cross-domain `apps/web/src/_assets/` hoist** ([folder-structure-cleanup.md](../ops/folder-structure-cleanup.md) Chunk 4). TFT becoming the third asset-handling domain is the trigger to lift the asset-manifest pattern out of `lol/_shared/` and `steam/_shared/` into a top-level location. Skip if the runtime-proxy pivot has already retired bundled assets — the hoist only earns its keep against the bundled pattern.
- **Set boundaries as patch-equivalents.** TFT sets are coarser than LoL patches but the same "shade chart background at set boundaries" idiom applies — see [vnext-ideas.md](../cross-cutting/vnext-ideas.md) "Patch-aware everything."

---

## Cross-references

- [self-portrait-surfaces.md](../cross-cutting/self-portrait-surfaces.md) — the parent reframe.
- [steam-integration.md](../steam/steam-integration.md) — sibling integration roadmap; closer to active.
- [riot-investigation-2026-05-07.md](../lol/riot-investigation-2026-05-07.md) — Riot rate-limiter and backfill patterns to reuse.
