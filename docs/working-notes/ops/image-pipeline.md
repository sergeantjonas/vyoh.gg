...
**Status:** Active — **scoped 2026-09-16; chunks 1, 2 and 3 all shipped the same day.** Chunk 1 is verified green (run `35148907656`, both images published and anonymously pullable) and chunk 2 is verified locally against those published images; both are **proven on the box 2026-09-17: the first real deploy pulled `sha-9f02168` for both images and brought the stack up with `--no-build`, and a second deploy the same day rolled the api forward on the auth fix**. Chunk 4 is optional and post-launch. Replaces the on-box build in `scripts/deploy.sh` with images built by GitHub Actions and pushed to GHCR; the box only pulls. This is what makes the 8 GB box in [hosting.md](hosting.md) valid — the sizing argument for 16 GB rested entirely on `pnpm install` plus the Vite SSR build peaking at several GB next to a live stack, and that spike moves to a GitHub runner. Four chunks; the first two are the pipeline, the third is the documentation debt they create, the fourth is optional. Runbook step 5 is now a pull rather than a build, which is what the 8 GB box rests on.
...
- **Rollback becomes a tag.** Today a bad deploy means re-rsyncing an older tree and rebuilding under incident pressure. With a registry it is `VYOH_IMAGE_TAG=sha-<old> scripts/deploy.sh`. Note the limit: that rolls the *code* back, not the schema. The api entrypoint has already run `migrate deploy` by then, and Prisma has no down-migrations here, so a rollback across a destructive migration is a restore from § 6, not a tag.
...
- **Public images, so every build arg is public.** `docker history` on a pulled image shows them, and the images are world-readable. That is fine for `VITE_API_URL`, `VITE_SITE_URL` and the browser Sentry DSN, which are already in the served bundle — but it means **a credential must never be a build arg**. When E3's sourcemap upload lands ([error-tracking.md](error-tracking.md)), `SENTRY_AUTH_TOKEN` takes a `RUN --mount=type=secret` with `secrets:` on `build-push-action`, or the upload moves to a workflow step after the build so the token never reaches a layer at all. The box pulls anonymously — no registry credential on the box. **Confirmed on the first run**: both packages came out publicly pullable with no manual step, verified by an anonymous manifest fetch and a `docker pull` under an empty `DOCKER_CONFIG`. A claim to the contrary was written here while scoping chunk 1, from the docs line that a first publish is private — that describes personal-scope publishing, not a package the `images` job creates in a public repo's namespace. If a future package ever does come out private, flipping it is a one-time settings change, not a pipeline problem. Everything baked into the web image (`VITE_API_URL`, `VITE_SITE_URL`, later the browser Sentry DSN) is already public in the served bundle.
...
- Two `docker/build-push-action` steps, `context: .`, `platforms: linux/amd64`, `cache-from/cache-to: type=gha,scope=<image>`. The web step passes `VITE_API_URL`, `VITE_SITE_URL`, `VITE_SENTRY_DSN` and `VITE_SENTRY_SAMPLE_RATE` from `vars.*` and `BUILD_COMMIT` from the sha. The api step passes nothing — `BUILD_COMMIT` is a runtime env for the api, read from compose.
...
## Chunk 2 — compose pulls, deploy.sh stops building · shipped 2026-09-16
...
Two things the scope above did not settle, decided while writing it:

- **`BUILD_COMMIT` is derived from the tag, not from local `HEAD`.** `build_commit="${tag#sha-}"`. Deploying an older tag while stamping its Sentry events and its status page with today's checkout is how a build identifier starts lying at exactly the moment someone is reading it during an incident.
- **The narrowed rsync drops `--delete` for the loose files and keeps it for `deploy/`.** At the top level `--delete` would now mean "remove everything else in `/srv/vyoh`", and that is where `.env` lives. `deploy/` is a directory we own end to end, so it keeps `--delete` and a retired nginx conf cannot linger. `deploy/nginx/README.md` needed no change: its install steps copy from `deploy/nginx/*`, which still ships.

Verified locally against the **published** images rather than a local build, which is the stronger probe: `compose config` resolves `:main` with no tag set and `:sha-90e43de` with one; `compose pull` + `up -d --no-build --wait` brought the real stack up from GHCR, migrations and all, and all three of `deploy.sh`'s smoke targets answered 200. Both refusal paths were exercised on a never-built tag — `docker manifest inspect` exits 1, so `deploy.sh` stops before touching the box, and `--no-build` errors rather than falling back to building. `shellcheck` is clean apart from SC2029, which is the intended client-side expansion the script already used.

The real proof is still runbook step 5 on the box.

**Found while probing, not fixed here** (pre-existing, and it fails the same way with or without this chunk): `compose.prod.yaml` declares `STEAM_API_KEY: ${STEAM_API_KEY:-}`, but `steam-client.service.ts` calls `requireEnv("STEAM_API_KEY")` at construction, so the api crash-loops on first deploy rather than degrading. Same shape as the owner-auth env gap that [pre-launch-sweep.md](pre-launch-sweep.md) records — an optional-looking compose var the code requires unconditionally. → [open-work.md](../open-work.md)
...
- **Old images are never reclaimed.** `deploy.sh` still runs `docker image prune -f`, but with `sha-` tags nothing is ever dangling, so the box accumulates one image pair per deploy. That is the trade rather than an oversight — a rollback to a recent tag then needs no network at all — but it is disk growth nobody is watching, and chunk 4's retention item is where it gets solved if it matters.
- **A bare `docker compose up -d` on the box moves the stack to `:main`.** `${VYOH_IMAGE_TAG:-main}` is what makes a no-argument `compose pull` do something sensible, and the same default is what reverts a sha-pinned stack the moment anyone runs `up` without the variable — verified 2026-09-16. `deploy.sh` always passes it, so this only bites a hand-run command, which is precisely the incident case. Recorded where someone would be standing when it matters: [hosting.md § Restoring prod after an incident](hosting.md#restoring-prod-after-an-incident).
- **GHCR is a third party in the deploy path.** If it is down, deploy is blocked but the running stack is unaffected, and an incident restore still works because it reuses the containers already on the box; the `build:` block kept in compose is the manual escape hatch, at the cost of the RAM spike this note exists to avoid. Acceptable for a personal box.
