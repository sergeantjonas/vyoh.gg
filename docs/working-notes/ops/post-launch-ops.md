...
- **A missing asset is a 404 with `max-age=3600`**, matching nginx's `proxy_cache_valid 404 1h`, so an asset published after the first request appears within the hour. The resolver misses (`ability`, `map`, both achievement icons, and unknown rune and spell ids) answer the same way; they were 404s under the year-long header. Each resolver throws `NotFoundException` for a miss, and only that is caught, so a database failure behind one reaches the filter as a reported 5xx instead of passing for a missing icon.
