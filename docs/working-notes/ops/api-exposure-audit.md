...
**Fixed** by moving the check **inside `resolveSummoner` itself** rather than adding a route guard, which the original note proposed. The choke point turned out to be the better boundary for three reasons: it is the only path to Account-V1 *and* the only writer of a `Summoner` row, so nothing can reach either without passing it; it fixes both offending services without touching their constructors, neither of which injects `IdentityService`; and the cron path is unaffected because `syncAccountMatches` passes accounts read from the same roster the allowlist is built from. A route guard would have covered the three known routes while leaving the next direct caller exposed.
...
`SteamClientService.fetchJson` builds every request path with the key as a query parameter, because that is Valve's Web API contract and there is no header alternative: `?key=${encodeURIComponent(this.apiKey)}` ([steam-client.service.ts:56](../../../apps/api/src/steam/client/steam-client.service.ts#L56), and 11 further call sites).

The success-path log strips it correctly — `path.split("?")[0]` at [steam-client.service.ts:330](../../../apps/api/src/steam/client/steam-client.service.ts#L330). **The error path directly above it does not:** [steam-client.service.ts:316](../../../apps/api/src/steam/client/steam-client.service.ts#L316) logs the whole `path`, key included, on any timeout, DNS failure, abort, or connection reset. The same unredacted `path` is then stored as a public field on every `SteamClientError` thrown at [:321](../../../apps/api/src/steam/client/steam-client.service.ts#L321) and [:336](../../../apps/api/src/steam/client/steam-client.service.ts#L336).
...
- ~~**Session tokens are planned to be stored raw.**~~ — **closed 2026-08-13: shipped hashed.** `Session` keys on `tokenHash` (`apps/api/prisma/schema.prisma`) and `auth.service.ts` stores a SHA-256 of the cookie, so the recommendation below was taken before the table first existed. Retained for the reasoning.

  Original finding: The `Session` model uses the cookie value itself as the primary key ([owner-auth.md:42-50](owner-auth.md)), so the table holds directly usable tokens. Anything that can read it — a backup, a stray Prisma Studio session, a query log — yields working sessions rather than just evidence that sessions existed. Persist a SHA-256 of the token and look up by hashing the incoming cookie. The table does not exist yet, so this costs nothing now and a migration later.
- ~~**"Opaque random" does not pin a generation mechanism.**~~ — **closed 2026-08-13:** `auth.service.ts` generates the token as `randomBytes(32).toString("base64url")`, which is the call this asked for. Name the call explicitly in the note before chunk 1 (`crypto.randomBytes(32).toString("base64url")` or equivalent CSPRNG), so the implementation session cannot reach for `Math.random()` or a UUID.
...
1. **The ordering is structural, not incidental.** `ImminentHero` is mounted with an appid taken from the wishlist response (`upcoming-panel.tsx`), so the web cannot ask about an appid the server's own list does not contain. (As of 2026-08-11 the panel reads `/steam/upcoming`, whose set is wishlist ∪ owned-unreleased — the guard was widened to match on 2026-08-12, below.)
...
**Widened 2026-08-12, and the gate got cheaper doing it.** The wishlist stopped being the whole set the moment the panel started reading `/steam/upcoming`: buying a game deletes its wishlist row, so from that point the guard was refusing the one appid the hero most wanted to ask about — a pre-order 404'd its own metadata. `SteamUpcomingService.membershipOf()` now answers the question the set actually poses, returning the provenance rather than a boolean: the library arm first (an enrichment row flagged coming-soon, narrowed to a non-removed library row), the wishlist arm second. Still a closed owner-controlled set, so the property that makes this a real clamp is unchanged, and the library arm reads our own DB — a pre-order resolves through a Steam outage that would take the wishlist arm down. The provenance is what removes the spend: an owned title *has* an enrichment row, carrying every field this endpoint returns including the accent the anchor pass already extracted from the same art, so the store call, both image fetches and the Vibrant pass are skipped entirely for the case that was 404ing before. Only a wishlisted title — unowned by definition, no row — still pays the projection.

**Measured live 2026-08-12**, and the two paths are two orders of magnitude apart. The pre-order (`2584270`) went from 404 to **200 in 11 ms**, carrying a real accent, blurb and platform flags off its row. A wishlisted title on the same list (`3949550`) answers **200 in 990 ms** cold — the store call, the art fetch and the Vibrant pass, which is exactly what the row path skips. The refusals are unchanged and still land before any spend: an owned-but-released appid (`730`) 404s in 2 ms and a nonsense one (`999999999`) in 1.6 ms.
...
This is the finding that ties F-20, F-21 and `/steam/summary` together, and it is the reason they rank above their individual costs. [steam/client/rate-limiter.service.ts](../../../apps/api/src/steam/client/rate-limiter.service.ts) runs **a single Bottleneck instance for the entire Steam Web API surface** — 100,000 calls per 24 h, 5 req/s, 4 concurrent. The `family` labels (`store-items`, `appdetails`, `player-summaries`) are log tags, not separate reservoirs.
