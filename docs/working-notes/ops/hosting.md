...
2. Dump dev: `VYOH_COMPOSE_FILE=compose.yaml VYOH_BACKUP_DIR=~/dumps scripts/backup.sh`
   — both vars matter: the first would otherwise dump the *prod* stack, the
   second would write to `/var/backups/vyoh` and need root. On macOS the run
   ends with `mapfile: command not found` and a non-zero exit, because the
   retention pass at `backup.sh:93` uses a bash 4 builtin and `/bin/bash` is
   3.2. The archive is already finalised and verified by then, so the dump is
   good — but never chain it as `backup.sh && scp`, and note this is a laptop-only
   failure: the systemd timer runs on Debian with bash 5.
3. `scp` it to the box.
