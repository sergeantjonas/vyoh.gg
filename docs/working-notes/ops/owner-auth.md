...
**Status:** Active — **chunks 1 and 2 both shipped 2026-08-13. The launch gate is closed:** every mutating route answers 401 without an owner session, and the frontend renders them locked. Only chunk 3 remains. It was recorded as "prod wiring rather than code", and that has now been wrong in two load-bearing ways. The second, found 2026-09-17 on the first real login against the box: GitHub's callback carries an RFC 9207 `iss` parameter the DTO never declared, which `forbidNonWhitelisted` turned into a 400 on every login, and no amount of wiring would have fixed it. The first, found 2026-08-20: `compose.prod.yaml` shipped 2026-07-27, *before* chunk 1 added four unconditional `requireEnv` calls, and was never updated to pass them — so the production api could not have booted at all. Fixed the same day, along with the root `.env.example`, which held none of the api's secrets because in dev they live in `apps/api/.env` and the compose stack never reads that file. The rest of chunk 3 genuinely is wiring: a separate prod OAuth app, and the values themselves. Plan written 2026-05-14; naming option 1 confirmed and built. The companion status-page admin surface landed 2026-09-02: its Steam sync rows are public and read-only, and its granular LoL trigger added the fifth gated route, `POST /status/sync/patches`. See § "Status page admin surface" below.
...
- ~~The callback rejects GitHub's `iss` parameter~~ — fixed 2026-09-17, on the first real login against the box. GitHub sends an RFC 9207 issuer identifier (`iss=https://github.com/login/oauth`) on the authorisation response, `GithubCallbackQueryDto` did not declare it, and the global pipe's `forbidNonWhitelisted` therefore answered `400 property iss should not exist` to *every* login. The dev app never saw it — it predates the rollout, and a registration made today gets the current behaviour, so this was only ever going to surface on a fresh OAuth app. Declared and **checked against the issuer** rather than tolerated: an `iss` that is present and different did not come from the server the handshake started with, and is denied with `state` before the code is exchanged. Absent stays accepted, because only the authorisation server decides whether to send it.
- ~~Separate prod GitHub OAuth app (never the dev app's secret) + env values set on the chosen hosting target.~~ — done 2026-09-17. A distinct registration with callback `https://api.vyoh.gg/auth/github/callback`, a fresh `SESSION_SECRET`, and all four values hand-written into `/srv/vyoh/.env`. Verified by a real login against the box, which is also what surfaced the `iss` 400 above — no test could have, since the parameter comes from GitHub.
- ~~Cookie `domain` configured per the hosting-dependent section above.~~ — decided 2026-09-17: left **empty**, which the `.env.example` comment already argued for. Separate subdomains are not the question; whether anything other than the api origin has to *read* the cookie is, and nothing does.
