...
- **Write order matters.** Transition write runs before the upsert; reversing it produces `endedAt < startedAt` by a few milliseconds (Node's clock vs. Postgres `CURRENT_TIMESTAMP`).
...
3. **Session history** — doesn't exist. Steam's API never tells you "I played Helldivers from 19:42 to 21:58 last night." You get `playtime_forever` (cumulative) and `playtime_2weeks` (rolling), and that's it.
...
So `computeTransition` reads the open session from the DB instead. The previous player-state row only contributes the `endedAt` anchor (the last moment we observed me still in the game). An orphan session left behind from a pre-Chunk-3 deploy converges on the very next tick: the open row's appid doesn't match the prior state's appid, so the transition force-closes it with `endedAt = now`.
...
| --- | --- | --- | --- |
| Session-close hook | event-driven | ~5–20 calls/day typical | Primary realtime signal — fires the moment I close a game |
...
If I close Game A and open Game B within a single 2-min window, the next tick sees `prev=A`, `next=B`, and emits one `closeAndOpen`. The "few seconds of nothing" between them collapses. No way to detect it from a 2-min tick, no plan to care.
...
**Offline+rebooted gap.** If the machine reboots mid-session, the open session's `endedAt` anchors to the pre-reboot `lastPolledAt`, which is correct. But if I reboot, play the same game offline for an hour, then come back online — that hour gets logged as "we don't know" (the recently-played backstop catches the unlocks but not the session). Acceptable trade.
...
- Polling-as-signal travels. "The cheapest endpoint is the most useful one" works anywhere a vendor exposes liveness but not history — Slack presence, GitHub status, Twitch streams. Poll a tiny endpoint, treat transitions as events, fire the downstream work and forget.
- The pure transition function + DB-as-truth pair tests cleanly. Eight unit tests on `computeTransition` cover the whole state machine. The integration test in `play-sessions.service.spec.ts` only has to confirm the service applies the right action — correctness is already settled in the pure function's tests.
- 13.6k → ~950 calls/day is real, with the same coverage and faster detection. The polling-cron isn't free; the savings come from collapsing the 4×/hour full-library sweep into event-driven refreshes.
- Two small calls drive most of the resilience: open-session row is truth, and the transition write runs before the upsert. Both look like nothing in a code review and quietly hold the design together.
