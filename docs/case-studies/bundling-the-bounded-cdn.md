...
> **Status (2026-05-16): superseded by [runtime-image-proxy.md](./runtime-image-proxy.md).** The bundled approach shipped and worked correctly for the LoL set throughout its lifetime. It outgrew its problem when a second content stream (Steam) entered with a different cadence — wishlist adds and recently-played titles don't fit "wait for the next refresh PR." The pivoted architecture (NestJS proxy at `/img/*`, Sharp transcoding, browser HTTP cache + Nginx at the deploy layer) replaces both the bundled LoL set and the bundled Steam set with one shared shape. This write-up stays intact as the "first architecture" story because the *reasoning* it captures (Phase 0 timeout as worst-case bound, manifest-as-build-artifact, hash-keyed PR diffing) is still correct for the system as it stood. The pivot story lives in the new case study.
...
**5. The inlined manifest blew the bundle budget — and the fix had to keep synchronous lookups.** Decision 3 in the working note framed the manifest as "just import the JSON, it's already on disk." That was true for correctness — `import "./manifest.json"` ships the data alongside the helpers with no network race — but the JSON had grown to ~250 KB by the time the `size-limit` budget landed in CI, and the next merge failed with a 41.72 kB overage on the main chunk's 200 kB gzipped budget. The shape of the fix was forced by the original decision's good property: the URL helpers are called synchronously from render paths (`<img src={championSquareIconUrl(name)}>`), so an async fetch-on-demand would have meant either a render-time suspense boundary or a top-of-bundle preload that re-introduces the bytes anyway. What actually paid off was noticing how *little* of the manifest the runtime needs: the helpers only ask "does this asset exist, and what are its variants?" — never the file size, hash, or absolute path. The refresh script now emits two artifacts — the full `manifest.json` for its own diffing (and for this case study's "what got bundled" provenance) and a slim `manifest.gen.ts` next to the helpers containing `{ "Aatrox": 7, ... }` (variant bitmask: `1=square, 2=card, 4=backdrop`) plus `Set<string>` for items, runes, summoner spells, and role-icon slugs. Paths are derived from bucket conventions at the call site: `/lol/champions/<alias>/<variant>.webp`, `/lol/items/<id>.webp`, etc. The bundle dropped from 241.72 kB back to 180.14 kB; synchronous lookups intact; the script still has the full manifest to diff against. **Lesson:** the right unit for "what to ship to the client" is *what the runtime asks*, not *what the build produces*. A build artifact and its runtime mirror are different artifacts with different schemas — collapsing them is a convenience that costs the budget on the first growth event.
