...
- **Prime, on the server only**: `/lol/$accountSlug/matches/$matchId` (7.9 kB / 1.6 ms warm). The panel renders in place for a server render, so without the await the document carries the detail in its dehydrated payload and a skeleton in its markup. The client keeps `prefetchQuery`, so this is one loader with two shapes rather than a route that changed its mind — the reasons the client cannot block (an awaited `navigate()` inside `startViewTransition`, a skipped pending skeleton) are click-path only. Measured 2026-09-03: 1451 → 2304 characters of server-rendered text.
- **Prime, on the server only**: `/steam/library/$appid`'s game row (3.9 kB / 14 ms). The panel's body gates on that row, and the 664 kB list it used to come from stays client-side, so the row got its own endpoint rather than the list an exception. The description (7.7 kB) and achievement list (7 kB) prime beside it as tolerated regions; screenshots and the unlock timeline stay client-side as media and chart geometry. Measured 2026-09-05: 124 → 3387 characters.
- **Prime, on the server only**: `/lol/$accountSlug/champions/$championKey`'s champion extras (12.6 kB / 12 ms). The body is derived from the 2000-match window, which is the "do not prime" case two bullets down, so the api computes the same `detail` and `overall` aggregates with the shared functions and returns them with the extras; the page reads them until the window lands. This is the shape for any detail surface whose data is a large list the page aggregates: move the aggregate behind the api rather than the list into the document. Measured 2026-09-05: 200 → 648 characters.
...
- **Prime**: `/lol/$accountSlug/matches` (25 kB / 6 ms), `/steam/achievements` (23 kB / 4 ms), `/steam/wishlist` (7 kB / 1 ms). Each one's list *is* its page, and each reads our own Postgres cache rather than an upstream. Measured 2026-07-27: 62 → 1506, 176 → 1302, 184 → 628 characters. `/steam/upcoming` (2026-08-11) primes on the same grounds: its payload is the wishlist's coming-soon subset plus a handful of owned pre-orders, over the same single upstream call the wishlist route already awaits.
...
| `/steam/wishlist`, `/steam/upcoming`, `/steam/achievements` | fatal | the list is the route, not a region of it |
| `/lol/$accountSlug/matches` | fatal | the list is the route, not a region of it |
| `/lol/$accountSlug/matches/$matchId` | fatal, server only | the panel is the route; the client branch stays non-blocking because `match-row.tsx` awaits `navigate()` inside `startViewTransition` |
| `/steam/portrait` | tolerated | two independent halves; either one still argues without the other |
| `/steam/library/$appid` game row | fatal on an outage, server only; a 404 is tolerated and carried out as loader data | the panel is the route — but the server primes the public projection, so the owner's hidden games are exactly the rows that 404 there, and a fatal 404 would turn the owner's own refresh into an error card. The flag is what lets the hydrating render agree with the server, since a failed query is not dehydrated. The client branch primes nothing because a click from the library already holds the row |
| `/steam/library/$appid` description + achievements | tolerated | regions of a panel whose identity card still says something true without them |
| `/lol/$accountSlug/champions/$championKey` extras | fatal, server only | the aggregates are the body; a champion with no matches still answers (`detail: null`) so the empty state is a real 200, and the client branch primes nothing because the champion table already holds the window |
...
The portal row has a second decision inside it: **what to render for the one render that has no DOM.** `null` is right when the portaled subtree is chrome — [backdrop-portal.tsx](../apps/web/src/_shared/backdrop/backdrop-portal.tsx) has nothing to say to an HTML-only reader. It is wrong when the portaled subtree *is* the page, which is the case for every route mounting a [slide-panel.tsx](../apps/web/src/_shared/slide-panel.tsx): a match, a champion and a Steam game are the app's detail URLs, and a portal-only panel served them a document with no content in it. `PanelLayer` renders the children in place instead and adopts the portal one commit later.

Rendering in place is only sound while **nothing between the element and the document root creates a containing block for `position: fixed`** — a `transform`, `filter`, `backdrop-filter`, `perspective`, `contain`, `container-type: size`/`inline-size`, or a `will-change` naming any of them. That holds for the route outlet today (`<main>` carries only `relative` and overflow, and the `m.div` wrapping it animates `opacity` alone) and it is a property of the current ancestors rather than a guarantee, which is why the portal stays for the steady state rather than being dropped. Keep both halves pinned: that the server render contains the content ([ssr-hydration.test.tsx](../apps/web/src/lib/ssr-hydration.test.tsx)) and that the hydrated render has left its parent ([slide-panel.test.tsx](../apps/web/src/_shared/slide-panel.test.tsx)).

A browser-only API in a render path is the same bug with a different symptom. `DOMParser` in the about block's `useMemo` was unreachable on the server only while the description was never in its cache; the day the loader primed it, the render threw, React switched the whole route subtree to client rendering, and the document served the pending spinner under a 200 — no mismatch and no console error, so only a character count noticed. Anything that parses or measures in render has to work without a DOM, or move behind `useHydrated()`.
...
The live one was [presence-mounts.tsx](../apps/web/src/lib/presence-mounts.tsx), which polls live-game and Steam-player-state from the root so the favicon dot works on every route. Three code-split surfaces read those same query keys: the profile hero's live halo, `/`'s now-playing strip, and the Steam identity hero's backdrop, which switches to the live game's art. Each rendered on the hydrating client render and on no server render, so React discarded the tree for the LoL landing page, for `/`, and for `/steam`. It reproduces **only while the owner is actually playing something**, which is why it survived a full 12-route sweep in chunk 4b, was found in chunk 5 by accident, and hid on `/steam` for six more weeks. A probe for this class has to *force* the live state — fulfil the presence request with a live game in Playwright — rather than wait for the owner to be playing.
...
- **Gate it on `useHydrated()`** when priming cannot make the two sides agree. `/`'s now-playing strip is the example: it advances the clock with `Date.now()`, so the string differs by the time hydration runs whatever you prime, and its Steam branch reads the 664 kB owned-games query the priming rule keeps client-side. Live-presence chrome is not content a crawler wants, so client-only costs nothing. The Steam identity hero gates for the second reason alone: its subject is the live game *or* the most-played owned title, so priming player state would settle one branch and leave the other to the same race. Use `useHydratedSync()` there rather than `useHydrated()` — it is the tree's shape that changes, and a client-side navigation should render the data on its first pass.
...
**How to apply:** when a change touches the build (a new env var, a plugin with content detection, a formatter reading ambient state), ask what it reads that a container will not have. And before claiming a deploy works, probe **the container**, not a local server or a scratchpad harness — the four SSR chunks each verified against something more convenient than production, and each one shipped a defect that the next chunk's less convenient probe caught. Once the box pulls its images rather than building them ([image-pipeline.md](working-notes/ops/image-pipeline.md) chunk 2, and the GHCR packages flipped to public), the container to probe is the one CI built: `docker pull ghcr.io/sergeantjonas/vyoh-web:sha-<7>` and run that rather than a local `docker build`, because it is then the exact artefact the box runs instead of a second build of the same commit. Two traps in the probe itself:
...
| Hover sheen on card/tile art | `<SheenOverlay group="tile" \| "row" />` in [sheen-overlay.tsx](../apps/web/src/components/sheen-overlay.tsx); sits on a `relative overflow-hidden` parent that carries the matching `group/tile` or `group/row`. |
...
The compositor + paint baseline measured by [`tools/perf-probe`](../tools/perf-probe/) (chromium, 1440×900, dev server) defines a budget that any new surface added to the app must respect. The nine currently baselined scenarios are `lol-overview`, `lol-champion-panel`, `steam-library`, `steam-portrait`, `steam-sessions`, `recap`, `lol-recap`, `lol-patches` and `wishlist-upcoming`. Full numbers and the diagnostic trail live in [progressive-paint-audit.md](working-notes/cross-cutting/progressive-paint-audit.md).
...
| steam-sessions | ≤ 40 | ≤ 125 ms | ≤ 2 | 25–26 layers / 84–92 ms raster median ~90 / 0 long tasks / dropped=0 across a 3-run bracket (2026-09-15, final shape: primed loader, hero, strip with brush minimap, six records chips, heatmap beside the off-camera ledger). **Priming removed both long tasks and ~8 layers**: the earlier unprimed readings (31–35 layers / median ~85 ms / 2 long tasks with the strip; 31–37 / ~60 / 1 with only the heatmap) paid for a fetch-then-cascade inside the load window, where the primed route renders warm. Raster is unchanged within noise. The ceiling stays at 40 as headroom rather than being tightened to the new floor. **Unmeasured: the live variant** — while a game is open the hero pill pings, the strip's live bar pulses (both opacity, both 2.4 s) and the masthead carries a drop-shadow; the probe cannot open a game, so re-probe by hand the next time one is. First baseline the day before read 33 layers / median ~61 ms with the hero, five frosted chips and a game-art backdrop claim; the heatmap card costs ~4 layers, so the ceiling moved from 36 to 40 to record that rather than absorb it. **The first cut of the heatmap read 156–180 layers**: the LoL sibling's per-cell reveal animates `opacity` on 168 cells and each promotes its own layer for the stagger, which is invisible on `/lol/*/trends` only because that card sits below the fold. The reveal here runs once on the grid wrapper. Unlike the portrait this route is *not* loader-primed — the twelve-week payload is 89 kB — so the load phase includes the fetch and the entrance cascade. |
...
| wishlist-upcoming | ≤ 52 | ≤ 150 ms | ≤ 2 | 46 layers / 86–92 ms median ~87 / 1 long task across 3-run bracket (2026-06-11, post-chunk-4). The layer count sits above the other non-list routes by design: the imminent-hero leases the page-wide Steam backdrop and the frosted calendar wrapper + art-forward capsule tiles each promote. Raster stays low (~90 ms) because the capsule tiles are opaque art, not blur — only the calendar wrapper carries `backdrop-blur-sm`. dropped=0. Hero is skip-gated (nothing day-precise within 60 days → no hero, no lease); when skipped the layer count drops back toward the steam-library floor. The handle predates the 2026-08-11 route split and now measures `/steam/upcoming`; the wishlist list route has no scenario of its own. |
