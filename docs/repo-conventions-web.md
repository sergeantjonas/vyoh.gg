...
| steam-sessions | ≤ 40 | ≤ 125 ms | ≤ 2 | 31–35 layers / 67–90 ms raster median ~85 / 2 long tasks / dropped=0 across a 3-run bracket (2026-09-15, with the session strip, its brush minimap and the hour heatmap all mounted). The strip added ~25 ms of raster and a second long task over the heatmap-only reading (31–37 layers / median ~60 ms / 1 long task) without moving the layer count: two SVGs, no filters. Long tasks now sit exactly on the ceiling, so the next band on this route re-probes before it lands. First baseline the day before read 33 layers / median ~61 ms with the hero, five frosted chips and a game-art backdrop claim; the heatmap card costs ~4 layers, so the ceiling moved from 36 to 40 to record that rather than absorb it. **The first cut of the heatmap read 156–180 layers**: the LoL sibling's per-cell reveal animates `opacity` on 168 cells and each promotes its own layer for the stagger, which is invisible on `/lol/*/trends` only because that card sits below the fold. The reveal here runs once on the grid wrapper. Unlike the portrait this route is *not* loader-primed — the twelve-week payload is 89 kB — so the load phase includes the fetch and the entrance cascade. |
