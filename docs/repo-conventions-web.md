...
| steam-sessions | ≤ 36 | ≤ 125 ms | ≤ 2 | 33 layers / 55–101 ms raster median ~61 / 0–1 long tasks / dropped=0 across a 3-run bracket (2026-09-14, first baseline: one editorial hero, five frosted chips, a game-art backdrop claim). Seven layers above the portrait for the backdrop claim and the hero's unlock row. Unlike the portrait this route is *not* loader-primed — the twelve-week payload is 89 kB — so the load phase includes the fetch and the entrance cascade; the 101 ms outlier is the cold run. |
